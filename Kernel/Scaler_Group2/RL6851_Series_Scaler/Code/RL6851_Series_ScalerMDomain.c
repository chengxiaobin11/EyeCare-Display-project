...
//----------------------------------------------------------------------------------------------------
// ID Code      : RL6851_Series_ScalerMDomain.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------
...
#if(_DRR_SUPPORT == _ON)
    if((ScalerDrrGetType() != _DRR_NONE_TYPE) && (_FREESYNC_LINEBUFFER_PRIORITY == _ON))
    {
#if(_DISPLAY_ROTATION_SUPPORT == _ON)
        if(ScalerRotationCheckStatus() == _TRUE)
        {
...
#endif // End of #if(_DRR_SUPPORT == _ON)
    {
        ...
        if((GET_DISP_LATENCY_RATIO() != 0x00) && (ucMemoryConfig == _LINE_BUFFER_MODE))
        {
            ...
            PDATA_DWORD(0) = ScalerFRCBWCalUsage(usHwidth, usHwidth, GET_MEMORY_DATA_BIT(), _IMG_CMP_PARAM_LLB_8B_TYP, PDATA_DWORD(0), _FALSE);
...
#if(_DRR_SUPPORT == _ON)
            if(ScalerDrrGetType() != _DRR_NONE_TYPE)
            {
...
#if(_DRR_SUPPORT == _ON)
            if((GET_DRR_IVF_INFO() == _TRUE) && (ScalerDrrGetType() != _DRR_NONE_TYPE))
            {
...
void ScalerMDomainSetDisplayTimingGen(void)
{
    ...
    ScalerDisplaySetDspcLineLength(GET_MDOMAIN_OUTPUT_HBWIDTH());
}
...
bit ScalerMDomainFSLineBufferDclkDvtotalCheck(void)
{
    ...
    if(ScalerDrrGetType() != _DRR_NONE_TYPE)
    {
        ...
        // IVTotal_max
        PDATA_WORD(1) = (DWORD)g_stMDomainInputData.usHFreq * 1000 / GET_DRR_VFREQ_MIN();
        // Frame sync Dvtotal
...
void ScalerMDomainODConfig(void)
{
    ...
    {
        case _LINE_BUFFER_MODE:
            SET_OD_USE_PARAM(ScalerODGetParamSet(_MEMORY_ROW, _MEMORY_TOTAL_BW_OD_ONLY));
            break;
...
#if(_FRC_HW_AUTO_3BLOCK_TOGGLE_SUPPORT == _ON)
            if(GET_FRC_3BLOCK_TOGGLE_STATUS() == _TRUE)
            {
                SET_OD_USE_PARAM(ScalerODGetParamSet((_MEMORY_ROW / 3 * 2) - GET_FRC_TOTAL_USE_ROW(), GET_MEMORY_REMAIN_BW()));
            }
            else
#endif
            {
                SET_OD_USE_PARAM(ScalerODGetParamSet(_MEMORY_ROW - GET_FRC_TOTAL_USE_ROW(), GET_MEMORY_REMAIN_BW()));
            }

            DebugMessageFRC("4. FRC used BW", GET_MEMORY_BW());
...
WORD ScalerMDomainODCalculateMemoryUseSpace(BYTE ucODBit10x)
{
    ...
    return ((((DWORD)CEILING(_PANEL_DH_WIDTH, _OD_PIXEL_PER_COMP)) * _PANEL_DV_HEIGHT * ucODBit10x / _MEMORY_BIT_NUM) /
...
{
    WORD usOdWidth = 0;
    WORD usDVTotal = pstMemBWParam->usDVTotal;
    WORD usDVFreq = pstMemBWParam->usDVFreq;
    DWORD ulOdUsedBw = 0;

    if(pstMemBWParam->ucFBCoopEn == _TRUE)
    {
        usOdWidth = CEILING((_PANEL_DH_WIDTH / 2), _OD_PIXEL_PER_COMP) * 2;
    }
    else
    {
        usOdWidth = CEILING(_PANEL_DH_WIDTH, _OD_PIXEL_PER_COMP);
    }

    // Write or Read (unit: 10)
    ulOdUsedBw = GET_DWORD_MUL_DIV(((DWORD)usOdWidth * ucODBit10x), ((DWORD)usDVTotal * usDVFreq), ((DWORD)_MEMORY_BUS_WIDTH * 10 * 10 * 10));
    ulOdUsedBw += GET_DWORD_MUL_DIV(ulOdUsedBw, (((WORD)_DCLK_SPREAD_RANGE / _DPLL_SSCG_DIV_MODE) + _DCLK_SPREAD_RANGE_MARGIN), ((WORD)4 * 100));
...
void ScalerMDomainCheckDisplaySetting(void)
{
    ...
    if(GET_MEMORY_SELECT() != _FRAME_SYNC_LINE_BUFFER)
    {
        DWORD ulPathBwSum = GET_MEMORY_BW();
...
void ScalerMDomainGetMemBWParam(StructMemoryBWUsageParam *pstMemBWParam)
{
    ...
    pstMemBWParam->usIVTotal = GET_MDOMAIN_INPUT_VTOTAL();
    pstMemBWParam->usIVFreq = GET_MDOMAIN_INPUT_VFREQ();
    pstMemBWParam->usDVTotal = GET_MDOMAIN_OUTPUT_VTOTAL();
    pstMemBWParam->usDVFreq = g_usDVFreq;
