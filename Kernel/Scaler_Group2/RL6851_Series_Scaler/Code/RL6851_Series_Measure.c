...
//----------------------------------------------------------------------------------------------------
// ID Code      : RL6851_Series_Measure.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------
...
DWORD ScalerMeasureGetHFreq(void);
...
void ScalerMeasureInterfaceInitial(EnumSourceTypeDef enumSourceType, EnumSourceSearchPort *penumSourceSearchPort)
{
    ...
    switch(enumSourceType)
    {
        ...
        case _SOURCE_DVI:
            ...
            ScalerTMDSSetDeOnlyModeConfig(_D0_INPUT_PORT, _DE_ONLY_HS_72T);
            break;
#endif
...
bit ScalerMeasureFinalSetting(EnumSourceTypeDef enumSourceType, EnumSourceSearchPort enumSourceSearchPort)
{
    ...
    if(ScalerMeasureUpdateActiveRegion() == _FALSE)
    {
        return _FALSE;
    }
#endif
...
#if(_FRC_SUPPORT == _ON)
//--------------------------------------------------
// Description  : Calculate H frequency
// Input Value  : none
...
DWORD ScalerMeasureGetHFreq(void)
{
    ...
    ulHSPeriod = GET_DWORD_MUL_DIV(g_ulActiveRegionCount,
                                   (DWORD)g_stInputTimingData.usHTotal * 10,
                                   ulTotalCount);
