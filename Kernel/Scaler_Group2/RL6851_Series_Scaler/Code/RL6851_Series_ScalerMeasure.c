...
//----------------------------------------------------------------------------------------------------
// ID Code      : RL6851_Series_ScalerMeasure.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------
...
#if((_MEASURE_ADJUSTED_IHF_SUPPORT == _ON) || (_MEASURE_ADJUSTED_IVF_SUPPORT == _ON) || (_FRC_SUPPORT == _ON))
DWORD g_ulActiveRegionCount;
#endif
...
#if((_MEASURE_ADJUSTED_IHF_SUPPORT == _ON) || (_MEASURE_ADJUSTED_IVF_SUPPORT == _ON) || (_FRC_SUPPORT == _ON))
DWORD ScalerMeasureGetdActiveRegionPixels(WORD usHWidth, WORD usVHeight, WORD usHTotal);
DWORD ScalerMeasureGetAdjustedActiveRegionCount(WORD usNewHWidth, WORD usNewVHeight);
#endif
...
bit ScalerMeasureCheckInputInfoConsistency(EnumSourceTypeDef enumSourceType, EnumSourceSearchPort enumSourceSearchPort);

#if((_MEASURE_ADJUSTED_IHF_SUPPORT == _ON) || (_MEASURE_ADJUSTED_IVF_SUPPORT == _ON))
bit ScalerMeasureUpdateActiveRegion(void);
#endif
...
// Input Value  : usNewHWidth       --> target H width
...
DWORD ScalerMeasureGetAdjustedActiveRegionCount(WORD usNewHWidth, WORD usNewVHeight)
{
    ...
    ulTotalCount = ScalerMeasureGetdActiveRegionPixels(g_stInputTimingData.usHWidth,
                                                       g_stInputTimingData.usVHeight,
                                                       g_stInputTimingData.usHTotal);

    ulNewTotalCount = ScalerMeasureGetdActiveRegionPixels(usNewHWidth,
                                                          usNewVHeight,
                                                          g_stInputTimingData.usHTotal);

    return GET_DWORD_MUL_DIV(GET_INPUT_TIMING_ACTIVE_REGION(), ulNewTotalCount, ulTotalCount);
}
#endif
...
bit ScalerMeasureCheckInputInfoConsistency(EnumSourceTypeDef enumSourceType, EnumSourceSearchPort enumSourceSearchPort)
{
...
#if(_DVI_SUPPORT == _ON)
    // check Hstart if Get Timing form TMDS Mac (ie. DE-only mode change HS Width)
    if(((enumSourceType == _SOURCE_DVI) || (enumSourceType == _SOURCE_HDMI)) && (ScalerTMDSGetDeOnlyModeConfig(enumSourceSearchPort) == (EnumDeOnlyConfig)(_DE_ONLY_ENABLE | _DE_ONLY_HS_72T)))
    {
        if(GET_INPUT_TIMING_HSTART() <= 72)
        {
            // No need to return FALSE!
            ScalerTMDSSetDeOnlyModeConfig(enumSourceSearchPort, _DE_ONLY_HS_8T);
        }
        else if(GET_INPUT_TIMING_HSTART() <= 8)
        {
    ...
    if((enumSourceType == _SOURCE_DVI) || (enumSourceType == _SOURCE_HDMI))
    {
        ...
        if(ScalerMeasureCheckTmdsDeOnlyTiming(&g_stMeasureSyncHV, (g_stInputTimingData.usVFreq)) == _FALSE)
        {
            // check DE-only mode enable or not
            if((ScalerTMDSGetDeOnlyModeConfig(enumSourceSearchPort) & _DE_ONLY_ENABLE) == _BIT1)
            {
                // Disable DE-only mode
                ScalerTMDSSetDeOnlyModeConfig(enumSourceSearchPort, _DE_ONLY_DISABLE);
    ...
    if(abs(GET_SYNC_H_WIDTH() - GET_INPUT_TIMING_HWIDTH()) > _INPUT_TIMING_HWIDTH_DELTA)
    {
#if((_DVI_SUPPORT == _ON) || (_HDMI_SUPPORT == _ON))
        if((enumSourceType == _SOURCE_DVI) || (enumSourceType == _SOURCE_HDMI))
        {
...
{
    WORD usHwidth = GET_INPUT_TIMING_HWIDTH();
    WORD usLineCntMargin = 0;
    DWORD ulInputRegionCntBackUp = 0;
    ...
    // check active region count
    usLineCntMargin = (GET_INPUT_TIMING_ACTIVE_REGION() / GET_INPUT_TIMING_VHEIGHT()) / 2;
    ulInputRegionCntBackUp = ScalerMeasureGetInputRegionCountResult(_WAIT);

    if(ABSDWORD(GET_INPUT_TIMING_ACTIVE_REGION(), ulInputRegionCntBackUp) > usLineCntMargin)
    {
        DebugMessageMeasure("[Careful] Check active region count change", GET_INPUT_TIMING_ACTIVE_REGION());
        return _FALSE;
    }
...
void ScalerMeasureDynamicColorInitial(WORD usDHStart, WORD usDHEnd, WORD usDVStart, WORD usDVEnd)
{
    ...
    usIHStart = ScalerMeasureTransDDomainToIDomainHorizonalPosition(g_stInputTimingData.usHWidth, usDHStart);
    usIHEnd = ScalerMeasureTransDDomainToIDomainHorizonalPosition(g_stInputTimingData.usHWidth, usDHEnd);
    usIVStart = ScalerMeasureTransDDomainToIDomainVerticalPosition(g_stInputTimingData.usVHeight, usDVStart);
...
bit ScalerMeasureDynamicColorGetResult(BYTE *pucColorAverageValue, bit bWait)
{
    ...
    // pop up H/V start
    ScalerMeasureGetAutoResult(&stAutoData, ((ScalerGetBit(P0_7D_AUTO_ADJ_CTRL1, _BIT0) != 0x00) ? _WAIT : _NOT_WAIT));
