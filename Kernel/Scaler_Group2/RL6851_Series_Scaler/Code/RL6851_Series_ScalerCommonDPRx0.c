...
//----------------------------------------------------------------------------------------------------
// ID Code      : RL6851_Series_ScalerCommonDPRx0.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------
...
BYTE g_ucDpRx0AuxModeBackup_EXINT;
#endif
...
WORD ScalerDpRx0GetDecodeErrorCount(BYTE ucDpRx0LaneNumber);
...
bit ScalerDpRx0InterlaceVtotalGetMSACheck(StructDpInfo *pstDpInfo);
bit ScalerDpRx0InterlaceCheck(StructDpInfo *pstDpInfo);
bit ScalerDpRx0DeOnlyTimngCheck(StructDpInfo *pstDpInfo, DWORD ulFrameRate);
bit ScalerDpRx0DeOnlyJudge(StructDpInfo *pstDpInfo);
...
void ScalerDpRx0MstReset_EXINT0(void);
#endif
...
void ScalerDpRx0MstToSstInterlaceReset(void);
...
#if(_DP_TX_SUPPORT == _ON)
void ScalerDpRx0UpdateSinkCount(BYTE ucSinkCount);
#endif
bit ScalerDpRx0GetMsaTimingInfo(StructDpInfo *pstDpInfo);
...
void ScalerDpRx0SwitchHDR10CmpAudioInfo(void);
#endif
#endif
...
#endif
        {
            ...
            if(ucDpLaneCount != 0x00)
            {
...
#endif
                {
                    ...
                    g_ucDpRx0LinkRate = ucDpLinkRate;
                    g_ucDpRx0LaneCount = ucDpLaneCount;
...
bit ScalerDpRx0VBIOSMSACheck(bit bDeskewEnhanced)
{
    ...
    bMSACheckResult = ScalerDpRx0VbiosAssrCheck(bMSACheckResult);
#endif
...
bit ScalerDpRx0InterlaceVtotalGetMSACheck(StructDpInfo *pstDpInfo)
{
    ...
    // Vfreq (unit : 0.01Hz)
    usVfreq = _GDI_CLK_KHZ * 1000 * 100 / pstDpInfo->stDpLinkInfo.ulVBsToBsCountN;
...
bit ScalerDpRx0InterlaceCheck(StructDpInfo *pstDpInfo)
{
    ...
    usHTotalCount = ((DWORD)pstDpInfo->stDpLinkInfo.usHBsToBsCount * _GDI_CLK_KHZ * 2 + ((pstDpInfo->stDpLinkInfo.ulLinkClockHz / 1000) >> 1)) / (pstDpInfo->stDpLinkInfo.ulLinkClockHz / 1000);
    ...
    if((ABSDWORD(pstDpInfo->stDpLinkInfo.ulVBsToBsCountN, pstDpInfo->stDpLinkInfo.ulVBsToBsCountN1)) > (usHTotalCount / 2))
    {
        ...
        {
            if(ScalerDpRx0InterlaceVtotalGetMSACheck(pstDpInfo) == _TRUE)
            {
    ...
    else
    {
        ...
        if((pstDpInfo->stDpLinkInfo.b1InterlaceFieldN != pstDpInfo->stDpLinkInfo.b1InterlaceFieldN1) && (ScalerGetBit(PB6_01_DP_VBID, _BIT2) == _BIT2))
        {
            ...
            SET_DP_RX0_INTERLACE_VTT_FW_MODE(_EVEN);
    ...
    else if(pstDpInfo->stDpLinkInfo.b1InterlaceOddMode == _TRUE)
    {
        ...
        {
            DebugMessageRx0("DP MAC RX0: Fake Interlace", 0);
            pstDpInfo->stDpLinkInfo.b1InterlaceOddMode = _FALSE;
        }
        else
        {
...
// Input Value  : StructDpInfo *pstDpInfo, ulFrameRate
...
bit ScalerDpRx0DeOnlyJudge(StructDpInfo *pstDpInfo)
{
    ...
    ulFrameRate = ScalerComputeDwordMulDiv((DWORD)_GDI_CLK_KHZ, 100000, pstDpInfo->stDpLinkInfo.ulVBsToBsCountN);

    if(ScalerDpRx0DeOnlyTimngCheck(pstDpInfo, ulFrameRate) == _TRUE)
    {
        // Line Rate (Unit : 0.01Hz)
        ulLineRate = ScalerComputeDwordMulDiv(pstDpInfo->stDpLinkInfo.ulLinkClockHz, 1, (DWORD)pstDpInfo->stDpLinkInfo.usHBsToBsCount * 2) * 100;
...
bit ScalerDpRx0GetVideoInfo(void)
{
    ...
    if((ScalerGetBit(PB6_03_MN_STRM_ATTR_MISC1, _BIT6)) == _BIT6)
    {
        ...
        ScalerRead(PB6_47_VSC7, 3, pData, _AUTOINC);
...
bit ScalerDpRx0VscCheck(void)
{
    ...
    ScalerRead(PB6_47_VSC7, 3, pucVscColorFormat, _AUTOINC);
...
void ScalerDpRx0MstReset(void)
{
    ...
    ScalerDpRx0SetDpcdBitValue(0x00, 0x20, 0x03, ~(_BIT5 | _BIT4), 0x00);
}
...
// Output Value : None
//--------------------------------------------------
void ScalerDpRx0MstReset_EXINT0(void) using 1
{
    ...
    {
        ScalerDpRx0SetDpcdValue_EXINT0(0x00, 0x00, (0x30 + pData_EXINT[0]), 0x00);
    }
    ...
    // Clear 0x111
    ScalerDpRx0SetDpcdValue_EXINT0(0x00, 0x01, 0x11, 0x00);

    // Clear 0x1C0 ~ 0x1C2
    ScalerDpRx0SetDpcdValue_EXINT0(0x00, 0x01, 0xC0, 0x00);
    ScalerDpRx0SetDpcdValue_EXINT0(0x00, 0x01, 0xC1, 0x00);
    ScalerDpRx0SetDpcdValue_EXINT0(0x00, 0x01, 0xC2, 0x00);
    ...
    // Disable MST
    ScalerSetBit_EXINT(PB_01_PHY_DIG_RESET_CTRL, ~_BIT7, 0x00);

    // Disable Read 2C0 INT
    CLR_REG_DP_RX_2C0_IRQ_EXINT();

    // Clear 0x2C0
    ScalerDpRx0SetDpcdValue_EXINT0(0x00, 0x02, 0xC0, 0x00);
    ...
    // Disable Rx Igrone Error
    ScalerSetBit_EXINT(PB7_D1_DP_IIC_SET, ~_BIT3, 0x00);

    // Message Upstream Disable
    CLR_REG_DP_RX_UPSTREAM_REPLY_EXINT();

    // Reset Rx MSG Block
    ScalerSetBit_EXINT(PB7_C0_DPCD_CTRL, ~_BIT6, _BIT6);
    ScalerSetBit_EXINT(PB7_C0_DPCD_CTRL, ~_BIT6, 0x00);
    ...
    // Clear DOWN_REP_MSG_RDY & UP_REQ_MSG_RDY Bit of DPCD Table
    ScalerDpRx0SetDpcdBitValue_EXINT0(0x00, 0x20, 0x03, ~(_BIT5 | _BIT4), 0x00);
...
bit ScalerDpRx0MstToSstInterlaceCheck(WORD *pusVTotal, BYTE *pucOddFlg)
{
    ...
    // Reset Intercae Mode
    ScalerDpRx0MstToSstInterlaceReset();
    ...
    if((*pucOddFlg) == _TRUE)
    {
        ...
        // Set Vtt odd fw mode
        SET_DP_RX0_INTERLACE_VTT_FW_MODE_MST2SST(_ODD);
    ...
    {
        // Check VBID Field Toggle and VBID Interlace Info
        if(((pData[1] & _BIT1) != (pData[2] & _BIT1)) && (ScalerGetBit(PA_29_MST2SST_VBID, _BIT2) == _BIT2))
        {
            // Enable DP Interlace
            ScalerSetBit(PA_56_INTERLACE_MODE_CONFIG, ~(_BIT7 | _BIT5 | _BIT0), _BIT7);
            ...
            // Set Vtt Even fw mode
            SET_DP_RX0_INTERLACE_VTT_FW_MODE_MST2SST(_EVEN);
...
void ScalerDpRx0UpdateSinkCount(BYTE ucSinkCount)
{
    BYTE ucRxPreSinkCount = ScalerDpRx0GetDpcdInfo(0x00, 0x02, 0x00);
    BYTE ucRxCurSinkCount = ((ucRxPreSinkCount & _BIT6) | ucSinkCount);

    ScalerDpRx0SetDpcdValue(0x00, 0x02, 0x00, ucRxCurSinkCount);

    if(ucSinkCount > 1)
    {
        ScalerDpRx0DfpDeviceDetailConfig();
    }
    else
    {
        ScalerDpRx0DfpDeviceDetailConfigReset();
    }

    if((ucRxPreSinkCount != ucRxCurSinkCount) && (ScalerSyncDpGetHotPlugStatus(_D0_INPUT_PORT) == _HIGH))
    {
        ScalerDpRx0SetDpcdBitValue(0x00, 0x02, 0x04, ~_BIT6, _BIT6);
...
bit ScalerDpRx0GetMsaTimingInfo(StructDpInfo *pstDpInfo)
{
...
#endif
    {
           ...
           (pstDpInfo->stDpTimingInfo.usVHeight == 0x00) || (pstDpInfo->stDpTimingInfo.usVSWidth == 0x00))
        {
            ...
            {
#if(_DP_MST_SUPPORT == _ON)
                if(ScalerGetBit(PB_01_PHY_DIG_RESET_CTRL, _BIT7) == 0x00)
#endif
                {
    ...
    DebugMessageRx0("DP MAC RX0: MSA HTotal", pstDpInfo->stDpTimingInfo.usHTotal);
    DebugMessageRx0("DP MAC RX0: MSA HStart", pstDpInfo->stDpTimingInfo.usHStart);
    DebugMessageRx0("DP MAC RX0: MSA HWidth", pstDpInfo->stDpTimingInfo.usHWidth);
    DebugMessageRx0("DP MAC RX0: MSA HSW", pstDpInfo->stDpTimingInfo.usHSWidth);
    DebugMessageRx0("DP MAC RX0: MSA HSP", pstDpInfo->stDpTimingInfo.b1HSP);
    DebugMessageRx0("DP MAC RX0: MSA VTotal", pstDpInfo->stDpTimingInfo.usVTotal);
    DebugMessageRx0("DP MAC RX0: MSA VStart", pstDpInfo->stDpTimingInfo.usVStart);
    DebugMessageRx0("DP MAC RX0: MSA VHeight", pstDpInfo->stDpTimingInfo.usVHeight);
    DebugMessageRx0("DP MAC RX0: MSA VSW", pstDpInfo->stDpTimingInfo.usVSWidth);
    DebugMessageRx0("DP MAC RX0: MSA VSP", pstDpInfo->stDpTimingInfo.b1VSP);
...
void ScalerDpRx0SetInitialRSV0InfoType(void)
{
    ...
    ScalerSetByte(PB6_E0_DP_SDP_RSV0_MAT_HB1, _SPD_INFO_TYPE);
#else
#if((_DP_HDR10_SUPPORT == _ON) && (_AUDIO_SUPPORT == _ON))
    // Set Audio Info Type to RSV0
    ScalerSetByte(PB6_E0_DP_SDP_RSV0_MAT_HB1, _DP_AUDIO_TIMESTAMP);
#endif
#endif
...
void ScalerDpRx0GetInfoFrameRSV0(BYTE *pucDPRx0RSV0Data)
{
    ...
    do
    {
        ScalerSetByte(PB6_D8_DP_SDP_RSV0_ADR, ucTemp);
        pucTmepData1[ucTemp] = ScalerGetByte(PB6_D9_DP_SDP_RSV0_DAT);
    ...
    do
    {
        ...
        {
            ScalerSetByte(PB6_D8_DP_SDP_RSV0_ADR, ucTemp);
            pucTmepData2[ucTemp] = ScalerGetByte(PB6_D9_DP_SDP_RSV0_DAT);
    ...
    memcpy(pucDPRx0RSV0Data, pucTmepData2, _HW_DP_INFOFRAME_RSV0_LENGTH);
}
#endif
...
void ScalerDpRx0InfoFrameRSV0Check(void)
{
...
#if(_DP_HDR10_SUPPORT == _ON)
    ucInfoFrameType = ScalerGetByte(PB6_E0_DP_SDP_RSV0_MAT_HB1);
    ...
    if(ucInfoFrameType == _HDR_INFO_TYPE)
    {
        switch(g_ucDPRx0InfoTypeCompare)
        {
            ...
            case _SPD_INFO_TYPE:
                ...
                if(GET_DP_RX0_AMD_SPD_INFO_FRAME_RECEIVED() == _TRUE)
                {
                    if(memcmp(pucDPRx0RSV0Data, g_pucDPRx0InfoTemp, _HW_DP_INFOFRAME_RSV0_LENGTH) == 0)
                    {
                        if(memcmp(g_pucDPRx0RSV0Backup, g_pucDPRx0InfoTemp, _HW_DP_INFOFRAME_RSV0_LENGTH) == 0)
                        {
                            CLR_DP_RX0_HDR_INFO_FRAME_RECEIVED();
                            memset(g_pucDPRx0HDR10Backup, 0, _HW_DP_INFOFRAME_RSV0_LENGTH);
                        }
                        else
                        {
            ...
            case _DP_AUDIO_TIMESTAMP:
                ...
                if(GET_DP_RX0_AUDIO_INFO_FRAME_RECEIVED() == _TRUE)
                {
                    // Maud:[0:15], Naud:[16:31] is constant, Check Naud (arrary[16:27])
                    if(memcmp(&pucDPRx0RSV0Data[16], &g_pucDPRx0InfoTemp[16], 12) == 0)
                    {
                        if(memcmp(g_pucDPRx0RSV0Backup, g_pucDPRx0InfoTemp, _HW_DP_INFOFRAME_RSV0_LENGTH) == 0)
                        {
                            CLR_DP_RX0_HDR_INFO_FRAME_RECEIVED();
    ...
    else
    {
        switch(g_ucDPRx0InfoTypeCompare)
        {
            ...
            case _SPD_INFO_TYPE:
                ...
                if(memcmp(pucDPRx0RSV0Data, g_pucDPRx0InfoTemp, _HW_DP_INFOFRAME_RSV0_LENGTH) == 0)
                {
                    if(memcmp(g_pucDPRx0HDR10Backup, g_pucDPRx0InfoTemp, _HW_DP_INFOFRAME_RSV0_LENGTH) == 0)
                    {
                        CLR_DP_RX0_AMD_SPD_INFO_FRAME_RECEIVED();
                        memset(g_pucDPRx0RSV0Backup, 0, _HW_DP_INFOFRAME_RSV0_LENGTH);
                    }
                    else
                    {
                        // Do nothing: confuse case, keep last time status
    ...
    ScalerSetByte(PB6_E0_DP_SDP_RSV0_MAT_HB1, ScalerDpRx0SelectInfoFrameType(ucInfoFrameType));

    // Get infoframe after switch infoframe header for check
    ScalerDpRx0GetInfoFrameRSV0(g_pucDPRx0InfoTemp);
#else
...
void ScalerDpRx0InfoFrameRSV0Initial(void)
{
    ...
    g_ucDPRx0InfoTypeCompare = ScalerGetByte(PB6_E0_DP_SDP_RSV0_MAT_HB1);

    // Set DP RSV0 info type: HDR
    ScalerSetByte(PB6_E0_DP_SDP_RSV0_MAT_HB1, _HDR_INFO_TYPE);
...
void ScalerDpRx0SwitchHDR10CmpAudioInfo(void)
{
...
#endif
    {
        ...
        ScalerDpRx0GetInfoFrameRSV0(pucRSV0Data1);
        ...
        if(memcmp(pucRSV0Data1, pucRSV0Data2, _HW_DP_INFOFRAME_RSV0_LENGTH) != 0)
        {
            ...
            {
                SET_DP_RX0_AUDIO_INFO_FRAME_RECEIVED();
                memcpy(g_pucDPRx0RSV0Backup, pucRSV0Data1, _HW_DP_INFOFRAME_RSV0_LENGTH);
            }
            else
            {
                CLR_DP_RX0_AUDIO_INFO_FRAME_RECEIVED();
