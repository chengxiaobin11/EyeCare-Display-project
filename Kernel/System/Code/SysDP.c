...
void SysDpCableDetection(void)
{
...
#endif
    {
...
#if(_D0_DP_CONNECTOR_TYPE == _DP_CONNECTOR_EMBEDDED)
                            ScalerTimerReactiveTimerEvent(0, _SCALER_TIMER_EVENT_DP_RX0_HOTPLUG_ASSERTED);
#else
        ...
        {
#if(_D6_DP_CONNECTOR_TYPE == _DP_CONNECTOR_EMBEDDED)
            ScalerTimerActiveTimerEvent(0, _SCALER_TIMER_EVENT_DP_RX0_HOTPLUG_ASSERTED);
#else
...
#endif
    {
...
#endif
#if(_D1_DP_CONNECTOR_TYPE == _DP_CONNECTOR_EMBEDDED)
                        ScalerTimerReactiveTimerEvent(0, _SCALER_TIMER_EVENT_DP_RX1_HOTPLUG_ASSERTED);
#else
        ...
        {
#if(_D1_DP_CONNECTOR_TYPE == _DP_CONNECTOR_EMBEDDED)
            ScalerTimerActiveTimerEvent(0, _SCALER_TIMER_EVENT_DP_RX1_HOTPLUG_ASSERTED);
#else
...
#endif
    {
        if(ScalerSyncDpCableStatus(_D2_INPUT_PORT) != SysDpCableStatus(_D2_INPUT_PORT))
        {
            if(SysDpCableStatus(_D2_INPUT_PORT) == _TRUE)
            {
                if((GET_DP_DC_OFF_HPD_HIGH() == _ON) || (SysPowerGetPowerStatus() != _POWER_STATUS_OFF))
                {
#if(_D2_DP_CONNECTOR_TYPE == _DP_CONNECTOR_EMBEDDED)
                    ScalerTimerReactiveTimerEvent(0, _SCALER_TIMER_EVENT_DP_RX2_HOTPLUG_ASSERTED);
#else
        ...
        {
#if(_D2_DP_CONNECTOR_TYPE == _DP_CONNECTOR_EMBEDDED)
            ScalerTimerActiveTimerEvent(0, _SCALER_TIMER_EVENT_DP_RX2_HOTPLUG_ASSERTED);
#else
