...
{
	if(GET_MODE_STATE() == _MODE_STATUS_ACTIVE && (GET_INPUT_TIMING_VFREQ()>580 &&GET_INPUT_TIMING_VFREQ()<620)\
		&& GET_INPUT_TIMING_HWIDTH() == 1920 && GET_INPUT_TIMING_VHEIGHT() == 1080 )
...
void SysModeDrrSetMdomainInfo(void)
{
    ...
    if(ScalerDrrGetType() != _DRR_NONE_TYPE)
    {
        ...
        ScalerDrrSetPanelVfeqInfo(enumSearchPort);
...
WORD SysModeDrrGetIVFreq(EnumSourceSearchPort enumSearchPort)
{
    BYTE ucPortType = GET_PORT_TYPE(SysSourceGetInputPortType(enumSearchPort));
    ...
    ScalerFreeSyncGetSourceInfo(enumSearchPort, ucPortType, &stSourceInfo);
    ...
    if(ScalerDrrGetType() != _DRR_NONE_TYPE)
    {
        ...
        if((GET_DRR_VFREQ_MAX() >= GET_DRR_PANEL_MIN_FRAME_RATE()) &&
           ...
           (ScalerDrrVfreqMaxInfoEnable(enumSearchPort, ucPortType) == _TRUE))
        {
            DebugMessageScaler("DRR Get IVF from SPD info", GET_DRR_VFREQ_MAX());
            return GET_DRR_VFREQ_MAX();
        }
    }
    else
#endif
    {
        ...
        if(ScalerFreeSyncVfreqMaxInfoEnable(enumSearchPort, ucPortType) == _TRUE)
        {
    ...
    if(ScalerFreeSyncCalculateIVFmaxFromCVT(&usIVfreq, &stSourceInfo, ucPortType) == _TABLE_SEARCHING_WITH_IVF_MAX)
    {
