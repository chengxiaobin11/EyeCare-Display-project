...
WORD UserCommonAdjustOCCPCMGammaGetTableAddress(EnumOCCPCMGammaType enumOCCPCMGammaType);
WORD UserCommonAdjustOCCPCMGammaGetsRGBCTTableAddress(EnumOCCPCMGammaCTMatrix enumOCCPCMGammaCTMatrix, EnumOGCModeSelect enumOGCModeSelect);
#endif
#endif
...
void UserCommonAdjustColorSetDynamicColorInitialBoundary(WORD usDHStart, WORD usDHEnd, WORD usDVStart, WORD usDVEnd);
void UserCommonAdjustColorSetAutoMeasure(BYTE ucColor);
bit UserCommonAdjustColorGetDynamicColorMeasureResult(BYTE *pucMeasureResult, bit bWait);
#endif
...
void UserCommonAdjustDigitalFilterEnhancePhase(void)
{
    ...
    {
        for(ucI = 1; tDF_ENHANCE_PHASE_SETTING[ucI * 4] < usPixelRate; ucI++) {}
...
void UserCommonAdjustRingingFilter(void)
{
    ...
    {
        for(ucI = 1; tRF_OFFSET_COEF_SETTING[ucI * 2] < usPixelRate; ucI++) {}
...
// BYTE UserCommonAdjustRealValueToPercent_BACK(WORD usRealValue, WORD usMax, WORD usMin, WORD usCenter)
// {
// 	WORD usTemp = 0;
// 	BYTE usoSDNumCenter = _OSD_BACKLIGHT_DEFAULE;//_BACKLIGHT_DEFAULT;//50;
// 	if(usMax == usCenter)
// 	{
// 		usoSDNumCenter = 100;
// 	}
...
// 	if((usMax <= usMin) || (usCenter <= usMin) || (usCenter > usMax))	// || (usCenter >= usMax))
// 	{
// 		usTemp = 0;
// 	}
// 	else
...
// 		if(usMax == usCenter)
// 		{
// 			if(usRealValue == usMax)
// 			{
// 				return 100;//usoSDNumCenter;
// 			}
// 			else
// 			{
// 				usTemp = (((DWORD)(usRealValue - usMin) * 10000) / (usCenter - usMin));
				
// 			}
// 		}

// 		if(usMax == usRealValue)
// 		{
// 			return 100;
// 		}
// 		else if(usRealValue < usCenter)
// 		{
// 			usTemp = (((DWORD)(usRealValue - usMin) * 10000) / (usCenter - usMin)) * 75 / 100;
// 		}
// 		else if(usRealValue > usCenter)
// 		{
// 			usTemp = (((DWORD)(usRealValue - usCenter) * 10000) / (usMax - usCenter)) * 75 / 100 + 5000;
// 		}
// 		else
// 		{
// 			return usoSDNumCenter;
// 		}
// 	}
...
BYTE *UserCommonAdjustDICOMAddr(EnumDICOMTypeSelect enumDICOMTypeSelect)
{
    ...
    else if(enumDICOMTypeSelect == _DICOM3_TYPE)
    {
        pucOutputGammaTypeAddr = g_pucDISP_CALIBRATION_FLASH + _OGC_DICOM3_ADDRESS;
    }
    else if(enumDICOMTypeSelect == _DICOM4_TYPE)
    {
        pucOutputGammaTypeAddr = g_pucDISP_CALIBRATION_FLASH + _OGC_DICOM4_ADDRESS;
    }
    else
#endif
    {
...
void UserCommonAdjustPCM(EnumPCMType enumPCMMode, BYTE *pucIGammaTableArray, BYTE *pucOGammaTableArray, BYTE *pucColorMatrixArray, BYTE ucPCMBankNum, WORD usLength)
{
    ...
    if(enumGammaIndex >= (sizeof(tusDIMMING_GAMMA_OSD_INDEX) / sizeof(tusDIMMING_GAMMA_OSD_INDEX[0])))
    {
        enumGammaIndex = 0;
    }
#endif
...
#if(_OCC_PCM_GAMMA_SUPPORT == _ON)
        if(enumPCMMode == _PCM_OCC_GAMMA)
        {
            ScalerColorOCCPCMGammaLoadTable(pucIGammaTableArray, (g_pucDISP_CALIBRATION_FLASH + _DIMMING_OCC_GAMMA10_ADDRESS), pucColorMatrixArray, _OGC_FLASH_BANK, usLength, _OCC_GAMMA_FLASH_BANK);
        }
        else
#endif
        {
            ...
            {
                ScalerColorPCMLoadTable(_PCM_DISPLAY_P3, pucIGammaTableArray, (g_pucDISP_CALIBRATION_FLASH + _DIMMING_OCC_GAMMA10_ADDRESS), pucColorMatrixArray, ucPCMBankNum, usLength, ucGammaBankNum);
            }
            else
#endif
            {
...
#endif // End of #if(_LOCAL_DIMMING_SUPPORT == _ON)
    {
...
#endif
        {
            ...
            if(enumPCMMode == _PCM_OCC_GAMMA)
            {
                ScalerColorOCCPCMGammaLoadTable(pucIGammaTableArray, pucOGammaTableArray, pucColorMatrixArray, _OGC_FLASH_BANK, usLength, _OCC_GAMMA_FLASH_BANK);
            }
            else
#endif
            {
...
WORD UserCommonAdjustOCCPCMGammaGetTableAddress(EnumOCCPCMGammaType enumOCCPCMGammaType)
{
    return ScalerColorOCCPCMGammaGetTableAddress(enumOCCPCMGammaType);
}
...
void UserCommonAdjustColorSetDynamicColorInitialBoundary(WORD usDHStart, WORD usDHEnd, WORD usDVStart, WORD usDVEnd)
{
    ...
    ScalerMeasureDynamicColorInitial(usDHStart, usDHEnd, usDVStart, usDVEnd);
}
...
void UserCommonAdjustColorSetAutoMeasure(BYTE ucColor)
{
    ScalerMeasureDynamicColorSetAutoMeasure(ucColor);
}
...
bit UserCommonAdjustColorGetDynamicColorMeasureResult(BYTE *pucMeasureResult, bit bWait)
{
    ...
    if(ScalerMeasureDynamicColorGetResult(&stAutoData.ucColorAverageValue, bWait) == _FALSE)
    {
        return _FALSE;
    }
