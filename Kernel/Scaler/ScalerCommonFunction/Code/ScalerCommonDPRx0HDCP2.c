...
void ScalerDpRx0Hdcp2ClearDpcdReadValue(BYTE ucRSAType, WORD usLength);
...
void ScalerDpRx0Hdcp2SkeProc_WDINT(void);
void ScalerDpRx0Hdcp2HPrimeProc_WDINT(void);
void ScalerDpRx0Hdcp2LoadDpcdValue_WDINT(BYTE ucDpcdAccessPortH, BYTE ucDpcdAccessPortM, BYTE ucDpcdAccessPortL, BYTE ucLength, BYTE *pucReadArray);
void ScalerDpRx0Hdcp2AkeInitProc_WDINT(void);
void ScalerDpRx0Hdcp2ClearDpcdReadValue_WDINT(BYTE ucRSAType, WORD usLength);
void ScalerDpRx0Hdcp2SetDpcdReadValue_WDINT(BYTE ucRSAType, BYTE *pucReadArray, WORD usLength);
#endif
...
void ScalerDpRx0Hdcp2RepeaterCapableProc(void);
void ScalerDpRx0Hdcp2ClearDpcdRepeaterInfo(void);
#endif
...
void ScalerDpRx0Hdcp2Handler(void)
{
    ...
    if(GET_DP_RX0_HDCP2_AKE_INIT_RECEIVED() == _TRUE)
    {
        ...
        ScalerDpRx0Hdcp2ClearDpcdRepeaterInfo();
#endif
    ...
    if(ScalerGetBit_EXINT(PB7_DC_AUX_IRQ_STATUS, _BIT7) == _BIT7)
    {
        ...
        switch(ucMessageId)
        {
            case _HDCP_2_2_AKE_INIT:
                ...
                ScalerTimerWDCancelTimerEvent_EXINT0(_SCALER_WD_TIMER_EVENT_DP_RX0_HDCP2_H_PRIME);

                ScalerTimerWDCancelTimerEvent_EXINT0(_SCALER_WD_TIMER_EVENT_DP_RX0_HDCP2_AKE_INIT);

                // Execute AKE_INIT Procedure after 3msec
                ScalerTimerWDActivateTimerEvent_EXINT0(3, _SCALER_WD_TIMER_EVENT_DP_RX0_HDCP2_AKE_INIT);
#endif
            ...
            case _HDCP_2_2_AKE_NO_STORED_KM:
                ...
                if(GET_DP_RX0_HDCP2_CLEAR_DPCD_READ_VALUE_DONE() == _FALSE)
                {
                    ScalerTimerWDCancelTimerEvent_EXINT0(_SCALER_WD_TIMER_EVENT_DP_RX0_HDCP2_AKE_INIT);
            ...
            case _HDCP_2_2_AKE_STORED_KM:
                ...
                ScalerTimerWDActivateTimerEvent_EXINT0(4, _SCALER_WD_TIMER_EVENT_DP_RX0_HDCP2_H_PRIME);
            ...
            case _HDCP_2_2_SKE_SEND_EKS:
                ...
                ScalerTimerWDActivateTimerEvent_EXINT0(4, _SCALER_WD_TIMER_EVENT_DP_RX0_HDCP2_SKE);
            ...
            case _HDCP_2_2_SKE_TYPE_VALUE:
                ...
                {
                    if(GET_DP_RX0_HDCP2_AUTHENTICATION_DONE() == _TRUE)
                    {
...
void ScalerDpRx0Hdcp2ResetProc(void)
{
...
#if(_WD_TIMER_INT_SUPPORT == _ON)
    ScalerTimerWDCancelTimerEvent(_SCALER_WD_TIMER_EVENT_DP_RX0_HDCP2_AKE_INIT);
#endif

    ScalerDpRx0Hdcp2ClearDpcdReadValue(_HDCP_2_2_RX_H_PRIME, 32);
    ScalerDpRx0Hdcp2ClearDpcdReadValue(_HDCP_2_2_RX_L_PRIME, 32);
    ScalerDpRx0Hdcp2ClearDpcdReadValue(_HDCP_2_2_RX_M_PRIME, 32);
    ScalerDpRx0Hdcp2ClearDpcdReadValue(_HDCP_2_2_RX_PAIRING_INFO, 16);
    ...
    ScalerDpRx0Hdcp2ClearDpcdReadValue(_HDCP_2_2_RX_V_PRIME, 190);
#endif
}
//--------------------------------------------------
...
void ScalerDpRx0Hdcp2ResetProc_EXINT0(void) using 1
{
    ...
    CLR_DP_RX0_HDCP2_SKE_SEND_EKS_RECEIVED();
    CLR_DP_RX0_HDCP2_AUTHENTICATION_ACTIVE();
    CLR_DP_RX0_HDCP2_CLEAR_DPCD_READ_VALUE_DONE();
...
void ScalerDpRx0Hdcp2ClearDpcdReadValue(BYTE ucRSAType, WORD usLength)
{
    ...
    for(pData[0] = 0; pData[0] < usLength; pData[0]++)
    {
        ScalerDpRx0SetDpcdValue(pData[1], pData[2], pData[3], 0x00);
...
#if(_WD_TIMER_INT_SUPPORT == _ON)
//--------------------------------------------------
// Description  : WD Timer Event Proc of DP RX0 HDCP2 SKE Proc
// Input Value  :
// Output Value : None
//--------------------------------------------------
void ScalerDpRx0Hdcp2SkeProc_WDINT(void) using 3
{
    if(GET_DP_RX0_HDCP2_AUTHENTICATION_DONE() == _TRUE)
    {
        // Disable HDCP2.2 Auth. Done
    ...
    // Calculate Dkey 2
    ScalerHdcp2DkeyCalculate_WDINT(_HDCP_2_2_DKEY_2, g_pucDpRx0Hdcp2Rtx, g_pucDpRx0Hdcp2Rrx, g_pucDpRx0Hdcp2Km, g_pucDpRx0Hdcp2Rn);

    // Save Edkey_ks to g_pucHdcp2EDkeyKs
    ScalerDpRx0Hdcp2LoadDpcdValue_WDINT(0x06, 0x93, 0x18, 16, g_pucHdcp2EDkeyKs);

    // Save Riv to g_pucHdcp2Riv
    ScalerDpRx0Hdcp2LoadDpcdValue_WDINT(0x06, 0x93, 0x28, 8, g_pucHdcp2Riv);
    ...
    if(ScalerDpRx0GetDpcdBitInfo_WDINT(0x06, 0x92, 0x1F, (_BIT1 | _BIT0)) == _BIT1)
    {
        // Enable HDCP2.2 Auth. Done
        ScalerSetBit_EXINT(PB_20_HDCP_DEBUG, ~_BIT4, _BIT4);
...
// Output Value : None
//--------------------------------------------------
void ScalerDpRx0Hdcp2HPrimeProc_WDINT(void) using 3
{
    BYTE ucBackUp = ScalerGetByte_EXINT(PB7_D0_AUX_MODE_SET);

    SET_DP_RX0_AUX_MANUAL_MODE_EXINT();

    // Calculate H'
    if(ScalerHdcp2HPrimeCalculate_WDINT(g_pucDpRx0Hdcp2dKey0, g_pucDpRx0Hdcp2dKey1, g_pucDpRx0Hdcp2Rtx, g_pucDpRx0Hdcp2TxCaps, g_pucDPRx0Caps) == _TRUE)
    {
        for(pData_EXINT[0] = 0; pData_EXINT[0] < 32; pData_EXINT[0]++)
        {
            g_pucHdcp2Sha256[pData_EXINT[0]] = ScalerGetByte_EXINT(P63_35_HDCP_SHA_0 + pData_EXINT[0]);
        ...
        // Load H prime to DPCD Table
        ScalerDpRx0Hdcp2SetDpcdReadValue_WDINT(_HDCP_2_2_RX_H_PRIME, g_pucHdcp2Sha256, 32);
    }

    ScalerDpRx0CpIRQ_WDINT(_DP_HDCP2_H_PRIME_AVAILABLE);
...
// Output Value : None
//--------------------------------------------------
void ScalerDpRx0Hdcp2LoadDpcdValue_WDINT(BYTE ucDpcdAccessPortH, BYTE ucDpcdAccessPortM, BYTE ucDpcdAccessPortL, BYTE ucLength, BYTE *pucReadArray) using 3
{
    BYTE ucIndex = 0x00;
    ...
    {
        pucReadArray[ucIndex] = ScalerDpRx0GetDpcdInfo_WDINT(ucDpcdAccessPortH, HIBYTE(usDpcdAddress), LOBYTE(usDpcdAddress));
...
// Output Value : None
//--------------------------------------------------
void ScalerDpRx0Hdcp2AkeInitProc_WDINT(void) using 3
{
    ScalerDpRx0Hdcp2ClearDpcdReadValue_WDINT(_HDCP_2_2_RX_H_PRIME, 32);
    ScalerDpRx0Hdcp2ClearDpcdReadValue_WDINT(_HDCP_2_2_RX_L_PRIME, 32);
    ScalerDpRx0Hdcp2ClearDpcdReadValue_WDINT(_HDCP_2_2_RX_M_PRIME, 32);
    ScalerDpRx0Hdcp2ClearDpcdReadValue_WDINT(_HDCP_2_2_RX_PAIRING_INFO, 16);

    SET_DP_RX0_HDCP2_CLEAR_DPCD_READ_VALUE_DONE();
}
...
// Output Value : None
//--------------------------------------------------
void ScalerDpRx0Hdcp2ClearDpcdReadValue_WDINT(BYTE ucRSAType, WORD usLength) using 3
{
    switch(ucRSAType)
    {
        case _HDCP_2_2_RRX:
    ...
    for(pData_EXINT[0] = 0; pData_EXINT[0] < usLength; pData_EXINT[0]++)
    {
        ScalerDpRx0SetDpcdValue_WDINT(pData_EXINT[1], pData_EXINT[2], pData_EXINT[3], 0x00);
        PDATA_WORD_EXINT(1) = PDATA_WORD_EXINT(1) + 1;
    }
}
...
// Output Value : None
//--------------------------------------------------
void ScalerDpRx0Hdcp2SetDpcdReadValue_WDINT(BYTE ucRSAType, BYTE *pucReadArray, WORD usLength) using 3
{
    switch(ucRSAType)
    {
        case _HDCP_2_2_RRX:
    ...
    for(pData_EXINT[0] = 0; pData_EXINT[0] < usLength; pData_EXINT[0]++)
    {
        ScalerDpRx0SetDpcdValue_WDINT(pData_EXINT[1], pData_EXINT[2], pData_EXINT[3], pucReadArray[pData_EXINT[0]]);
        PDATA_WORD_EXINT(1) = PDATA_WORD_EXINT(1) + 1;
    }
}
#endif
...
void ScalerDpRx0Hdcp2ClearDpcdRepeaterInfo(void)
{
    ...
    if((ScalerDpRx0GetDpcdInfo(0x06, 0x80, 0x2A) != 0x00) || (ScalerDpRx0GetDpcdInfo(0x06, 0x80, 0x2B) != 0x00))
    {
        ...
        for(ucIndex = 0; ucIndex < 20; ucIndex++)
        {
            ...
            ScalerDpRx0SetDpcdValue(0x06, 0x80, (0x14 + ucIndex), 0x00);
        }

        for(ucIndex = 0; ucIndex < 15; ucIndex++)
        {
            // DPCD [0x6802C ~ 0x6803A]
            ScalerDpRx0SetDpcdValue(0x06, 0x80, (0x2C + ucIndex), 0x00);
        }
    }
}
