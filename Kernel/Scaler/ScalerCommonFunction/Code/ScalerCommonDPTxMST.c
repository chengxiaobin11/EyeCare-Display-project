...
//----------------------------------------------------------------------------------------------------
// ID Code      : ScalerCommonDPTxMST.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------

#define __SCALER_COMMON_DPTXMST__
...
StructTxUpRequestMsgHeaderInfo g_stUpReqMsgHeaderInfo;
BYTE g_pucDownReplyBuf0Data[259];
BYTE g_pucDownReplyBuf1Data[259];
BYTE g_pucUpRequestBackUp[30];
UnionTxEdidReadPos g_punTxPortEdidReadPosition[1];

#if(_DP_MST_TX_INTERCEPT_DDCCI_MSG_SUPPORT == _ON)
bit g_bDPTxInterceptDdcciMsg;
bit g_bDPTxInterceptDdcciMsgAdjustLCR;
#endif

//****************************************************************************
// FUNCTION DECLARATIONS
//****************************************************************************
void ScalerDpTxMstMsgHandler(void);
...
bit ScalerDpTxMstSelfGenCSNTimerEventCheck(void);
...
bit ScalerDpTxMstI2CWrite(BYTE ucI2CDeviceID, BYTE ucWriteNum, BYTE *pucData, bit bNoStop);
...
bit ScalerDpTxMstGetStreamAllocate(void);
#if(_DP_HDCP_2_2_SUPPORT == _ON)
void ScalerDpTxMstQueryEncryptionWriteSha256Input(bit bRepeater);
bit ScalerDpTxMstQueryEncryptionCompareSha256Result(void);
#endif
...
void ScalerDpTxMstReset_EXINT0(void);
#endif

#if(_DP_MST_TX_INTERCEPT_DDCCI_MSG_SUPPORT == _ON)
void ScalerDpTxMstInterceptDdcciMsgSetting(bit bStatus);
#endif
...
void ScalerDpTxMstDownReplyProc(void)
{
    ...
    if((GET_DP_TX_DOWN_REPLY_READY() == _TRUE) && (GET_DP_TX_RD_BUFFER_STATUS() != _DP_TX_READ_BUF_UP_REQUEST_IN_USE) && (GET_DP_TX_DOWN_REPLY_READ_DISABLE() == _FALSE))
    {
        ...
        for(ucRetry = 0; ucRetry < 20; ucRetry++)
        {
            ...
            if((ucRetry == 0) || (ScalerGetBit(PBD_B1_AUX_IRQ_EVENT, _BIT1) == _BIT1))
            {
                // Clear AUX IRQ Flag
            ...
            if(ScalerTimerPollingFlagProc(5, PBD_CC_MSG_RD_FINISH, _BIT1, _TRUE) == _TRUE)
            {
                // CLR SBMSG Finish Flag
                CLR_REG_DP_TX_UPSTREAM_SBMSG_FINISH_FLAG();

                if(GET_REG_DP_TX_UPSTREAM_MSG_BODY_LENGTH() != 0x00)
                {
...
void ScalerDpTxMstDownRequestAnal(void)
{
    BYTE ucLCR = GET_REG_DP_RX_DOWN_REQUEST_MSG_LCR();
...
#if(_DP_MST_TX_INTERCEPT_DDCCI_MSG_SUPPORT == _ON)
        if(GET_DP_TX_INTERCEPT_DDCCI_MSG_ADJUST_LCR() == _TRUE)
        {
            ucLCR = 0;
        }
#endif

        if((ucLCR > 0) && (GET_DP_TX_DOWNSTREAM_MSGCAP_STATUS() == _DP_MSG_SUPPORT))
        {
...
void ScalerDpTxMstUpRequestProc(void)
{
    ...
    if((GET_DP_TX_UP_REQUEST_READY() == _TRUE) && (GET_DP_TX_RD_BUFFER_STATUS() != _DP_TX_READ_BUF_DOWN_REPLY_IN_USE) && (GET_DP_TX_DOWN_REPLY_READ_DISABLE() == _FALSE))
    {
        ...
        for(ucRetry = 0; ucRetry < 20; ucRetry++)
        {
            ...
            if(ScalerTimerPollingFlagProc(5, PBD_CC_MSG_RD_FINISH, _BIT1, _TRUE) == _TRUE)
            {
                ...
                {
                    SET_DP_TX_UP_REQUEST_MSG_LENGTH(GET_REG_DP_TX_UPSTREAM_MSG_LENGTH());

                    SET_DP_TX_UP_REQUEST_MSG_LCR(GET_REG_DP_TX_UPSTREAM_MSG_LCR());

                    SET_DP_TX_UP_REQUEST_MSG_LCT(GET_REG_DP_TX_UPSTREAM_MSG_LCT());

                    SET_DP_TX_UP_REQUEST_MSG_BROADCAST(GET_REG_DP_TX_UPSTREAM_MSG_BROADCAST());

                    SET_DP_TX_UP_REQUEST_MSG_PATH(GET_REG_DP_TX_UPSTREAM_MSG_PATH());

                    SET_DP_TX_UP_REQUEST_MSG_MSN(GET_REG_DP_TX_UPSTREAM_MSG_MSN());

                    SET_DP_TX_UP_REQUEST_MSG_READ_FINISH(GET_REG_DP_TX_UPSTREAM_MSG_READ_FINISH());
    ...
    {
        if((GET_DP_TX_UP_REQUEST_MSG_READ_FINISH() == _TRUE) && (GET_DP_REQUEST_POOL_FREE() == _TRUE) && (GET_DP_RX_REPLY_BUF_FREE() == _TRUE))
        {
            // Occupy Request Pool
            ...
            for(pData[0] = 0; pData[0] < GET_DP_TX_UP_REQUEST_MSG_LENGTH(); pData[0]++)
            {
            ...
            if(GET_REG_DP_TX_UPSTREM_MSG_CRC_STATUS() == (_BIT1 | _BIT0))
            {
                switch(GET_DP_MSG_REQUEST_ID())
                {
                    case _DP_MSG_CONNECTION_STATUS_NOTIFY:
                        ...
                        if(ScalerDpTxMstSelfGenCSNTimerEventCheck() == _FALSE)
                        {
                            if((GET_DP_TX_UP_REQUEST_MSG_LCR() > 0) && (GET_DP_TX_UP_REQUEST_MSG_LCR() >= GET_DP_TX_UP_REQUEST_BACKUP_DATA(_UP_REQ_BACK_UP_LCR)))
                            {
                                ...
                                if(((GET_DP_TX_UP_REQUEST_MSG_LCR() == GET_DP_TX_UP_REQUEST_BACKUP_DATA(_UP_REQ_BACK_UP_LCR))) &&
                                   (((GET_DP_MSG_REQUEST_ARRAY(18) & _BIT5) >> 5) == _DP_PLUG) && (GET_DP_TX_UP_REQUEST_BACKUP_DATA(_UP_REQ_BACK_UP_DP_STATUS) == _DP_UNPLUG))
                                {
                                    SET_DP_TX_BYPASS_UNPLUG_CSN_IS_PENDING();
                                    DebugMessageMst("Unplug event is pending!!", 0);
                                }
                                else
                                {
                                ...
                                // Set LCT = 1
                                SET_DP_TX_UP_REQUEST_BACKUP_DATA(_UP_REQ_BACK_UP_LCT, GET_DP_TX_UP_REQUEST_MSG_LCT());

                                // Set LCR = 6
                                SET_DP_TX_UP_REQUEST_BACKUP_DATA(_UP_REQ_BACK_UP_LCR, GET_DP_TX_UP_REQUEST_MSG_LCR());

                                // Set Broadcast and Path Bit
                                SET_DP_TX_UP_REQUEST_BACKUP_DATA(_UP_REQ_BACK_UP_BROADCAST, GET_DP_TX_UP_REQUEST_MSG_BROADCAST());
                                SET_DP_TX_UP_REQUEST_BACKUP_DATA(_UP_REQ_BACK_UP_PATH, GET_DP_TX_UP_REQUEST_MSG_PATH());

                                SET_DP_TX_UP_REQUEST_BACKUP_DATA(_UP_REQ_BACK_UP_MSG_MSN, GET_DP_TX_UP_REQUEST_MSG_MSN());

                                // Set MSG Body Length
                                SET_DP_TX_UP_REQUEST_BACKUP_DATA(_UP_REQ_BACK_UP_MSG_LENGTH, GET_DP_TX_UP_REQUEST_MSG_LENGTH());
                                ...
                                if(GET_DP_MSG_REQUEST_ID() == _DP_MSG_CONNECTION_STATUS_NOTIFY)
                                {
                                    ...
                                    SET_DP_TX_DOWNSTREAM_SINK_COUNT(pData[0] & 0xBF);

                                    if((GET_DP_TX_UP_REQUEST_BACKUP_DATA(_UP_REQ_BACK_UP_DP_STATUS) == _TRUE) && (GET_DP_TX_BYPASS_UNPLUG_CSN_IS_PENDING() == _FALSE))
                                    {
                                        // Set Rx Sink Count
            ...
            switch(GET_DP_GEN_UP_REQUEST_MSG())
            {
                case _DP_MSG_CONNECTION_STATUS_NOTIFY:
                    ...
                    if(GET_DP_TX_UP_REQUEST_BYPASS_REGEN() == _TRUE)
                    {
                        ...
                        // Set LCR = 6
                        SET_REG_DP_RX_UPSTREAM_REPLY_LCR(GET_DP_TX_UP_REQUEST_BACKUP_DATA(_UP_REQ_BACK_UP_LCR) - 1);
                        ...
                        {
                            SET_DP_REQUEST_MSG_CONNECTION_STATUS_DP_PLUG_STATUS(_DP_UNPLUG);
                        }
                        else
                        {
                    ...
                    else
                    {
                        ...
                        if(GET_DP_RX_REPORT_DOWNSTREAM_DP_CABLE_STATUS() == _TRUE)
                        {
                            // Set Rx Sink Count
                            pData[0] = ((GET_REG_DP_DPCD_INFO(0x00, 0x02, 0x00) & _BIT6) | (GET_DP_TX_DOWNSTREAM_SINK_COUNT() + 1));
                            ...
                            ScalerTimerActiveTimerEvent(_DP_MST_TX_FORCE_REBUILD_TOPOLOGY_TIMEOUT, _SCALER_TIMER_EVENT_DP_TX_FORCE_REBUILD_TOPOLOGY_EVENT);
                        }
                        else
                        {
                            // Set Rx Sink Count
                            pData[0] = ((GET_REG_DP_DPCD_INFO(0x00, 0x02, 0x00) & _BIT6) | 1);
...
//--------------------------------------------------
// Description  : Dp Tx Mst Self Gen CSN Check
// Input Value  : None
// Output Value : _TRUE(Have a Self Gen CSN) or _FALSE()
//--------------------------------------------------
bit ScalerDpTxMstSelfGenCSNTimerEventCheck(void)
{
...
void ScalerDpTxMstDownRequestEventProc(void)
{
    switch(GET_DP_TX_DOWN_REQUEST_EVENT())
    {
        ...
        case _DP_TX_DPCD_READ:
            ...
            {
#if(_HW_DP_TX_VSC_SDP_TRANSMIT_SUPPORT == _OFF)
                // While MST2SST mode, force to disable the VSC_SDP capability (02210h)
                if(GET_DP_TX_DOWNSTREAM_MSGCAP_STATUS() == _DP_MSG_NO_SUPPORT)
                {
                    // Check the start address of DPCD which is <= 02210h
                    if((GET_DP_DOWN_REQUEST_REMOTE_DPCD_READ_MSG_ADDRESS_H() == 0x00) && (GET_DP_DOWN_REQUEST_REMOTE_DPCD_READ_MSG_ADDRESS_M() == 0x22) && (GET_DP_DOWN_REQUEST_REMOTE_DPCD_READ_MSG_ADDRESS_L() <= 0x10))
                    {
                        // Check the end address of DPCD which is >= 02210h
                        if((GET_DP_DOWN_REQUEST_REMOTE_DPCD_READ_MSG_ADDRESS_L() + GET_DP_DOWN_REQUEST_REMOTE_DPCD_READ_MSG_NUM_OF_READ() - 1) >= 0x10)
                        {
                            // Set the reply data byte to zero which position is matching to 02210h
                            SET_DP_DOWN_REPLY_REMOTE_DPCD_READ_MSG_DATA(0x10 - GET_DP_DOWN_REQUEST_REMOTE_DPCD_READ_MSG_ADDRESS_L()) = 0x00;
                        }
                    }
                }
#endif
        ...
        case _DP_TX_I2C_WRITE:
            ...
            if(ScalerDpTxMstI2CWrite(GET_DP_DOWN_REQUEST_REMOTE_I2C_WRITE_MSG_WRITE_DEVICE_ID(),
                                     GET_DP_DOWN_REQUEST_REMOTE_I2C_WRITE_MSG_NUM_OF_WRITE(),
                                     &GET_DP_DOWN_REQUEST_REMOTE_I2C_WRITE_MSG_DATA_TO_WRITE(0), 0) == _TRUE)
            {
                SET_DP_TX_MSG_DOWN_REQUEST_ANAL_FINISH();
            }
            else
        ...
        case _DP_TX_QUERY_STREAM_ENCRYPTION_STATUS:
            ...
            if(GET_DP_DOWN_REPLY_QUERY_STREAM_ENCRYPTION_STATUS_MSG_SIGNED() == 1)
            {
                ...
                if(GET_DP_TX_HDCP2_MODE() == _ENABLE)
                {
                    ...
                    ScalerDpTxMstQueryEncryptionWriteSha256Input(GET_DP_DOWN_REPLY_QUERY_STREAM_ENCRYPTION_STATUS_MSG_STREAM_REPEATER_PRESENT());

                    // Caompare Signature L with L' from Downstream
                    SET_DP_DOWN_REPLY_QUERY_STREAM_ENCRYPTION_STATUS_MSG_SIGNED(ScalerDpTxMstQueryEncryptionCompareSha256Result());
                }
                else
#endif
                {
...
#if(_DP_HDCP_2_2_SUPPORT == _ON)
            pData[1] = (GET_DP_RX_HDCP2_MODE() == _TRUE) ? GET_DP_RX_HDCP2_AUTH_PROC_DONE() : GET_DP_RX_HDCP_AUTH_PROC_DONE();
            pData[2] = (GET_DP_TX_HDCP2_MODE() == _TRUE) ? GET_DP_TX_HDCP2_AUTH_PROC_PASS() : GET_DP_TX_HDCP_AUTH_PROC_PASS();
#else
            pData[1] = GET_DP_RX_HDCP_AUTH_PROC_DONE();
            pData[2] = GET_DP_TX_HDCP_AUTH_PROC_PASS();
#endif
            pData[0] = (GET_DP_DOWN_REPLY_QUERY_STREAM_ENCRYPTION_STATUS_MSG_STREAM_AUTHEN() & pData[1] & pData[2]);
            ...
            pData[0] = GET_DP_DOWN_REPLY_QUERY_STREAM_ENCRYPTION_STATUS_MSG_STREAM_OUTPUT_CP_TYPE();
...
bit ScalerDpTxMstWriteMsgToDownstreamStatusCheck(EnumDpMessageType enumMsgType)
{
    ...
    for(ucRetry = 0; ucRetry < 20; ucRetry++)
    {
        ...
        if(ScalerTimerPollingFlagProc(5, PBD_B6_MSG_WR_FINISH, _BIT0, _TRUE) == _TRUE)
        {
            // CLR SBMSG Finish Flag
...
bit ScalerDpTxMstDownRequestMsgAnal(void)
{
    ...
    // Any Downrequest Message From Up Stream Source
    if(GET_DP_MSG_REQUEST_ID() != _DP_MSG_NONE)
    {
        // Cancel TimerEvent as Receiving Topology Rebuilding MSG
        ScalerTimerCancelTimerEvent(_SCALER_TIMER_EVENT_DP_TX_FORCE_REBUILD_TOPOLOGY_EVENT);
    }

#if(_DP_MST_TX_INTERCEPT_DDCCI_MSG_SUPPORT == _ON)
    // Ddcci Read Need Intercept
    if(GET_DP_TX_INTERCEPT_DDCCI_MSG_STATUS() == _TRUE)
    {
        if((GET_DP_MSG_REQUEST_ID() == _DP_MSG_REMOTE_I2C_WRITE) || (GET_DP_MSG_REQUEST_ID() == _DP_MSG_REMOTE_I2C_READ))
        {
            if(g_unDpRequestPool.pucData[2] == _DP_MSG_IIC_DEVICE_DDCCI)
            {
                ucLCR = 0;
            }
        }
    ...
    switch(GET_DP_MSG_REQUEST_ID())
    {
            ...
            if(GET_DP_TX_DOWNSTREAM_DPPLUG_STATUS() == _DP_PLUG)
            {
                ...
                else
                {
                    ...
                    else if(GET_DP_TX_LT_RESULT() == _DP_TX_TRAINING_FAIL)
                    {
                        // Report Link Failure
                        SET_DP_TX_ILLEGAL_DOWN_REQUEST_MSG();
                        SET_DP_REPLY_ILLEGAL_MSG_NACK_REASON(_LINK_FAILURE);
                    }
                    else
                    {
                        return _FALSE;
        ...
        case _DP_MSG_ALLOCATE_PAYLOAD:
            ...
            {
                if(GET_DP_DOWN_REQUEST_ALLOCATE_PAYLOAD_MSG_PBN() > ScalerDpTxMstGetAvailablePBN())
                {
            ...
            if(GET_DP_TX_DOWNSTREAM_DPPLUG_STATUS() == _DP_PLUG)
            {
                ...
                else
                {
                    if((GET_DP_REPLY_POOL_FREE() == _TRUE) && (GET_DP_RX_REPLY_BUF_FREE() == _TRUE))
                    {
                        ...
                        if(ScalerDpTxMstGetStreamAllocate() == _FALSE)
                        {
        ...
        case _DP_MSG_QUERY_STREAM_ENCRYPTION_STATUS:
            ...
            {
#if(_DP_HDCP_2_2_SUPPORT == _ON)
                if(GET_DP_TX_HDCP2_VERSION_SUPPORTED() == _TRUE)
                {
                    if(GET_DP_TX_HDCP2_AUTH_PROCCESSING() == _FALSE)
                    {
...
#endif
                {
                    ...
                    {
                        if(GET_DP_TX_HDCP_AUTH_PROCCESSING() == _FALSE)
                        {
...
bit ScalerDpTxMstSyncDownStreamTableSetting(BYTE ucPayloadID, BYTE ucStartPosition, BYTE ucLength)
{
    ...
    for(ucReadIndex = 0; ucReadIndex < 20; ucReadIndex++)
    {
        ...
        if(ScalerDpTxAuxNativeRead(0x00, 0x02, 0xC0, 1, pData) == _TRUE)
        {
            if((pData[0] & _BIT0) == _BIT0)
            {
                ...
                for(ucPollingIndex = 0; ucPollingIndex < 20; ucPollingIndex++)
                {
...
void ScalerDpTxMstDownReplyDirectlyMsg(void)
{
    ...
    switch(GET_DP_MSG_REQUEST_ID())
    {
        case _DP_MSG_ENUM_PATH_RESOURCES:
            ...
            else
            {
                ...
                if(ScalerDpRxMstGetSourceMuxBackup(_DP_ST_SOURCE2) == _DP_ST_NONE)
                {
        ...
        case _DP_MSG_QUERY_PAYLOAD:
            ...
            {
                if(g_pucDpStreamToIdMapping[ScalerDpRxMstGetSourceMuxBackup(pucData[0])] == GET_DP_DOWN_REQUEST_QUERY_PAYLOAD_MSG_PAYLOAD_ID())
                {
        ...
        case _DP_MSG_QUERY_STREAM_ENCRYPTION_STATUS:
            ...
            if(GET_DP_TX_HDCP2_CAPABLE_SUPPORTED() == _TRUE)
            {
                ...
                SET_DP_DOWN_REPLY_QUERY_STREAM_ENCRYPTION_STATUS_MSG_STREAM_REPEATER_PRESENT(GET_DP_TX_HDCP2_REPEATER_SUPPORTED());

                if(GET_DP_TX_DOWNSTREAM_DPPLUG_STATUS() == _DP_PLUG)
                {
                    pData[0] = (GET_DP_TX_HDCP2_CAPABLE_SUPPORTED() == _TRUE) ? _DP_OUTPUT_SINK_TYPE_MSG_NO_SUPPORT : _DP_OUTPUT_SINK_TYPE_NON_AUTHENTICABLE;
                }
                else
                {
                    pData[0] = 0x00;
            ...
            if(GET_DP_TX_HDCP2_MODE() == _ENABLE)
            {
                SET_DP_DOWN_REPLY_QUERY_STREAM_ENCRYPTION_STATUS_MSG_STREAM_AUTHEN(GET_DP_TX_HDCP2_AUTH_PROC_PASS());
            }
            else
#endif
            {
...
#if(_DP_HDCP_2_2_SUPPORT == _ON)
            if((GET_DP_TX_DOWNSTREAM_DPPLUG_STATUS() == _DP_PLUG) &&
               ((GET_DP_TX_HDCP2_CAPABLE_SUPPORTED() == _TRUE) || (GET_DP_TX_HDCP_DOWNSTREAM_SUPPORTED() == _TRUE)))
#else
    ...
    else
    {
        ...
        if(GET_DP_TX_INTERCEPT_DDCCI_MSG_ADJUST_LCR() == _TRUE)
        {
            CLR_DP_TX_INTERCEPT_DDCCI_MSG_ADJUST_LCR();
...
BYTE ScalerDpTxMstGetDownReplyLct(void)
{
    if(GET_DP_TX_MSG_DOWN_REPLY_BUF0_MSG_FINISH() == _TRUE)
    {
        return ((BYTE)g_stTxMsgBackupHeaderInfo0.b4MsgLct);
    }
    else
    {
        return ((BYTE)g_stTxMsgBackupHeaderInfo1.b4MsgLct);
    }
}
    ...
    switch(GET_DP_MSG_REPLY_ID())
    {
        ...
        case _DP_MSG_ALLOCATE_PAYLOAD:
            ...
            else
            {
                ...
                pucData[0] = ScalerDpRxMstSearchMatchStream(GET_DP_DOWN_REPLY_ALLOCATE_PAYLOAD_MSG_PAYLOAD_ID());

                for(pucData[1] = _DP_ST_SOURCE3; pucData[1] <= _DP_ST_SOURCE5; pucData[1]++)
                {
                    if(ScalerDpRxMstGetSourceMuxBackup(pucData[1]) == pucData[0])
                    {
        ...
        case _DP_MSG_POWER_DOWN_PHY:
            ...
            if((ScalerDpTxMstGetDownReplyPathBit() == _TRUE) &&
               (GET_DP_MSG_REPLY_TYPE() == _DP_MSG_ACK) &&
               (ScalerDpTxMstGetStreamAllocate() == _FALSE))
            {
...
bit ScalerDpTxMstSyncTableProcess(BYTE ucPayloadID, WORD usPBN)
{
    ...
    BYTE ucStreamNum = ScalerDpRxMstSearchMatchStream(ucPayloadID);

    // Find which ST Source Is assigned to the STx Containing current payload ID
    if(ucStreamNum == ScalerDpRxMstGetSourceMuxBackup(_DP_ST_SOURCE3))
    {
        enumTxCurrentSTX = _DP_ST_SOURCE3;
    }
    else if(ucStreamNum == ScalerDpRxMstGetSourceMuxBackup(_DP_ST_SOURCE4))
    {
        enumTxCurrentSTX = _DP_ST_SOURCE4;
    }
    else if(ucStreamNum == ScalerDpRxMstGetSourceMuxBackup(_DP_ST_SOURCE5))
    {
...
void ScalerDpTxMonitorDownstreamBW(void)
{
    ...
    if((GET_DP_TX_LINK_PBN_PER_TIMESOLT() != ucCurrentPBNPerTimerslot))
    {
        ...
        else
        {
            ...
            pData[1] = GET_DP_RX_MST_STREAM_TIME_SLOT(pData[0]);
...
void ScalerDpTxLocalRecoveryPayloadTable(BYTE ucCurrentPBNPerTimerslot)
{
    ...
    for(pData[0] = 0; pData[0] < 3; pData[0]++)
    {
        if(ScalerGetBit(GET_DP_TX_STX_START_ADDRESS(ucStreamOrder[pData[0]] + 2), 0x3F) != 0)
        {
            ...
            // Get STx
            pData[1] = ScalerDpRxMstGetSourceMuxBackup(ucStreamOrder[pData[0]] + 2);
...
bit ScalerDpTxMstI2CRead(void)
{
...
#if(_DP_TX_EDID_MODIFY_REMOVE_HDR_FS2_SUPPORT == _ON)
    BYTE ucStoreEdidReadLength = 0;
    BYTE ucI2CEdidReadLength = 0;
#endif
    ...
    if(GET_DP_TX_INTERCEPT_DDCCI_MSG_STATUS() == _TRUE)
    {
        if(g_unDpRequestPool.pucData[ucArrayIndex] == _DP_MSG_IIC_DEVICE_DDCCI)
        {
            // Reply Null Msg
            g_unDpReplyPool.stRemoteI2CRead.pucI2CRead[0] = 0x6E;
            g_unDpReplyPool.stRemoteI2CRead.pucI2CRead[1] = 0x80;
            g_unDpReplyPool.stRemoteI2CRead.pucI2CRead[2] = (0x50 ^ 0x6E ^ 0x80);

            // Set Number Of Byte Read
            SET_DP_DOWN_REPLY_REMOTE_I2C_READ_MSG_NUM_OF_READ(3);

            // Set Message Body Length
            SET_REG_DP_RX_UPSTREAM_REPLY_LENGTH(6);

            SET_DP_TX_INTERCEPT_DDCCI_MSG_ADJUST_LCR();
    ...
    if(GET_DP_DOWN_REQUEST_REMOTE_I2C_READ_MSG_NUM_OF_WRITE() > 0)
    {
        ...
        for(ucCnt = 0; ucCnt < GET_DP_DOWN_REQUEST_REMOTE_I2C_READ_MSG_NUM_OF_WRITE(); ucCnt++)
        {
            ...
            if(ScalerDpTxMstI2CWrite(g_unDpRequestPool.pucData[ucArrayIndex], g_unDpRequestPool.pucData[pucData[0]],
                                     &g_unDpRequestPool.pucData[pucData[1]], (bit)(g_unDpRequestPool.pucData[pucData[2]] & _BIT4)) == _TRUE)
#else
            if(ScalerDpTxAuxI2CoverWrite(g_unDpRequestPool.pucData[ucArrayIndex], 1, &g_unDpRequestPool.pucData[pucData[1] + g_unDpRequestPool.pucData[pucData[0]] - 1],
...
#if(_DP_TX_EDID_MODIFY_REMOVE_HDR_FS2_SUPPORT == _ON)
    if(g_unDpRequestPool.pucData[ucArrayIndex] == _DP_MSG_IIC_DEVICE_EDID)
    {
        ucLength = g_unDpRequestPool.pucData[ucArrayIndex + 1];

        // Calculate the length of reply by Stored Edid & the length of reply by current I2C Read edid
        if((GET_DP_TX_EDID_READ_POSITION() + ucLength) > _DP_TX_EDID_SIZE_MAX)
        {
            if(GET_DP_TX_EDID_READ_POSITION() < _DP_TX_EDID_SIZE_MAX)
            {
                // Case partial reply by Stored Edid, partial reply by current I2C Read edid
                ucStoreEdidReadLength = _DP_TX_EDID_SIZE_MAX - GET_DP_TX_EDID_READ_POSITION();
                ucI2CEdidReadLength = ucLength - ucStoreEdidReadLength;
            }
            else
            {
                // Case all reply by current I2C Read edid
                ucStoreEdidReadLength = 0;
                ucI2CEdidReadLength = ucLength;
            }
        }
        else
        {
            // Case all reply by Stored Edid
            ucStoreEdidReadLength = ucLength;
            ucI2CEdidReadLength = 0;
        }

        if(ucStoreEdidReadLength != 0)
        {
            if(ScalerDpTxGetDfpDeviceStoredEdid(GET_DP_TX_EDID_READ_POSITION(), ucStoreEdidReadLength, g_unDpReplyPool.stRemoteI2CRead.pucI2CRead) == _TRUE)
            {
                INC_DP_TX_EDID_READ_POSITION(g_unDpRequestPool.pucData[ucArrayIndex + 1]);

                ucReadIndex += ucStoreEdidReadLength;
            }
        }

        if(ucI2CEdidReadLength != 0)
        {
            if(GET_DP_TX_EDID_READ_SEGMENT() > 0)
            {
                if(ScalerDpTxAuxI2CoverWrite(0x30, 1, &GET_DP_TX_EDID_READ_SEGMENT(), 1) == _FALSE)
                {
                    return _FALSE;
                }
            }

            if(ScalerDpTxAuxI2CoverWrite(0x50, 1, &GET_DP_TX_EDID_READ_OFFSET(), 1) == _FALSE)
            {
                return _FALSE;
            }

            ucLength = _DP_I2C_MAX_TRANSACTION_LENGTH;

            do
            {
                if((ucI2CEdidReadLength - ucReadIndex) <= _DP_I2C_MAX_TRANSACTION_LENGTH)
                {
                    ucLength = ucI2CEdidReadLength - ucReadIndex;
            ...
            while(ucI2CEdidReadLength > ucReadIndex);
        }
    }
    else
#endif
    ...
    // Set Number Of Byte Read
    SET_DP_DOWN_REPLY_REMOTE_I2C_READ_MSG_NUM_OF_READ(ucReadIndex);

    // Set Message Body Length
    SET_REG_DP_RX_UPSTREAM_REPLY_LENGTH(3 + ucReadIndex);
...
bit ScalerDpTxMstI2CWrite(BYTE ucI2CDeviceID, BYTE ucWriteNum, BYTE *pucData, bit bNoStop)
{
    ...
    switch(ucI2CDeviceID)
    {
        ...
        case _DP_MSG_IIC_DEVICE_EDID_SEGMENT:
            ...
            if(ucWriteNum > 0)
            {
                SET_DP_TX_EDID_READ_SEGMENT(pucData[ucWriteNum - 1]);
            }
        ...
        case _DP_MSG_IIC_DEVICE_EDID:
            ...
            if(ucWriteNum > 0)
            {
                SET_DP_TX_EDID_READ_OFFSET(pucData[ucWriteNum - 1]);
            }
        ...
        default:
            ...
            do
            {
                if((ucWriteNum - ucWriteIndex) <= _DP_I2C_MAX_TRANSACTION_LENGTH)
                {
                ...
                if(ScalerDpTxAuxI2CoverWrite(ucI2CDeviceID, ucLength, &pucData[ucWriteIndex], bMot) == _TRUE)
                {
...
void ScalerDpTxMstQueryEncryptionWriteShaInput(void)
{
    ...
    SET_REG_DP_TX_SHA_INPUT_3(GET_DP_TX_HDCP_M0_0());

    // Enable write 32-bit data to SHA-1 block
    SET_REG_DP_TX_SHA_INPUT_DB();

    // 12. Write Data into SHA-1 Transform Input
    SET_REG_DP_TX_SHA_INPUT_0(GET_DP_TX_HDCP_M0_1());
    SET_REG_DP_TX_SHA_INPUT_1(GET_DP_TX_HDCP_M0_2());
    SET_REG_DP_TX_SHA_INPUT_2(GET_DP_TX_HDCP_M0_3());
    SET_REG_DP_TX_SHA_INPUT_3(GET_DP_TX_HDCP_M0_4());

    // Enable write 32-bit data to SHA-1 block
    SET_REG_DP_TX_SHA_INPUT_DB();

    // 13. Write Data into SHA-1 Transform Input
    SET_REG_DP_TX_SHA_INPUT_0(GET_DP_TX_HDCP_M0_5());
    SET_REG_DP_TX_SHA_INPUT_1(GET_DP_TX_HDCP_M0_6());
    SET_REG_DP_TX_SHA_INPUT_2(GET_DP_TX_HDCP_M0_7());
...
void ScalerDpTxMstQueryEncryptionWriteSha256Input(bit bRepeater)
{
    ...
    // Load Riv (64-bits)
    memcpy(&pucHmacSha256Input[10], g_pucDpTxHdcp2Riv, sizeof(g_pucDpTxHdcp2Riv));

    // Load Receiver ID of DSP (40-bits)
    for(pData[0] = 0; pData[0] < 5; pData[0]++)
    {
        pucHmacSha256Input[pData[0] + 18] = g_pucDpTxHdcp2ReceiverId[pData[0]];
    }
    ...
    if(bRepeater == _TRUE)
    {
        ...
        for(pData[0] = 0; pData[0] < 20; pData[0]++)
        {
            pucHmacSha256Input[pData[0] + 23] = g_pucDpTxHdcp2DownStreamV[pData[0]];
        }
    }
    else
    {
    ...
    ScalerHdcp2RepeaterLPrimeCalculate(g_pucDpTxHdcp2dKey0, g_pucDpTxHdcp2dKey1, pucHmacSha256Input);
}
...
bit ScalerDpTxMstQueryEncryptionCompareSha256Result(void)
{
    ...
    {
        // MSB first
        if(GET_REG_DP_RX_HDCP_SHA256_RESULT(ucCnt) != GET_DP_DOWN_REPLY_QUERY_STREAM_ENCRYPTION_STATUS_MSG_STREAM_SIGNATURE_L(ucCnt))
        {
            return _FALSE;
        }
    }
...
// Output Value : None
//--------------------------------------------------
void ScalerDpTxMstReset_EXINT0(void) using 1
{
// HW Reset-----------------------------------
    ...
    // Reset Tx MSG FIFO
    SET_REG_DP_TX_DOWNSTREAM_MSG_WRITE_PTR_EXINT(0x00);

    // Reset Tx Write MSG FSM
    SET_REG_DP_TX_DOWNSTREAM_MSG_WRITE_DISABLE_EXINT();

    // Reset Tx Read MSG FSM
    SET_REG_DP_TX_UPSTREAM_MSG_READ_DISABLE_EXINT();

    // CLR MSG Finish Flag
    CLR_REG_DP_TX_UPSTREAM_MSG_READ_FINISH_FLAG_EXINT();

    // CLR SBMSG Finish Flag
    CLR_REG_DP_TX_UPSTREAM_SBMSG_FINISH_FLAG_EXINT();

    // Buf_CLR=1 WR_PTR = 0x00
    CLR_REG_DP_TX_UPSTREAM_MSG_BUF_EXINT();
...
// FW Reset-----------------------------------
    ...
    // Cancel Timers
    ScalerTimerCancelTimerEvent_EXINT0(_SCALER_TIMER_EVENT_DP_RX_MSG_COUNT_RESET_EVENT);
    ScalerTimerCancelTimerEvent_EXINT0(_SCALER_TIMER_EVENT_DP_TX_MSG_COUNT_RESET_EVENT);
    ScalerTimerCancelTimerEvent_EXINT0(_SCALER_TIMER_EVENT_DP_TX_CABLE_STATUS_CHANGE_EVENT);
...
#if(_DP_MST_TX_INTERCEPT_DDCCI_MSG_SUPPORT == _ON)
//--------------------------------------------------
// Description  : DP Tx Intercept Ddcci MSG
// Input Value  : bStatus
// Output Value : None
//--------------------------------------------------
void ScalerDpTxMstInterceptDdcciMsgSetting(bit bStatus)
{
    SET_DP_TX_INTERCEPT_DDCCI_MSG_STATUS(bStatus);
}
#endif
