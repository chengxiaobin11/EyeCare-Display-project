...
#define _SCALER_SET_SMBUS_BLOCK_PROTOCOL_CMD      0xC8
...
bit ScalerDebugCheckDebugPort(BYTE ucInputPort);
#endif // End of #if(_DEBUG_MESSAGE_SUPPORT == _ON)
...
// Input Value  : None
// Output Value : enumMode --> _DEBUG_MODE / _DDCCI_MODE
//--------------------------------------------------
EnumDDCCIDebugMode ScalerDebugGetDDCCIDebugMode(void)
{
...
bit ScalerDebugCheckDebugPort(BYTE ucInputPort)
{
    switch(ucInputPort)
    {
#if(((_D0_TMDS_SUPPORT == _ON) || (_D0_INPUT_PORT_TYPE == _D0_DP_PORT)) && (_DEBUG_DDC_CHANNEL_SEL == _DDC0))
        case _D0_INPUT_PORT:

            return _TRUE;
#endif

#if(((_D1_TMDS_SUPPORT == _ON) || (_D1_INPUT_PORT_TYPE == _D1_DP_PORT)) && (_DEBUG_DDC_CHANNEL_SEL == _DDC1))
        case _D1_INPUT_PORT:

            return _TRUE;
#endif

#if(((_D2_TMDS_SUPPORT == _ON) || (_D2_INPUT_PORT_TYPE == _D2_DP_PORT)) && (_DEBUG_DDC_CHANNEL_SEL == _DDC2))
        case _D2_INPUT_PORT:

            return _TRUE;
#endif

#if((_D3_TMDS_SUPPORT == _ON) && (_DEBUG_DDC_CHANNEL_SEL == _DDC3))
        case _D3_INPUT_PORT:

            return _TRUE;
#endif

#if((_D4_TMDS_SUPPORT == _ON) && (_DEBUG_DDC_CHANNEL_SEL == _DDC4))
        case _D4_INPUT_PORT:

            return _TRUE;
#endif

#if((_D5_TMDS_SUPPORT == _ON) && (_DEBUG_DDC_CHANNEL_SEL == _DDC5))
        case _D5_INPUT_PORT:
    ...
    if(g_bRunCommand == _FALSE)
    {
        ...
        if((ucTempBuf & 0x04) != 0x00)
        {
            while((MCU_FF29_IIC_STATUS2 & _BIT1) == 0)
            {
                ...
                // Ingore Byte Count.
                if((g_ucDdcciCommandNumber == 0) && (g_stSmbusBlockRWCmd.b1SmbusEnable == _TRUE) && (g_stSmbusBlockRWCmd.b1FilterFlag == _FALSE))
                {
            ...
            if(g_bRunCommand == _TRUE)
            {
                ...
                switch(g_pucDdcciData[0])
                {
                    ...
                    case _SCALER_DUAL_BANK_BOOT_PROC_CMD:
                        ScalerDebugDualBankHandler(g_pucDdcciData[1]);
                        break;
                    ...
                    case _SCALER_FW_UPDATE_CMD:
                        if(((_FW_UPDATE_BACKGROUND_SUPPORT == _ON) && (g_pucDdcciData[1] == _FW_UPDATE_BACKGROUND_SUB_OP_CODE)) ||
                           ((_FW_UPDATE_FOREGROUND_SUPPORT == _ON) && (g_pucDdcciData[1] == _FW_UPDATE_FOREGROUND_SUB_OP_CODE)))
                        {
...
EnumISPBundleVerResult ScalerDebugUpdateBundleVersion(void)
{
    ...
    // Recovery sign flag
    if(ScalerFlashWrite(_DUAL_BANK_BUNDLE_VERSION_LOCATE_BANK, ((WORD)(_DUAL_BANK_BUNDLE_VERSION_LOCATE_SECTOR << 12)), _DUAL_BANK_BUNDLE_VERSION_COUNT, &g_pucDdcciData[3]) == _FALSE)
    {
        return _ISP_FAIL;
    }
