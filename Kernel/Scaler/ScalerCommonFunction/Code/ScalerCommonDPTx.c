...
BYTE g_ucDpTxConfigSetting = _DP_TX_CONFIG_NONE;
#endif
...
BYTE g_pucDpTxPreviousDfpDeviceEdidBackup[_DP_TX_EDID_SIZE_MAX]; // Record Down Stream Edid Info
#endif

WORD g_usDpTxEdidBw;
EnumDPTxEdidVendorModel g_enumDpTxEdidVendorModel;
...
void ScalerDpTxAuxTimeoutInitial(void);
bit ScalerDpTxLinkConfig(void);

#if(_DP_TX_EDID_MODIFY_REMOVE_HDR_FS2_SUPPORT == _ON)
bit ScalerDpTxGetDfpDeviceReadEdid(void);
void ScalerDpTxEdidModify(void);
void ScalerDpTxEdidRemoveCtaDataBlockProc(WORD *pusCtaDataBlockAddr, EnumEdidCtaDataBlock enumCtaDataBlock);
void ScalerDpTxEdidRemoveDidCtaDataBlockProc(WORD *pusDidDataBlockAddr, EnumEdidDidDataBlock enumDidDataBlock);
bit ScalerDpTxGetDfpDeviceStoredEdid(WORD usOffset, BYTE ucLength, BYTE *pucDataArray);
void ScalerDpTxEdidShiftCtaDataBlockAddrProc(EnumEdidDataBlockShiftDirect enumShiftDir, BYTE ucLengthExt, WORD *pusCtaDataBlockAddr, EnumEdidCtaDataBlock enumCtaBaseDataBlock);
void ScalerDpTxEdidShiftDidDataBlockAddrProc(EnumEdidDataBlockShiftDirect enumShiftDir, BYTE ucLengthExt, WORD *pusDidDataBlockAddr, EnumEdidDidDataBlock enumDidBaseDataBlock);
bit ScalerDpTxEdidCheckSumCal(BYTE *pucDdcRamAddr, WORD usStartAddr);
bit ScalerDpTxEdidDidCheckSumCal(BYTE *pucDdcRamAddr, WORD usStartAddr, WORD usEndAddr);
#endif

#if(_DP_TX_CONFIG_BY_USER_SUPPORT == _ON)
void ScalerDpTxLinkConfigAdjust(EnumDpTxConfig enumDpTxConfig);
#endif
...
void ScalerDpTxAudioChannelStatusSetting(void);
...
void ScalerDpTxAudioSleepAudioControl(void);
void ScalerDpTxAudioSetSleepAudioState(bit bAudioStandAloneStatus);
#endif
...
void ScalerDpTxInterlaceInfoSetting(BYTE ucInputPort, BYTE ucPortType);
#if(_DP_TX_BLOCK_CONTROL_BEFORE_STREAM_OUTPUT == _ON)
void ScalerDpTxCloneModeSwitch(bit bClone);
bit ScalerDpTxBlockStatusCheck(EnumSourceSearchPort enumInputPort);
...
#if(_DP_HDCP14_CTS_SUPPORT == _ON)
void ScalerDpTxHDCPResetKsvBackup(void);
bit ScalerDpTxHDCPDownstreamBksvChange(void);
#endif
#if(_DP_MST_SUPPORT == _ON)
bit ScalerDpTxGenConnectStatusNotifyMsgCheck(void);
bit ScalerDpTxSinkCountUpdate(void);
bit ScalerDpTxGetUpReqEnableStatusChange(void);
void ScalerDpTxUpdateDfpDeviceMsgCapFieldStatus(void);
#endif

void ScalerDpTxDownStreamEdidCheckSpecialVendor(void);
void ScalerDpTxDownStreamEdidBwSetting(void);
WORD ScalerDpTxGetDownStreamEdidBw(void);
EnumDPTxInputSource ScalerDpTxGetStreamSource(void);
...
void ScalerDpTxModeHandler(void)
{
    ...
    if(ScalerDpTxGetUpReqEnableStatusChange() == _TRUE)
    {
        ScalerDpTxUpdateDfpDeviceMsgCapFieldStatus();
    }
#endif
    ...
    switch(GET_DP_TX_MODE_STATE())
    {
        case _DP_TX_MODE_STATUS_UNPLUG:
            ...
            if(GET_DP_TX_MODE_STATE_CHANGE() == _TRUE)
            {
                ...
                SET_DP_TX_DOWNSTREAM_SINK_COUNT(0x00);

                SET_DP_TX_DOWNSTREAM_SINK_COUNT_CHANGE();
...
#if(_DP_TX_EDID_MODIFY_REMOVE_HDR_FS2_SUPPORT == _ON)
                memset(g_pucDpTxPreviousDfpDeviceEdidBackup, 0, _DP_TX_EDID_SIZE_MAX);
#endif
        ...
        case _DP_TX_MODE_STATUS_LINK_ON:
            ...
            if(GET_DP_TX_MODE_STATE_CHANGE() == _TRUE)
            {
...
#endif
                {
                    ...
                    {
                        if(ScalerDpTxHDCPDownstreamBksvChange() == _FALSE)
                        {
...
void ScalerDpTxInitial(void)
{
    ...
    // Set Aux HW Retry
    ScalerSetBit(PBD_AF_AUX_RETRY_1, ~(_BIT3 | _BIT2 | _BIT1 | _BIT0), _DP_AUX_TX_RETRY_LIMIT);
    ...
    ScalerSetBit(PBB_73_HPD_TIMER1, ~(_BIT7 | _BIT6 | _BIT5 | _BIT4), _BIT4);
    ScalerSetByte(PBB_74_HPD_TIMER2, 0xA5);
...
//--------------------------------------------------
// Description  : Dp Tx Aux Timeout Initial
// Input Value  : None
// Output Value : None
//--------------------------------------------------
void ScalerDpTxAuxTimeoutInitial(void)
{
    ...
    SET_DP_TX_DOWNSTREAM_LTTPR_NUM(8);
}
...
bit ScalerDpTxLinkConfig(void)
{
    ...
    {
        // Set Aux TO 400us
        SET_DP_TX_DOWNSTREAM_LTTPR_NUM(0);
    ...
    ScalerDpTxLinkConfigAdjust(GET_DP_TX_CONFIG_SETTING());
#endif
...
#if(_DP_TX_CONFIG_BY_USER_SUPPORT == _ON)
    // Set DP Tx Link Config by User
    if(GET_DP_TX_CONFIG_SETTING() == _DP_TX_CONFIG_NONE)
#endif
    {
        // Step2:Get link Rate
    ...
    if(ScalerDpTxAuxNativeRead(0x00, 0x00, 0x80, 16, pucData) == _FALSE)
    {
        return _FALSE;
    }
    ...
    ScalerDpTxDownStreamEdidCheckSpecialVendor();

    ScalerDpTxDownStreamEdidBwSetting();

#if(_DP_TX_EDID_MODIFY_REMOVE_HDR_FS2_SUPPORT == _ON)
    // Check DownStream EDID Change Process
    if(ScalerDpTxGetDfpDeviceReadEdid() == _FALSE)
    {
        DebugMessageMst("7. Check EDID Fail", 0);
    }

    ScalerDpTxEdidModify();
#endif
...
bit ScalerDpTxGetDfpDeviceReadEdid(void)
{
    ...
    BYTE ucBlock = 0;
    BYTE ucIndex = 0;
    BYTE pucEDIDTemp[_DP_TX_EDID_SIZE_MAX];

    memset(pucEDIDTemp, 0, _DP_TX_EDID_SIZE_MAX);

    while((ucBlock <= pucEDIDTemp[0x7E]) && (ucBlock < (_DP_TX_EDID_SIZE_MAX / 128)))
    {
        ucEdidSegment = ucBlock / 2;

        if(ScalerDpTxAuxI2CoverWrite(0x30, 1, &ucEdidSegment, 1) == _FALSE)
        {
            if(ucEdidSegment > 0)
            {
                break;
        ...
        if(ScalerDpTxAuxI2CoverWrite(0x50, 1, &ucEdidOffset, 1) == _FALSE)
        {
            return _FALSE;
        }
        ...
        for(ucIndex = 0; ucIndex < 8; ucIndex ++)
        {
            if(ucIndex < 7)
            {
                if(ScalerDpTxAuxI2CoverRead(0x50, 16, &pucEDIDTemp[TO_WORD(ucEdidSegment, ucEdidOffset)], 1) == _FALSE)
                {
                    return _FALSE;
                }
            }
            ...
            {
                if(ScalerDpTxAuxI2CoverRead(0x50, 16, &pucEDIDTemp[TO_WORD(ucEdidSegment, ucEdidOffset)], 0) == _FALSE)
                {
                    return _FALSE;
                }
            }
    ...
    memcpy(g_pucDpTxPreviousDfpDeviceEdidBackup, pucEDIDTemp, _DP_TX_EDID_SIZE_MAX);
...
void ScalerDpTxEdidModify(void)
{
    ...
    // Collect Start Addr of CTA Ext & DID Ext
    ScalerSyncGetCtaExtDbAddress(pusCtaDataBlockAddr, g_pucDpTxPreviousDfpDeviceEdidBackup);
    ScalerSyncGetDidExtDbAddress(pusDidDataBlockAddr, g_pucDpTxPreviousDfpDeviceEdidBackup);

    // Remove Cta & Did Ext Block's FS & HDR DB
    ScalerDpTxEdidRemoveCtaDataBlockProc(pusCtaDataBlockAddr, _CTA_FS_DB);
    ScalerDpTxEdidRemoveCtaDataBlockProc(pusCtaDataBlockAddr, _CTA_SHDR_DB);

    ScalerDpTxEdidRemoveDidCtaDataBlockProc(pusDidDataBlockAddr, _DID_CTA_FS_DB);
    ScalerDpTxEdidRemoveDidCtaDataBlockProc(pusDidDataBlockAddr, _DID_CTA_SHDR_DB);
}

//--------------------------------------------------
// Description  : Remove data block process
// Input Value  : pucDdcRamAddr; pusCtaDataBlockAddr
// Output Value :
//--------------------------------------------------
void ScalerDpTxEdidRemoveCtaDataBlockProc(WORD *pusCtaDataBlockAddr, EnumEdidCtaDataBlock enumCtaDataBlock)
{
    ...
    if(pusCtaDataBlockAddr[enumCtaDataBlock] != 0x00)
    {
        ...
        BYTE ucRemoveDbSize = (g_pucDpTxPreviousDfpDeviceEdidBackup[usDbStartAddr] & _CTA_DB_LEN_MASK) + 1;
        BYTE ucShiftDataStartAddr = usDbStartAddr + ucRemoveDbSize;

        if(enumCtaDataBlock == _CTA_FS_DB)
        {
            g_pucDpTxPreviousDfpDeviceEdidBackup[usDbStartAddr + 5] = g_pucDpTxPreviousDfpDeviceEdidBackup[usDbStartAddr + 5] & 0xFD;
        }
        else
        {
            // Shift Edid Content
            memmove(&g_pucDpTxPreviousDfpDeviceEdidBackup[usDbStartAddr], &g_pucDpTxPreviousDfpDeviceEdidBackup[ucShiftDataStartAddr], (0xFF - ucShiftDataStartAddr));

            // Zero padding
            memset(&g_pucDpTxPreviousDfpDeviceEdidBackup[0xFF - ucRemoveDbSize], 0x00, ucRemoveDbSize);

            // Repalce DTD Start Addr
            g_pucDpTxPreviousDfpDeviceEdidBackup[0x82] = g_pucDpTxPreviousDfpDeviceEdidBackup[0x82] - ucRemoveDbSize;

            // Shift the later Data Block
            ScalerDpTxEdidShiftCtaDataBlockAddrProc(_DB_SHIFT_REMOVE, ucRemoveDbSize, pusCtaDataBlockAddr, enumCtaDataBlock);
        ...
        ScalerDpTxEdidCheckSumCal(g_pucDpTxPreviousDfpDeviceEdidBackup, 0x80);
    }
}
...
void ScalerDpTxEdidRemoveDidCtaDataBlockProc(WORD *pusDidDataBlockAddr, EnumEdidDidDataBlock enumDidDataBlock)
{
    ...
    // Check DataBlock Exist
    if(pusDidDataBlockAddr[enumDidDataBlock] != 0x00)
    {
        // if HF-EEODB, Db addr must in 0x84
        WORD usDbStartAddr = pusDidDataBlockAddr[enumDidDataBlock];
        BYTE ucRemoveDbSize = (g_pucDpTxPreviousDfpDeviceEdidBackup[usDbStartAddr] & _CTA_DB_LEN_MASK) + 1;
        WORD usShiftDataStartAddr = usDbStartAddr + ucRemoveDbSize;
        BYTE ucByteInSection = 0;
        WORD usDidStartAddr = 0;
        WORD usDidChecksumAddr = 0;

        ucExtBlockNum = usDbStartAddr / 0x80;
        usDidStartAddr = ucExtBlockNum * 0x80 + 0x01;
        ucByteInSection = g_pucDpTxPreviousDfpDeviceEdidBackup[usDidStartAddr + 0x01];
        usDidChecksumAddr = usDidStartAddr + ucByteInSection + 4;

        if(enumDidDataBlock == _DID_CTA_FS_DB)
        {
            g_pucDpTxPreviousDfpDeviceEdidBackup[usDbStartAddr + 5] = g_pucDpTxPreviousDfpDeviceEdidBackup[usDbStartAddr + 5] & 0xFD;
        }
        else
        {
            g_pucDpTxPreviousDfpDeviceEdidBackup[usDidChecksumAddr] = 0x00;
            g_pucDpTxPreviousDfpDeviceEdidBackup[(ucExtBlockNum * 0x80) + 0x7F] = 0x00;

            // Shift Edid Content
            memmove(&g_pucDpTxPreviousDfpDeviceEdidBackup[usDbStartAddr], &g_pucDpTxPreviousDfpDeviceEdidBackup[usShiftDataStartAddr], ((ucExtBlockNum * 0x80) + 0x7F - usShiftDataStartAddr));

            // Zero padding
            memset(&g_pucDpTxPreviousDfpDeviceEdidBackup[(ucExtBlockNum * 0x80) + 0x7F - ucRemoveDbSize], 0x00, ucRemoveDbSize);

            // Repalce DTD Start Addr
            g_pucDpTxPreviousDfpDeviceEdidBackup[pusDidDataBlockAddr[_DID_CTA_DB] + 0x02] = g_pucDpTxPreviousDfpDeviceEdidBackup[pusDidDataBlockAddr[_DID_CTA_DB] + 0x02] - ucRemoveDbSize;

            // Shift the later Data Block
            ScalerDpTxEdidShiftDidDataBlockAddrProc(_DB_SHIFT_REMOVE, ucRemoveDbSize, pusDidDataBlockAddr, enumDidDataBlock);

            // Remove DataBlock from pusCtaDataBlockAddr
            pusDidDataBlockAddr[enumDidDataBlock] = 0x00;
        }

        // Checksum modify - Ext block
        ScalerDpTxEdidDidCheckSumCal(g_pucDpTxPreviousDfpDeviceEdidBackup, usDidStartAddr, usDidChecksumAddr);
        ScalerDpTxEdidCheckSumCal(g_pucDpTxPreviousDfpDeviceEdidBackup, (ucExtBlockNum * 0x80));
    }
}
...
bit ScalerDpTxGetDfpDeviceStoredEdid(WORD usOffset, BYTE ucLength, BYTE *pucDataArray)
{
    if((GET_DP_TX_DOWNSTREAM_DPPLUG_STATUS() == _DP_PLUG) && ((usOffset + ucLength) <= _DP_TX_EDID_SIZE_MAX))
    {
        memcpy(pucDataArray, &g_pucDpTxPreviousDfpDeviceEdidBackup[usOffset], ucLength);
...
void ScalerDpTxEdidShiftDidDataBlockAddrProc(EnumEdidDataBlockShiftDirect enumShiftDir, BYTE ucLengthExt, WORD *pusDidDataBlockAddr, EnumEdidDidDataBlock enumDidBaseDataBlock)
{
    ...
    for(ucIdx = 0; ucIdx < _DID_TOTAL_DB; ucIdx++)
    {
        if(pusDidDataBlockAddr[ucIdx] >= pusDidDataBlockAddr[enumDidBaseDataBlock])
        {
            if((EnumEdidDidDataBlock)ucIdx == enumDidBaseDataBlock)
            {
                // Find the block itself
                continue;
            }
            ...
            if(enumShiftDir == _DB_SHIFT_ADD)
            {
                pusDidDataBlockAddr[ucIdx] = pusDidDataBlockAddr[ucIdx] + ucLengthExt;
            }
            else if(enumShiftDir == _DB_SHIFT_REMOVE)
            {
                pusDidDataBlockAddr[ucIdx] = pusDidDataBlockAddr[ucIdx] - ucLengthExt;
...
bit ScalerDpTxEdidCheckSumCal(BYTE *pucDdcRamAddr, WORD usStartAddr)
{
    ...
    for(usEDIDIndex = usStartAddr; usEDIDIndex <= (0x7F + usStartAddr); usEDIDIndex++)
    {
        ...
        {
            pucDdcRamAddr[0x7F + usStartAddr] = 0x00 - ucEDIDCheckSum;
        }
    }
...
bit ScalerDpTxEdidDidCheckSumCal(BYTE *pucDdcRamAddr, WORD usStartAddr, WORD usEndAddr)
{
    ...
    for(; usStartAddr <= usEndAddr; usStartAddr++)
    {
        ucEDIDCheckSum += pucDdcRamAddr[usStartAddr];

        if (usStartAddr == (usEndAddr - 1))
        {
            pucDdcRamAddr[usEndAddr] = 0x00 - ucEDIDCheckSum;
        }
        else if (usStartAddr == usEndAddr)
        {
            ucEDIDCheckSum = ucEDIDCheckSum + pucDdcRamAddr[usEndAddr];
        }
    }
...
void ScalerDpTxLinkConfigAdjust(EnumDpTxConfig enumDpTxConfig)
{
    switch(enumDpTxConfig)
    {
        case _DP_TX_V11_1LANE_RBR:

            SET_DP_TX_DOWNSTREAM_DPCD_REV(_DP_VERSION_1_1);
            SET_DP_TX_DOWNSTREAM_MAX_LANE_NUM(_DP_ONE_LANE);
        ...
        case _DP_TX_V11_2LANE_RBR:
            ...
            SET_DP_TX_DOWNSTREAM_DPCD_REV(_DP_VERSION_1_1);
            SET_DP_TX_DOWNSTREAM_MAX_LANE_NUM(_DP_TWO_LANE);
            SET_DP_TX_DOWNSTREAM_MAX_LINK_RATE(_DP_LINK_RBR);
        ...
        case _DP_TX_V11_4LANE_RBR:
            ...
            SET_DP_TX_DOWNSTREAM_DPCD_REV(_DP_VERSION_1_1);
            SET_DP_TX_DOWNSTREAM_MAX_LANE_NUM(_DP_FOUR_LANE);
            SET_DP_TX_DOWNSTREAM_MAX_LINK_RATE(_DP_LINK_RBR);
        ...
        case _DP_TX_V11_1LANE_HBR:
            ...
            SET_DP_TX_DOWNSTREAM_DPCD_REV(_DP_VERSION_1_1);
            SET_DP_TX_DOWNSTREAM_MAX_LANE_NUM(_DP_ONE_LANE);
            SET_DP_TX_DOWNSTREAM_MAX_LINK_RATE(_DP_LINK_HBR);
        ...
        case _DP_TX_V12_1LANE_HBR2:
            ...
            SET_DP_TX_DOWNSTREAM_DPCD_REV(_DP_VERSION_1_2);
            SET_DP_TX_DOWNSTREAM_MAX_LANE_NUM(_DP_ONE_LANE);
    ...
    if(GET_DP_TX_STREAM_SOURCE_CHANGE() == _TRUE)
    {
        ...
        switch(GET_DP_TX_STREAM_SOURCE())
        {
            case _DP_TX_SOURCE_NONE:
                ...
                // Delay 150us for 2 Line Hs Delay to Avoid Dptx FSM Dead Lock
                DELAY_XUS(150);
...
#if((_SLEEP_AUDIO_SUPPORT == _ON) && ((_PS_AUDIO_INPUT == _PS_AUDIO_DP) || (_PS_AUDIO_INPUT == _PS_AUDIO_BOTH)))
    ScalerDpTxAudioSleepAudioControl();
#endif
...
void ScalerDpTxSSTSetting(void)
{
    ...
    ulMvidFWCal /= ((GET_DP_TX_DOWNSTREAM_DOWN_SPREAD() == _ENABLE) ? (27UL * (DWORD)GET_DP_TX_DOWNSTREAM_LINK_RATE() * 9975UL / 10000UL) : (27UL * (DWORD)GET_DP_TX_DOWNSTREAM_LINK_RATE()));

    ulMvidFWCal = ulMvidFWCal / 10 + (ulMvidFWCal % 10 > 5 ? 1 : 0);

    // Calculate HW measure Mvid period time, / 5 for get 5us times
    usMeasureTime = (WORD)(ulNvid / ((GET_DP_TX_DOWNSTREAM_DOWN_SPREAD() == _ENABLE) ? (27UL * GET_DP_TX_DOWNSTREAM_LINK_RATE() * 9975UL / 10000UL) : (27UL * (DWORD)GET_DP_TX_DOWNSTREAM_LINK_RATE())) / 5UL);
    usMeasureTime *= 2;
    ...
    do
    {
        ...
        if(abs(ulMvidFWCal - ulMvidHWMeasure) > (ulMvidFWCal >> 2))
        {
        ...
        if(abs(((ulMvid + (ucValidHwMvidMeaCnt / 2)) / ucValidHwMvidMeaCnt) - ulMvidHWMeasure) < 0x50)
        {
            if(ulMvidHWMeasure < ulHwMvidMin)
            {
                ulHwMvidMin = ulMvidHWMeasure;
    ...
    if(bMvidHWMode == _TRUE)
    {
        ...
        if(ucValidHwMvidMeaCnt > 2)
        {
            ...
            if(abs(ulMvidFWCal - ulMvid) > (ulMvidFWCal >> 1))
            {
                // HW Auto Mvid is wrong Set Mvid for FW mode
                ulMvid = ulMvidFWCal;
            }
    ...
    if(GET_DP_TX_EDID_VENDOR_MODEL() == _EDID_VENDOR_ASUS_PA248)
    {
        // Enable HW Auto Measure Mvid
        ScalerSetBit(PBB_A8_MN_VID_AUTO_EN_1, ~(_BIT7 | _BIT6), _BIT7);
...
void ScalerDpTxIRQEventCheck(void)
{
    ...
    if(GET_DP_TX_DOWNSTREAM_DPCD_REV() >= _DP_VERSION_1_2)
    {
        ...
        if(GET_DP_TX_IRQ_HPD_TYPE(_DP_TX_IRQ_HPD_DOWNSTREAM_PORT_STATUS_CHANGE) == _DP_TX_IRQ_HPD_DOWNSTREAM_PORT_STATUS_CHANGE)
        {
            ...
            ScalerDpTxAuxNativeRead(0x00, 0x02, 0x00, 1, &pucData[1]);

            SET_DP_TX_DOWNSTREAM_SINK_COUNT(pucData[1] & 0xBF);
            SET_DP_TX_DOWNSTREAM_SINK_COUNT_CHANGE();
...
void ScalerDpTxAudioSetting(void)
{
    ...
    else
    {
        ...
        ScalerDpTxAudioChannelStatusSetting();
#endif // End of #if(_AUDIO_SUPPORT == _ON)
    ...
    if(GET_DP_TX_AUDIO_STAND_ALONE() == _ON)
    {
        ...
        ucSecIdleEndCount = 2 + (48 / GET_DP_TX_DOWNSTREAM_LANE_NUM());
...
void ScalerDpTxAudioChannelStatusSetting(void)
{
    StructAudioChannelInfo stDpMacTxAudioStatus = {0x00, 0x00, 0x00, 0x00, 0x00};

    stDpMacTxAudioStatus = ScalerAudioGetRxChannelStatus();
    ...
    // Set Channel Status[23:20] to 4'b0000(Do not take into accoung) due to HW limitation.
    ScalerSetByte(PBC_87_CH_STATUS_2, (stDpMacTxAudioStatus.ucChannelStaus2 & 0x0F));
...
// Input Value  : _TRUE / _FALSE
// Output Value : None
//--------------------------------------------------
void ScalerDpTxAudioSetSleepAudioState(bit bAudioStandAloneStatus)
{
    if(bAudioStandAloneStatus == _TRUE)
    {
    ...
    if((ucPortType != _PORT_DP) || (GET_REG_DP_RX_MST_ENABLE() == _DISABLE))
    {
        ...
        else
        {
            if((GET_DP_TX_STREAM_SOURCE() == _DP_TX_SOURCE_NONE) && (GET_DP_TX_EXCEED_LINK_BANDWIDTH() == _FALSE))
            {
                ...
                // Get Tx Input Timing
                if(((ucPortType != _PORT_DP) && (ScalerDpTxMeasureInputInfo() == _TRUE)) ||
                   ((ucPortType == _PORT_DP) && (ScalerDpTxCloneCheckMSAInfo(ucInputPort) == _TRUE)))
                {
                    // DPTX Color Info Get
                    ScalerDpTxColorInfoSetting();

                    // Dp Tx Interlace info
                    ScalerDpTxInterlaceInfoSetting(ucInputPort, ucPortType);
...
void ScalerDpTxInterlaceInfoSetting(BYTE ucInputPort, BYTE ucPortType)
{
    if(GET_INPUT_TIMING_INTERLACE_FLG() == _TRUE)
    {
        ...
        {
            // Set Dp Tx Vtotal Number By Dp Rx Vtotal Number Info
            switch(ucInputPort)
            {
                ...
                case _D6_INPUT_PORT:
#endif
                    SET_DP_TX_INPUT_INTERLACE_VTOTAL_NUMBER(GET_DP_RX0_INTERLACE_VTOTAL_NUMBER());
                ...
                case _D1_INPUT_PORT:
                    ...
                    SET_DP_TX_INPUT_INTERLACE_VTOTAL_NUMBER(GET_DP_RX1_INTERLACE_VTOTAL_NUMBER());
...
bit ScalerDpTxBlockStatusCheck(EnumSourceSearchPort enumInputPort)
{
    ...
    if(GET_DP_RX_TARGET_MST_PORT() != _DP_MST_NO_PORT)
    {
        ...
        return ScalerDpRxHdcp14RepeaterCapabilitySwitchProc(enumInputPort, _ENABLE);
#endif
    }
    else
    {
...
#if(_DP_HDCP14_REPEATER_BIT_DISABLE_FOR_TX_CLONE_OFF == _ON)
            return ScalerDpRxHdcp14RepeaterCapabilitySwitchProc(enumInputPort, _DISABLE);
#endif
        }
        else
        {
...
void ScalerDpTxHDCPAuth1WriteAnAkey(void)
{
    ...
    ScalerDpTxAuxNativeWrite(0x06, 0x80, 0x0C, 8, pucData);
...
void ScalerDpTxHDCPAuth2WriteShaInput(void)
{
    ...
    do
    {
        ...
        ScalerDpTxAuxNativeRead(0x06, 0x80, 0x2C, ucKsvLengthReadOnce, pucKsvData);

        memcpy(&g_pucDpTxHdcpKsvFifo[usShaInputBitNumbers], pucKsvData, ucKsvLengthReadOnce);
...
void ScalerDpTxHDCPResetKsvBackup(void)
{
    memset(g_pucDpTxHdcpKsvBackup, 0, sizeof(g_pucDpTxHdcpKsvBackup));
}
...
bit ScalerDpTxHDCPDownstreamBksvChange(void)
{
    ...
    // Compare Bksv
    if((g_pucDpTxHdcpKsvBackup[0] != pData[0]) ||
       (g_pucDpTxHdcpKsvBackup[1] != pData[1]) ||
       (g_pucDpTxHdcpKsvBackup[2] != pData[2]) ||
       (g_pucDpTxHdcpKsvBackup[3] != pData[3]) ||
       (g_pucDpTxHdcpKsvBackup[4] != pData[4]))
    {
        return _TRUE;
    }
...
bit ScalerDpTxGetUpReqEnableStatusChange(void)
{
    if(GET_DP_TX_DOWNSTREAM_INFO_READY() == _FALSE)
    {
        return _FALSE;
    }
    ...
    if(GET_DP_TX_DOWNSTREAM_UP_REQUEST_CAP_STATUS() == _FALSE)
    {
        if(g_enumDpMSTCapablePort == _DP_MST_NO_PORT)
        {
            return _FALSE;
        ...
        if((bit)(GET_REG_DP_DPCD_INFO(0x00, 0x01, 0x11) & _BIT1) == _TRUE)
        {
            return _TRUE;
        }
    }
    else
    {
        ...
        if((GET_DP_GEN_DOWN_REQUEST_MSG() == _DP_MSG_CLEAR_PAYLOAD_ID_TABLE) ||
           (GET_DP_RX_SELF_GEN_DOWN_REQUEST_MSG() == _TRUE) || (GET_DP_TX_RECEIVED_MSG_COUNT() > 0))
        {
            return _FALSE;
        }
        ...
        if((bit)(GET_REG_DP_DPCD_INFO(0x00, 0x01, 0x11) & _BIT1) == _FALSE)
        {
            return _TRUE;
        }
    }
...
void ScalerDpTxUpdateDfpDeviceMsgCapFieldStatus(void)
{
    ...
    {
        pData[0] = (pData[0] & ~_BIT1) | ((!GET_DP_TX_DOWNSTREAM_UP_REQUEST_CAP_STATUS()) << 1);

        SET_DP_TX_DPCD_111(pData);

        // Clear Payload ID Only if Downstream PID Table is Not Empty
        if((GET_DP_TX_DOWNSTREAM_UP_REQUEST_CAP_STATUS() == _TRUE) && (GET_DP_TX_STREAM_SOURCE() != _DP_TX_SOURCE_MST) &&
           (ScalerDpTxAuxNativeRead(0x00, 0x02, 0xC1, 1, pData) == _TRUE))
        {
            if(pData[0] != 0x00)
            {
...
void ScalerDpTxDownStreamEdidCheckSpecialVendor(void)
{
    ...
    if(ScalerDpTxAuxI2CoverRead(0x50, ucEdidReadLength, pucVendorProduct, 0) == _FALSE)
    {
        return;
    }
    ...
    SET_DP_TX_EDID_VENDOR_MODEL(enumDPTxEdidVendorModel);
}
//--------------------------------------------------
...
void ScalerDpTxDownStreamEdidBwSetting(void)
{
    ...
    if(ScalerDpTxAuxI2CoverRead(0x50, 2, pucPixClkArray, 0) == _FALSE)
    {
        return;
    }

    // Unit: 0.01M
    SET_DP_TX_EDID_BW((((WORD)pucPixClkArray[1] << 8) | pucPixClkArray[0]));

    DebugMessageMst("Dp Tx Port Downstream Edid Native Bw = ", GET_DP_TX_EDID_BW());
}
