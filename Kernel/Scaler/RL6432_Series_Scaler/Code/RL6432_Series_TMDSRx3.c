...
//----------------------------------------------------------------------------------------------------
// ID Code      : RL6432_Series_TMDSRx3.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------

#define __RL6432_SERIES_TMDSRX3__
...
BYTE g_pucTMDSRx3LEMax[3];
BYTE g_pucTMDSRx3LEMin[3];
BYTE g_pucTMDSRx3Tap0Min[3];
BYTE g_pucTMDSRx3Tap1Max[3];
BYTE g_ucTMDSRx3Z0Status;
BYTE g_ucTMDSRx3SlaveAddr;
...
//****************************************************************************
void ScalerTMDSRx3Initial(void);
...
void ScalerTMDSRx3FLDBandFineTune_EXINT0(void);
#endif
...
#endif // End of #if(_D3_INPUT_PORT_TYPE != _D3_DUAL_DVI_PORT)
bit ScalerTMDSRx3CheckDataDFE(void);

bit ScalerTMDSRx3TMDSStableDetect(BYTE ucSourceType);
void ScalerTMDSRx3HotPlugEvent(void);
void ScalerTMDSRx3SwitchHdcpSlaveAddr(void);
void ScalerTMDSRx3BeforeHotPlugEventProc(bit bDisableZ0);
void ScalerTMDSRx3AfterHotPlugEventProc(bit bRestoreZ0);
void ScalerTMDSRx3Hdcp1p4AksvClear(void);
...
void ScalerTMDSRx3Initial(void)
{
    ...
    memset(&g_stTmdsMacRx3SpdInfo, 0x00, sizeof(g_stTmdsMacRx3SpdInfo));
#endif
    ...
    ScalerSetDataPortByte(P72_C9_HDMI_AP, _P72_CA_PT_A5_PORT_PAGE72_HDMI_PTRSV_3, _HDR_INFO_TYPE);
#endif
    ...
    if(ScalerGetBit(P72_E6_HDMI_FREQDET_CTRL, _BIT1) == 0x00)
    {
        ...
        ScalerSetBit(P72_38_HDMI_CTRL_38, ~(_BIT2 | _BIT1), _BIT2);
    ...
    ScalerSetBit(P72_38_HDMI_CTRL_38, ~(_BIT2 | _BIT1 | _BIT0), 0x00);
    ...
    ScalerSetBit(P72_1A_HDMI_CTRL_1A, ~_BIT3, 0x00);

    // Set time interval form bus free to Read Request(4.7us~500us)
    ScalerSetBit(P72_1B_HDMI_CTRL_1B, ~(_BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0), (_BIT3 | _BIT0));
...
void ScalerTMDSRx3InterruptInitial(void)
{
    ...
    ScalerSetBit(P72_F6_HDMI_FREQDET_BOUND_H, ~(_BIT1 | _BIT0), _BIT0);

    // Set Online Measure Input Clock Divider = /2 (No Matter if Input Clock >300Mhz or <=300Mhz)
    ScalerSetBit(P72_E7_HDMI_FREQDET_OFFSET, ~_BIT6, _BIT6);
    ...
    ScalerSetByte(P72_EC_HDMI_FREQDET_STABLE, 0x3F);
    ...
    ScalerSetBit(P72_26_HDMI_CTRL_26, ~_BIT4, _BIT4);
#endif
...
void ScalerTMDSRx3SetDFEInitial_EXINT0(void) using 1
{
    ...
    ScalerSetBit_EXINT(P7C_A0_L0_DFE_EN_1, ~(_BIT6 | _BIT5 | _BIT4 | _BIT3 | _BIT2), (_BIT6 | _BIT5));
    ScalerSetBit_EXINT(P7C_B0_L1_DFE_EN_1, ~(_BIT6 | _BIT5 | _BIT4 | _BIT3 | _BIT2), (_BIT6 | _BIT5));
    ScalerSetBit_EXINT(P7C_C0_L2_DFE_EN_1, ~(_BIT6 | _BIT5 | _BIT4 | _BIT3 | _BIT2), (_BIT6 | _BIT5));

    if(ScalerGetBit_EXINT(P7C_1B_CMU_01, _BIT0) == _BIT0) // DCDR
    {
    ...
    if(ScalerGetBit_EXINT(P7C_1B_CMU_01, _BIT0) == _BIT0) // DCDR
    {
        ...
        else if(GET_INPUT_PIXEL_CLK() <= 1000)
        {
            ...
            ScalerSetByte_EXINT(P7C_A2_L0_LIMIT_INIT, 0x8A);
            ScalerSetByte_EXINT(P7C_B2_L1_LIMIT_INIT, 0x8A);
            ScalerSetByte_EXINT(P7C_C2_L2_LIMIT_INIT, 0x8A);
    ...
    else    // ACDR
    {
        ...
        ScalerSetByte_EXINT(P7C_A2_L0_LIMIT_INIT, 0x5C);
        ScalerSetByte_EXINT(P7C_B2_L1_LIMIT_INIT, 0x5C);
        ScalerSetByte_EXINT(P7C_C2_L2_LIMIT_INIT, 0x5C);
...
// Output Value : None
//--------------------------------------------------
void ScalerTMDSRx3FLDBandFineTune_EXINT0(void) using 1
{
    WORD pusLaneFreq[3];
    BYTE ucCount = 0;
    BYTE ucLaneCount = 0;
    ...
    pucAutoVCOBand[0] = (ScalerGetByte_EXINT(P7C_35_ANA_CDR_11) & 0x7F);
    pucAutoVCOBand[1] = (ScalerGetByte_EXINT(P7C_36_ANA_CDR_12) & 0x7F);
    pucAutoVCOBand[2] = (ScalerGetByte_EXINT(P7C_37_ANA_CDR_13) & 0x7F);

    // Scan Lane
    ScalerSetBit_EXINT(P7C_32_ANA_CDR_08, ~(_BIT6 | _BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0), pucAutoVCOBand[0]);
    ScalerSetBit_EXINT(P7C_33_ANA_CDR_09, ~(_BIT6 | _BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0), pucAutoVCOBand[1]);
    ScalerSetBit_EXINT(P7C_34_ANA_CDR_10, ~(_BIT6 | _BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0), pucAutoVCOBand[2]);

    ucBacksetting2 = ScalerGetByte_EXINT(P72_EC_HDMI_FREQDET_STABLE);

    // Set Open Loop
    ScalerSetBit_EXINT(P7C_2B_ANA_CDR_01, ~(_BIT7 | _BIT6 | _BIT5), _BIT6);

    // Change Measure Count  64=>3
    ScalerSetBit_EXINT(P72_EC_HDMI_FREQDET_STABLE, ~(_BIT6 | _BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0), _BIT1);

    for(ucLaneCount = 0; ucLaneCount < 3; ucLaneCount++)
    {
        // Read Lane0 Result
        ScalerSetBit_EXINT(P72_E6_HDMI_FREQDET_CTRL, ~_BIT7, 0x00);
        ScalerSetBit_EXINT(P72_E6_HDMI_FREQDET_CTRL, ~(_BIT5 | _BIT4), ((ucLaneCount + 1) << 4));
        ...
        for(ucCount = 0; ucCount <= 60; ucCount++)
        {
            ...
            if(ScalerGetBit_EXINT(P72_E6_HDMI_FREQDET_CTRL, _BIT6) == _BIT6)
            {
                ...
                pusLaneFreq[ucLaneCount] = ((DWORD)((WORD)(ScalerGetByte_EXINT(P72_ED_HDMI_FREQDET_RESULT_M) << 8) | ScalerGetByte_EXINT(P72_EE_HDMI_FREQDET_RESULT_L))*27 / 100);
                break;
            }
        }
    }
    ...
    // Reload Best Band
    ScalerSetBit_EXINT(P7C_32_ANA_CDR_08, ~(_BIT6 | _BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0), pucBestVCOBand[0]);
    ScalerSetBit_EXINT(P7C_33_ANA_CDR_09, ~(_BIT6 | _BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0), pucBestVCOBand[1]);
    ScalerSetBit_EXINT(P7C_34_ANA_CDR_10, ~(_BIT6 | _BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0), pucBestVCOBand[2]);

    // Reload Measure TMDS Clock & Clear Unsatble IRQ Flag & Dummy POP Out
    ScalerSetBit_EXINT(P72_E6_HDMI_FREQDET_CTRL, ~(_BIT5 | _BIT4), 0x00);
    ScalerSetBit_EXINT(P72_E6_HDMI_FREQDET_CTRL, ~(_BIT7), 0x00);
    ScalerSetBit_EXINT(P72_E6_HDMI_FREQDET_CTRL, ~(_BIT7), _BIT7);
    for(ucCount = 0; ucCount <= 60; ucCount++)
    {
    ...
    // Set Stable Count
    ScalerSetByte_EXINT(P72_EC_HDMI_FREQDET_STABLE, ucBacksetting2);
...
void ScalerTMDSRx3SetPhy_EXINT0(void) using 1
{
    ...
    if(GET_INPUT_PIXEL_CLK() <= 2000)  // DCDR
    {
        ...
        ScalerSetBit_EXINT(P7C_2A_ANA_CDR_00, ~(_BIT3 | _BIT2 | _BIT1 | _BIT0), 0x00);
#endif
        // [0] En_APLL_Lane2 = 1'b1 --> Enable Lane2 VCO as CMU
        ScalerSetBit_EXINT(P7C_1B_CMU_01, ~_BIT0, _BIT0);

        if((GET_INPUT_PIXEL_CLK() <= 1000) && (ScalerGetBit_EXINT(P7C_17_BANDGAP_03, _BIT1) == _BIT1))  // MHL mode
        {
            // [6:5] CMU_REFCK_SEL = 2'b10 --> Ref. TMDS Clock w/ Hysteresis OP
            ScalerSetBit_EXINT(P7C_1B_CMU_01, ~(_BIT6 | _BIT5), _BIT6);
        }
        else
        {
            // [6:5] CMU_REFCK_SEL = 2'b00 --> Ref. TMDS Clock w/o Hysteresis OP
            ScalerSetBit_EXINT(P7C_1B_CMU_01, ~(_BIT6 | _BIT5), 0x00);
        }
    }
    else  // ACDR
    {
        ...
        ScalerSetBit_EXINT(P7C_2A_ANA_CDR_00, ~(_BIT3 | _BIT2 | _BIT1 | _BIT0), (_BIT2 | _BIT1 | _BIT0));
#endif
        // [0] En_APLL_Lane2 = 1'b0 --> Disable Lane2 VCO as CMU
        ScalerSetBit_EXINT(P7C_1B_CMU_01, ~_BIT0, 0x00);
    }
    ...
    ScalerSetBit_EXINT(P7C_1B_CMU_01, ~_BIT7, 0x00);

#if(_SOURCE_BACKGROUND_DETECT_SUPPORT == _ON)
    // [3:0] cdr_en = 4'b0000 --> Disable CDREN
    ScalerSetBit_EXINT(P7C_1A_CMU_00, ~_BIT3, 0x00);
    DELAY_5US_EXINT();
    ScalerSetBit_EXINT(P7C_1A_CMU_00, ~_BIT2, 0x00);
    DELAY_5US_EXINT();
    ScalerSetBit_EXINT(P7C_1A_CMU_00, ~_BIT1, 0x00);
    DELAY_5US_EXINT();
    ScalerSetBit_EXINT(P7C_1A_CMU_00, ~_BIT0, 0x00);

    if(ScalerGetBit_EXINT(P7C_17_BANDGAP_03, _BIT1) == _BIT1)  // MHL Mode
    {
        if(ScalerGetBit_EXINT(P7C_1B_CMU_01, _BIT0) == _BIT0) // DCDR
        {
            ...
            DELAY_5US_EXINT();
            ScalerSetBit_EXINT(P7C_1A_CMU_00, ~_BIT6, _BIT6);
            DELAY_5US_EXINT();
            ScalerSetBit_EXINT(P7C_1A_CMU_00, ~_BIT5, 0x00);
            DELAY_5US_EXINT();
            ScalerSetBit_EXINT(P7C_1A_CMU_00, ~_BIT4, _BIT4);
            DELAY_5US_EXINT();
            ScalerSetBit_EXINT(P7C_1A_CMU_00, ~_BIT2, _BIT2);
            DELAY_5US_EXINT();
            ScalerSetBit_EXINT(P7C_1A_CMU_00, ~_BIT0, _BIT0);
        }
        else
        {
            ...
            ScalerSetBit_EXINT(P7C_1A_CMU_00, ~_BIT7, _BIT7);
            DELAY_5US_EXINT();
            ScalerSetBit_EXINT(P7C_1A_CMU_00, ~_BIT6, 0x00);
            DELAY_5US_EXINT();
            ScalerSetBit_EXINT(P7C_1A_CMU_00, ~_BIT5, 0x00);
            DELAY_5US_EXINT();
            ScalerSetBit_EXINT(P7C_1A_CMU_00, ~_BIT4, _BIT4);
    ...
    else
    {
        ...
        ScalerSetBit_EXINT(P7C_1A_CMU_00, ~_BIT6, _BIT6);
        DELAY_5US_EXINT();
        ScalerSetBit_EXINT(P7C_1A_CMU_00, ~_BIT5, _BIT5);
        DELAY_5US_EXINT();
        ScalerSetBit_EXINT(P7C_1A_CMU_00, ~_BIT4, _BIT4);
        DELAY_5US_EXINT();
        ScalerSetBit_EXINT(P7C_1A_CMU_00, ~_BIT2, _BIT2);
        DELAY_5US_EXINT();
        ScalerSetBit_EXINT(P7C_1A_CMU_00, ~_BIT1, _BIT1);
        DELAY_5US_EXINT();
        ScalerSetBit_EXINT(P7C_1A_CMU_00, ~_BIT0, _BIT0);
    }
#else
    // [3:0] cdr_en = 4'b0000 --> Disable CDREN
    ScalerSetBit_EXINT(P7C_1A_CMU_00, ~(_BIT3 | _BIT2 | _BIT1 | _BIT0), 0x00);
    ...
    if(ScalerGetBit_EXINT(P7C_17_BANDGAP_03, _BIT1) == _BIT1)  // MHL Mode
    {
        ...
        {
            // [7:4] rx_en = 4'b1101 --> Enable RXEN for Lane0, 2, 3
            ScalerSetBit_EXINT(P7C_1A_CMU_00, ~(_BIT7 | _BIT6 | _BIT5 | _BIT4), (_BIT7 | _BIT6 | _BIT4));

            // [3:0] cdr_en = 4'b0101 --> Enable CDREX for Lane0, 2
            ScalerSetBit_EXINT(P7C_1A_CMU_00, ~(_BIT3 | _BIT2 | _BIT1 | _BIT0), (_BIT2 | _BIT0));
        }
        else
        {
            // [7:4] rx_en = 4'b1001 --> Enable RXEN for Lane0, 3
            ScalerSetBit_EXINT(P7C_1A_CMU_00, ~(_BIT7 | _BIT6 | _BIT5 | _BIT4), (_BIT7 | _BIT4));

            // [3:0] cdr_en = 4'b0001 --> Enable CDREX for Lane0
            ScalerSetBit_EXINT(P7C_1A_CMU_00, ~(_BIT3 | _BIT2 | _BIT1 | _BIT0), _BIT0);
        }
    }
    else
    {
        // [7:4] rx_en = 4'b1111 --> Enable RXEN
        ScalerSetBit_EXINT(P7C_1A_CMU_00, ~(_BIT7 | _BIT6 | _BIT5 | _BIT4), (_BIT7 | _BIT6 | _BIT5 | _BIT4));

        // [3:0] cdr_en = 4'b0001 ~ 4'b0011 ~ 4'b0111
        ScalerSetBit_EXINT(P7C_1A_CMU_00, ~(_BIT3 | _BIT2 | _BIT1 | _BIT0), _BIT0);
        ScalerSetBit_EXINT(P7C_1A_CMU_00, ~(_BIT3 | _BIT2 | _BIT1 | _BIT0), (_BIT1 | _BIT0));
        ScalerSetBit_EXINT(P7C_1A_CMU_00, ~(_BIT3 | _BIT2 | _BIT1 | _BIT0), (_BIT2 | _BIT1 | _BIT0));
    }
#endif
    ...
    ScalerSetBit_EXINT(P7C_2C_ANA_CDR_02, ~(_BIT1 | _BIT0), _BIT0);

    // [4] reg_sel_cont3 = 1'b1 --> BBPD UP/DN Pulse Div 3
    ScalerSetBit_EXINT(P7C_2D_ANA_CDR_03, ~_BIT4, _BIT4);

    // [7] en_fld_ref_ecbuss = 0 --> FLD Reference Lane3 Clock
    ScalerSetBit_EXINT(P7C_28_KOFFSET_04, ~_BIT7, 0x00);
    ...
    if(ScalerGetBit_EXINT(P7C_1B_CMU_01, _BIT0) == _BIT0) // DCDR
    {
        ...
        if(ScalerGetBit_EXINT(P7C_17_BANDGAP_03, _BIT1) == _BIT1)  // MHL Mode
        {
            ...
            ScalerSetBit_EXINT(P7C_1E_CMU_04, ~(_BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0), (_BIT5 | _BIT3 | _BIT1 | _BIT0));

            // [3:0] reg_cdr_cp_in = 4'b1010 --> Analog CDR Icp = 31.5uA
            ScalerSetBit_EXINT(P7C_31_ANA_CDR_07, ~(_BIT3 | _BIT2 | _BIT1 | _BIT0), (_BIT3 | _BIT1));
        }
        else   // HDMI Mode
#endif
        {
            ...
            ScalerSetBit_EXINT(P7C_1E_CMU_04, ~(_BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0), (_BIT5 | _BIT3 | _BIT2 | _BIT1));

            // [3:0] reg_cdr_cp_in = 4'b1100 --> Analog CDR Icp = 37.5uA
            ScalerSetBit_EXINT(P7C_31_ANA_CDR_07, ~(_BIT3 | _BIT2 | _BIT1 | _BIT0), (_BIT3 | _BIT2));
        }
    }
    else
    {
        ...
        ScalerSetBit_EXINT(P7C_1E_CMU_04, ~(_BIT5 | _BIT4 | _BIT3), 0x00);

        if(GET_INPUT_PIXEL_CLK() <= 3000)
        {
            // [6:4] reg_sel_cdr_res = 3'b101 --> Analog CDR Rs = 6K
            // [3:0] reg_cdr_cp_in = 4'b0101 --> Analog CDR Icp = 16.5uA
            ScalerSetBit_EXINT(P7C_31_ANA_CDR_07, ~(_BIT6 | _BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0), (_BIT6 | _BIT4 | _BIT2 | _BIT0));
        }
        else
        {
            // [6:4] reg_sel_cdr_res = 3'b011 --> Analog CDR Rs = 2K
            // [3:0] reg_cdr_cp_in = 4'b0011 --> Analog CDR Icp = 10.5uA
            ScalerSetBit_EXINT(P7C_31_ANA_CDR_07, ~(_BIT6 | _BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0), (_BIT5 | _BIT4 | _BIT1 | _BIT0));
        }
    }

    ucIcp = (ScalerGetByte_EXINT(P7C_31_ANA_CDR_07) & 0x0F);

    // [7:5] KD = 3'b000 --> KD = 0
    ScalerSetBit_EXINT(P7C_4F_KD, ~(_BIT7 | _BIT6 | _BIT5), 0x00);

#if(_D3_INPUT_PORT_TYPE == _D3_MHL_PORT)
    if(ScalerGetBit_EXINT(P7C_17_BANDGAP_03, _BIT1) == _BIT1)  // MHL Mode
    {
        // KI = 3'b001 --> KI = 1/16
        ScalerSetBit_EXINT(P7C_50_KI_L0, ~(_BIT2 | _BIT1 | _BIT0), _BIT0);

        if(GET_INPUT_PIXEL_CLK() <= 1000)
        {
            // KP = 0x23 --> KP = 8.75
            ScalerSetByte_EXINT(P7C_53_KP_L0, 0x23);
        }
        else
        {
            // KP = 0x2F --> KP = 11.75
            ScalerSetByte_EXINT(P7C_53_KP_L0, 0x2F);
        }
    }
    else   // HDMI Mode
#endif
    {
        // KI = 3'b000 --> KI = 0
        ScalerSetBit_EXINT(P7C_50_KI_L0, ~(_BIT2 | _BIT1 | _BIT0), 0x00);
        ScalerSetBit_EXINT(P7C_51_KI_L1, ~(_BIT2 | _BIT1 | _BIT0), 0x00);
        ScalerSetBit_EXINT(P7C_52_KI_L2, ~(_BIT2 | _BIT1 | _BIT0), 0x00);

        if(GET_INPUT_PIXEL_CLK() < 1500)
        {
            // KP = 0x20 --> KP = 8
            ScalerSetByte_EXINT(P7C_53_KP_L0, 0x20);
            ScalerSetByte_EXINT(P7C_54_KP_L1, 0x20);
            ScalerSetByte_EXINT(P7C_55_KP_L2, 0x20);
        }
        else
        {
            // KP = 0x0F --> KP = 3.75
            ScalerSetByte_EXINT(P7C_53_KP_L0, 0x0F);
            ScalerSetByte_EXINT(P7C_54_KP_L1, 0x0F);
            ScalerSetByte_EXINT(P7C_55_KP_L2, 0x0F);
        }
    }
    ...
    if(ScalerGetBit_EXINT(P7C_17_BANDGAP_03, _BIT1) == _BIT1)  // MHL Mode
    {
        ...
        if(GET_TMDS_RX3_CLK_MODE() == _TMDS_MHL_PPMODE_PHY_SETTING)
        {
            if(GET_INPUT_PIXEL_CLK() <= 1500)
            {
                ...
                ScalerSetBit_EXINT(P7C_2D_ANA_CDR_03, ~(_BIT1 | _BIT0), _BIT0);
                ScalerSetByte_EXINT(P7C_2E_ANA_CDR_04, 0x1D);
                ScalerSetByte_EXINT(P7C_2F_ANA_CDR_05, 0xDF);

                // [5:4] CMU_SELPREDIV = 2'b00 --> /1
                ScalerSetBit_EXINT(P7C_1F_CMU_05, ~(_BIT5 | _BIT4), 0x00);

                // [7:0] N_CODE = 8'b0100 1100 --> 76 + 4 = 80
                ScalerSetByte_EXINT(P7C_20_CMU_06, 0x4C);

                // [2:0] SEL_DIV = 3'b010 --> 750MHz ~ 1500MHz
                ScalerSetBit_EXINT(P7C_21_CMU_07, ~(_BIT2 | _BIT1 | _BIT0), _BIT1);

                // [6:4] rate_sel = 3'b010 --> 750MHz ~ 1500MHz
                ScalerSetBit_EXINT(P7C_3E_EN_01, ~(_BIT6 | _BIT5 | _BIT4), _BIT5);
            }
            else
            {
                // Analog CDR Auto FLD Counter
                ...
                ScalerSetBit_EXINT(P7C_2D_ANA_CDR_03, ~(_BIT1 | _BIT0), _BIT0);
                ScalerSetByte_EXINT(P7C_2E_ANA_CDR_04, 0x31);
                ScalerSetByte_EXINT(P7C_2F_ANA_CDR_05, 0x8F);

                // [5:4] CMU_SELPREDIV = 2'b00 --> /1
                ScalerSetBit_EXINT(P7C_1F_CMU_05, ~(_BIT5 | _BIT4), 0x00);

                // [7:0] N_CODE = 8'b0010 0100 --> 36 + 4 = 40
                ScalerSetByte_EXINT(P7C_20_CMU_06, 0x24);

                // [2:0] rate_sel = 3'b001 --> 1500MHz ~ 3000MHz
                ScalerSetBit_EXINT(P7C_21_CMU_07, ~(_BIT2 | _BIT1 | _BIT0), _BIT0);

                // [6:4] SEL_DIV = 3'b001 --> 1500MHz ~ 3000MHz
                ScalerSetBit_EXINT(P7C_3E_EN_01, ~(_BIT6 | _BIT5 | _BIT4), _BIT4);
            }
        }
        else  // MHL 24bit Mode
        {
            if(GET_INPUT_PIXEL_CLK() <= 750)
            {
                ...
                ScalerSetBit_EXINT(P7C_2D_ANA_CDR_03, ~(_BIT1 | _BIT0), _BIT0);
                ScalerSetByte_EXINT(P7C_2E_ANA_CDR_04, 0x13);
                ScalerSetByte_EXINT(P7C_2F_ANA_CDR_05, 0xDF);
                ...
                ScalerSetByte_EXINT(P7C_20_CMU_06, 0x74);

                // [2:0] SEL_DIV = 3'b011 --> 375MHz ~ 750MHz
                ScalerSetBit_EXINT(P7C_21_CMU_07, ~(_BIT2 | _BIT1 | _BIT0), (_BIT1 | _BIT0));

                // [6:4] rate_sel = 3'b011 --> 375MHz ~ 750MHz
                ScalerSetBit_EXINT(P7C_3E_EN_01, ~(_BIT6 | _BIT5 | _BIT4), (_BIT5 | _BIT4));
            }
            else if(GET_INPUT_PIXEL_CLK() <= 1500)
            {
                ...
                ScalerSetBit_EXINT(P7C_2D_ANA_CDR_03, ~(_BIT1 | _BIT0), _BIT0);
                ScalerSetByte_EXINT(P7C_2E_ANA_CDR_04, 0x27);
                ScalerSetByte_EXINT(P7C_2F_ANA_CDR_05, 0xDF);
                ...
                ScalerSetByte_EXINT(P7C_20_CMU_06, 0x38);
            ...
            else if(GET_INPUT_PIXEL_CLK() <= 3000)
            {
                ...
                ScalerSetBit_EXINT(P7C_2D_ANA_CDR_03, ~(_BIT1 | _BIT0), _BIT0);
                ScalerSetByte_EXINT(P7C_2E_ANA_CDR_04, 0x4F);
                ScalerSetByte_EXINT(P7C_2F_ANA_CDR_05, 0xDF);
                ...
                ScalerSetByte_EXINT(P7C_20_CMU_06, 0x1A);
            ...
            else
            {
                ...
                ScalerSetBit_EXINT(P7C_2D_ANA_CDR_03, ~(_BIT1 | _BIT0), _BIT0);
                ScalerSetByte_EXINT(P7C_2E_ANA_CDR_04, 0x9F);
                ScalerSetByte_EXINT(P7C_2F_ANA_CDR_05, 0xDF);
                ...
                ScalerSetByte_EXINT(P7C_20_CMU_06, 0x0B);

                // [2:0] SEL_DIV = 3'b000 --> 3000MHz ~ 6000MHz
                ScalerSetBit_EXINT(P7C_21_CMU_07, ~(_BIT2 | _BIT1 | _BIT0), 0x00);

                // [6:4] rate_sel = 3'b000 --> 3000MHz ~ 6000MHz
                ScalerSetBit_EXINT(P7C_3E_EN_01, ~(_BIT6 | _BIT5 | _BIT4), 0x00);
            }
        }
    }
    else     // HDMI Mode
    ...
    {
#if(_D3_HDMI_2_0_SUPPORT == _ON)
        if(GET_TMDS_RX3_HIGH_SPEED_MODE() == _FALSE)  // 1:10
#endif
        {
            if(GET_INPUT_PIXEL_CLK() <= 375)
            {
                ...
                ScalerSetBit_EXINT(P7C_21_CMU_07, ~(_BIT2 | _BIT1 | _BIT0), _BIT2);

                // [6:4] rate_sel = 3'b100 --> 187.5MHz ~ 375MHz
                ScalerSetBit_EXINT(P7C_3E_EN_01, ~(_BIT6 | _BIT5 | _BIT4), _BIT6);
            }
            ...
            else if(GET_INPUT_PIXEL_CLK() <= 1500)
            {
                ...
                ScalerSetBit_EXINT(P7C_2D_ANA_CDR_03, ~(_BIT1 | _BIT0), _BIT0);
                ScalerSetByte_EXINT(P7C_2E_ANA_CDR_04, 0x7C);
                ScalerSetByte_EXINT(P7C_2F_ANA_CDR_05, 0xF3);

                // [5:4] CMU_SELPREDIV = 2'b00 --> /1
                ScalerSetBit_EXINT(P7C_1F_CMU_05, ~(_BIT5 | _BIT4), 0x00);

                // [7:0] N_CODE = 8'b0001 0000 --> 16 + 4 = 20
                ScalerSetByte_EXINT(P7C_20_CMU_06, 0x10);
            ...
            else if(GET_INPUT_PIXEL_CLK() <= 3000)
            {
                ...
                ScalerSetBit_EXINT(P7C_2D_ANA_CDR_03, ~(_BIT1 | _BIT0), _BIT0);
                ScalerSetByte_EXINT(P7C_2E_ANA_CDR_04, 0xF9);
                ScalerSetByte_EXINT(P7C_2F_ANA_CDR_05, 0xF3);

                // [5:4] CMU_SELPREDIV = 2'b00 --> /2
                ScalerSetBit_EXINT(P7C_1F_CMU_05, ~(_BIT5 | _BIT4), _BIT4);
            ...
            else     // ACDR 1:10
            {
                ...
                ScalerSetBit_EXINT(P7C_2D_ANA_CDR_03, ~(_BIT1 | _BIT0), (_BIT1 | _BIT0));
                ScalerSetByte_EXINT(P7C_2E_ANA_CDR_04, 0xF3);
                ScalerSetByte_EXINT(P7C_2F_ANA_CDR_05, 0xF3);
    ...
    ScalerSetBit_EXINT(P7C_0E_SDM_03, ~_BIT0, 0x00);

    // [0] en_eqen_manual = 0 --> Disable Clkout
    ScalerSetBit_EXINT(P7C_10_EQEN_TIMER, ~_BIT0, 0x00);

    // [0] en_data_manual = 0 --> Disable Dataout
    ScalerSetBit_EXINT(P7C_11_DATA_TIMER, ~_BIT0, 0x00);
    ...
    ScalerSetBit_EXINT(P7C_12_ANA_CDR, ~(_BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0), (_BIT2 | _BIT0));

    // [6:4] RST_N = 3'b000 = 3'b111 -> Digital Circuit Reset Toggle
    // [2:0] Disable DCDR
    ScalerSetBit_EXINT(P7C_38_TMDS_RESET, ~(_BIT6 | _BIT5 | _BIT4 | _BIT2 | _BIT1 | _BIT0), 0x00);
    ScalerSetBit_EXINT(P7C_38_TMDS_RESET, ~(_BIT6 | _BIT5 | _BIT4 | _BIT2 | _BIT1 | _BIT0), (_BIT6 | _BIT5 | _BIT4));
    ...
    // Enable Tap0~4
    ScalerSetBit_EXINT(P7C_21_CMU_07, ~(_BIT7 | _BIT6 | _BIT5 | _BIT4 | _BIT3), (_BIT7 | _BIT6 | _BIT5 | _BIT4 | _BIT3));

    if(GET_INPUT_PIXEL_CLK() <= 2500)
    {
        // [3:1] LE_lowgain_highboost = 111 --> LE HighBoost Mode
        ScalerSetBit_EXINT(P7C_22_CMU_08, ~(_BIT3 | _BIT2 | _BIT1), (_BIT3 | _BIT2 | _BIT1));
    }
    else
    {
        // [3:1] LE_lowgain_highboost = 100 --> LE HighBoost Mode
        ScalerSetBit_EXINT(P7C_22_CMU_08, ~(_BIT3 | _BIT2 | _BIT1), _BIT3);
    }

    if(GET_INPUT_PIXEL_CLK() <= 2000)
    {
        // [7] SEL_DFE_DELAY = 1'b1 --> Increase DFE Delay
        ScalerSetBit_EXINT(P7C_22_CMU_08, ~_BIT7, _BIT7);
    }
    else
    {
        // [7] SEL_DFE_DELAY = 1'b0 --> Normal DFE Delay
        ScalerSetBit_EXINT(P7C_22_CMU_08, ~_BIT7, 0x00);
    }
    ...
    if(GET_INPUT_PIXEL_CLK() <= 2500)
    {
        // [7:5] LE_RLSEL = 3'b011
        // [4:2] LE_ISEL = 3'b000
        ScalerSetBit_EXINT(P7C_23_CMU_09, ~(_BIT7 | _BIT6 | _BIT5 | _BIT4 | _BIT3 | _BIT2), (_BIT6 | _BIT5));
    }
    else if(GET_INPUT_PIXEL_CLK() <= 3400)
    {
        // [7:5] LE_RLSEL = 3'b001
        // [4:2] LE_ISEL = 3'b010
        ScalerSetBit_EXINT(P7C_23_CMU_09, ~(_BIT7 | _BIT6 | _BIT5 | _BIT4 | _BIT3 | _BIT2), (_BIT5 | _BIT3));
    }
    else
    {
        // [7:5] LE_RLSEL = 3'b000
        // [4:2] LE_ISEL = 3'b011
        ScalerSetBit_EXINT(P7C_23_CMU_09, ~(_BIT7 | _BIT6 | _BIT5 | _BIT4 | _BIT3 | _BIT2), (_BIT3 | _BIT2));
    }

    if(GET_INPUT_PIXEL_CLK() <= 3000)
    {
        // [0] Half_PD = 0 --> Full Rate PD
        ScalerSetBit_EXINT(P7C_22_CMU_08, ~_BIT0, 0x00);
    }
    else
    {
        // [0] Half_PD = 1 --> Half Rate PD
        ScalerSetBit_EXINT(P7C_22_CMU_08, ~_BIT0, _BIT0);
    }
    ...
    ScalerSetBit_EXINT(P7C_1D_CMU_03, ~_BIT4, 0x00);

    // [5] Enable Foreground Calibration Auto Mode
    ScalerSetBit_EXINT(P7C_24_KOFFSET_00, ~_BIT5, _BIT5);

    // [6] Toggle Foreground Calibration
    ScalerSetBit_EXINT(P7C_24_KOFFSET_00, ~_BIT6, _BIT6);
    ScalerSetBit_EXINT(P7C_24_KOFFSET_00, ~_BIT6, 0x00);
    ScalerSetBit_EXINT(P7C_24_KOFFSET_00, ~_BIT6, _BIT6);
    ...
    ScalerSetBit_EXINT(P7C_2B_ANA_CDR_01, ~(_BIT7 | _BIT6 | _BIT5 | _BIT4 | _BIT3 | _BIT0), (_BIT7 | _BIT0));

    // [5] en_wide_temp = 1'b1 --> Enable Wide Temp VCO Band
    ScalerSetBit_EXINT(P7C_2D_ANA_CDR_03, ~_BIT5, _BIT5);

    // [3:2] en_vcm_tune = 2'b11 --> Vc = Vdd/2
    ScalerSetBit_EXINT(P7C_2C_ANA_CDR_02, ~(_BIT3 | _BIT2), (_BIT3 | _BIT2));

    if(ScalerGetBit_EXINT(P7C_1B_CMU_01, _BIT0) == _BIT0) // DCDR
    {
        // [7] reg_rstb_fsm --> Toggle for FLD reset
        ScalerSetBit_EXINT(P7C_2A_ANA_CDR_00, ~_BIT7, 0x00);
        ScalerSetBit_EXINT(P7C_2A_ANA_CDR_00, ~_BIT7, _BIT7);

        // Waiting for FLD Lock Band
        DELAY_XUS_EXINT(360);

        // [7] cmu_en = 1'b1 --> Enable CMU PLL
        ScalerSetBit_EXINT(P7C_1B_CMU_01, ~_BIT7, _BIT7);

        // Enable Digital CDR
        ScalerSetByte_EXINT(P7C_38_TMDS_RESET, 0xFF);

        // Waiting for CMU Stable
        DELAY_XUS_EXINT(150);

        // [0] en_clkout_manual = 1 --> Enable Clkout
        ScalerSetBit_EXINT(P7C_0E_SDM_03, ~_BIT0, _BIT0);

        // [0] en_eqen_manual = 1 --> Enable Adaptation
        ScalerSetBit_EXINT(P7C_10_EQEN_TIMER, ~_BIT0, _BIT0);
    ...
    else if(GET_INPUT_PIXEL_CLK() <= 3400) // 2000~3400 Analog CDR
    {
        ...
        ScalerSetBit_EXINT(P7C_12_ANA_CDR, ~(_BIT4 | _BIT1), (_BIT4 | _BIT1));
        ...
        if(ScalerGetBit_EXINT(P7C_17_BANDGAP_03, _BIT1) == _BIT1)  // MHL Mode
        {
            if(GET_INPUT_PIXEL_CLK() > 2500) // for 2500~3000 MHL PP Mode
            {
                ...
                g_pucTMDSRx3LEMax[0] = ScalerGetByte_EXINT(P7C_F3_COEF_4);

                // Set Max Value of LE
                ScalerSetBit_EXINT(P7C_A5_L0_INIT_3, ~(_BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0), g_pucTMDSRx3LEMax[0]);

                // Reload LE Max
                ScalerSetBit_EXINT(P7C_AA_L0_INIT_8, ~_BIT1, _BIT1);
                ScalerSetBit_EXINT(P7C_AA_L0_INIT_8, ~_BIT1, 0x00);
            }
        }
        else // HDMI Mode
#endif
        {
            if(GET_INPUT_PIXEL_CLK() > 2900) // for 2900~3400 10M Cable Case
            {
                ...
                g_pucTMDSRx3LEMax[1] = ScalerGetByte_EXINT(P7C_F3_COEF_4);

                // Read Back Lane2
                ScalerSetBit_EXINT(P7C_F2_COEF_3, ~(_BIT7 | _BIT6), _BIT7);
                g_pucTMDSRx3LEMax[2] = ScalerGetByte_EXINT(P7C_F3_COEF_4);

                // Read Back Tap1 Max
                ScalerSetBit_EXINT(P7C_F2_COEF_3, ~(_BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1), _BIT2);

                // Read Back Lane0
                ScalerSetBit_EXINT(P7C_F2_COEF_3, ~(_BIT7 | _BIT6), 0x00);
                g_pucTMDSRx3Tap1Max[0] = ScalerGetByte_EXINT(P7C_F3_COEF_4);

                // Read Back Lane1
                ScalerSetBit_EXINT(P7C_F2_COEF_3, ~(_BIT7 | _BIT6), _BIT6);
                g_pucTMDSRx3Tap1Max[1] = ScalerGetByte_EXINT(P7C_F3_COEF_4);

                // Read Back Lane2
                ScalerSetBit_EXINT(P7C_F2_COEF_3, ~(_BIT7 | _BIT6), _BIT7);
                g_pucTMDSRx3Tap1Max[2] = ScalerGetByte_EXINT(P7C_F3_COEF_4);

                // Lane0 LEQ >= 20
                if(g_pucTMDSRx3LEMax[0] >= 16)
                {
                    // Set Tap2 = 6
                    ScalerSetByte_EXINT(P7C_A6_L0_INIT_4, 0x06);

                    // Set Max Value of LE + Tap1
                    ScalerSetBit_EXINT(P7C_A5_L0_INIT_3, ~(_BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0), (g_pucTMDSRx3LEMax[0] + g_pucTMDSRx3Tap1Max[0]));

                    // Reload LE/Tap1~2
                    ScalerSetBit_EXINT(P7C_AA_L0_INIT_8, ~(_BIT2 | _BIT1), (_BIT2 | _BIT1));
                    ScalerSetBit_EXINT(P7C_AA_L0_INIT_8, ~(_BIT2 | _BIT1), 0x00);
                }

                // Lane1 LEQ >= 20
                if(g_pucTMDSRx3LEMax[1] >= 16)
                {
                    // Set Tap2 = 6
                    ScalerSetByte_EXINT(P7C_B6_L1_INIT_4, 0x06);

                    // Set Max Value of LE + Tap1
                    ScalerSetBit_EXINT(P7C_B5_L1_INIT_3, ~(_BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0), (g_pucTMDSRx3LEMax[1] + g_pucTMDSRx3Tap1Max[1]));

                    // Reload LE/Tap1~2
                    ScalerSetBit_EXINT(P7C_BA_L1_INIT_8, ~(_BIT2 | _BIT1), (_BIT2 | _BIT1));
                    ScalerSetBit_EXINT(P7C_BA_L1_INIT_8, ~(_BIT2 | _BIT1), 0x00);
                }

                // Lane2 LEQ >= 20
                if(g_pucTMDSRx3LEMax[2] >= 16)
                {
                    // Set Tap2 = 6
                    ScalerSetByte_EXINT(P7C_C6_L2_INIT_4, 0x06);

                    // Set Max Value of LE + Tap1
                    ScalerSetBit_EXINT(P7C_C5_L2_INIT_3, ~(_BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0), (g_pucTMDSRx3LEMax[2] + g_pucTMDSRx3Tap1Max[2]));

                    // Reload LE/Tap1~2
                    ScalerSetBit_EXINT(P7C_CA_L2_INIT_8, ~(_BIT2 | _BIT1), (_BIT2 | _BIT1));
                    ScalerSetBit_EXINT(P7C_CA_L2_INIT_8, ~(_BIT2 | _BIT1), 0x00);
                }
            }
        }
    ...
    else  // 3400~6000 Analog CDR
    {
        ...
        // Read Back Tap0 Min
        ScalerSetBit_EXINT(P7C_F2_COEF_3, ~(_BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1), _BIT1);

        // Read Back Lane0
        ScalerSetBit_EXINT(P7C_F2_COEF_3, ~(_BIT7 | _BIT6), 0x00);
        g_pucTMDSRx3Tap0Min[0] = ScalerGetByte_EXINT(P7C_F3_COEF_4);

        // Read Back Lane1
        ScalerSetBit_EXINT(P7C_F2_COEF_3, ~(_BIT7 | _BIT6), _BIT6);
        g_pucTMDSRx3Tap0Min[1] = ScalerGetByte_EXINT(P7C_F3_COEF_4);

        // Read Back Lane2
        ScalerSetBit_EXINT(P7C_F2_COEF_3, ~(_BIT7 | _BIT6), _BIT7);
        g_pucTMDSRx3Tap0Min[2] = ScalerGetByte_EXINT(P7C_F3_COEF_4);

        // Reload Max Value of Tap0
        ScalerSetByte_EXINT(P7C_A4_L0_INIT_2, g_pucTMDSRx3Tap0Min[0]);
        ScalerSetByte_EXINT(P7C_B4_L1_INIT_2, g_pucTMDSRx3Tap0Min[1]);
        ScalerSetByte_EXINT(P7C_C4_L2_INIT_2, g_pucTMDSRx3Tap0Min[2]);
        ...
        ScalerSetByte_EXINT(P7C_A1_L0_DFE_EN_2, 0x82);
        ScalerSetByte_EXINT(P7C_B1_L1_DFE_EN_2, 0x82);
        ScalerSetByte_EXINT(P7C_C1_L2_DFE_EN_2, 0x82);
        ...
        // Read Back LE Min
        ScalerSetBit_EXINT(P7C_F2_COEF_3, ~(_BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1), (_BIT3 | _BIT1));

        // Read Back Lane0
        ScalerSetBit_EXINT(P7C_F2_COEF_3, ~(_BIT7 | _BIT6), 0x00);
        g_pucTMDSRx3LEMin[0] = ScalerGetByte_EXINT(P7C_F3_COEF_4);

        // Read Back Lane1
        ScalerSetBit_EXINT(P7C_F2_COEF_3, ~(_BIT7 | _BIT6), _BIT6);
        g_pucTMDSRx3LEMin[1] = ScalerGetByte_EXINT(P7C_F3_COEF_4);

        // Read Back Lane2
        ScalerSetBit_EXINT(P7C_F2_COEF_3, ~(_BIT7 | _BIT6), _BIT7);
        g_pucTMDSRx3LEMin[2] = ScalerGetByte_EXINT(P7C_F3_COEF_4);
        ...
        ScalerSetByte_EXINT(P7C_A5_L0_INIT_3, g_pucTMDSRx3LEMax[0] + g_pucTMDSRx3Tap1Max[0]);
        ScalerSetByte_EXINT(P7C_B5_L1_INIT_3, g_pucTMDSRx3LEMax[1] + g_pucTMDSRx3Tap1Max[1]);
        ScalerSetByte_EXINT(P7C_C5_L2_INIT_3, g_pucTMDSRx3LEMax[2] + g_pucTMDSRx3Tap1Max[2]);

        // Reload LE + Tap1
        ScalerSetByte_EXINT(P7C_AA_L0_INIT_8, 0x02);
        ScalerSetByte_EXINT(P7C_BA_L1_INIT_8, 0x02);
        ScalerSetByte_EXINT(P7C_CA_L2_INIT_8, 0x02);
        ScalerSetByte_EXINT(P7C_AA_L0_INIT_8, 0x00);
        ScalerSetByte_EXINT(P7C_BA_L1_INIT_8, 0x00);
        ScalerSetByte_EXINT(P7C_CA_L2_INIT_8, 0x00);
    }
    ...
    if(ScalerGetBit_EXINT(P7C_17_BANDGAP_03, _BIT1) == _BIT1)  // MHL Mode
    {
        ...
        ScalerSetBit_EXINT(P7C_2B_ANA_CDR_01, ~_BIT4, _BIT4);

        // [1] reg_en_eqen_manual_l0 = 1'b0
        ScalerSetBit_EXINT(P7C_12_ANA_CDR, ~_BIT1, 0x00);

        // Reload Tap0 = 21
        ScalerSetByte_EXINT(P7C_A4_L0_INIT_2, 0x15);
        ScalerSetByte_EXINT(P7C_AA_L0_INIT_8, 0x01);
        ScalerSetByte_EXINT(P7C_AA_L0_INIT_8, 0x00);

        // Enable Sumamp High Gain Mode
        ScalerSetBit_EXINT(P7C_1D_CMU_03, ~_BIT4, _BIT4);
        ...
        ScalerTMDSRx3FLDBandFineTune_EXINT0();

        // [3:0] reg_cdr_cp_in --> Double Icp to Enlarge Locking Range
        if(ucIcp > 7)
        {
            ScalerSetBit_EXINT(P7C_31_ANA_CDR_07, ~(_BIT3 | _BIT2 | _BIT1 | _BIT0), (_BIT3 | _BIT2 | _BIT1 | _BIT0));
        }
        else
        {
            ScalerSetBit_EXINT(P7C_31_ANA_CDR_07, ~(_BIT3 | _BIT2 | _BIT1 | _BIT0), (ucIcp << 1));
        }
        ...
        ScalerSetBit_EXINT(P7C_2B_ANA_CDR_01, ~(_BIT6 | _BIT5), (_BIT6 | _BIT5));

        // Waiting for VCO Stable
        DELAY_XUS_EXINT(80);

        // [3:0] reg_cdr_cp_in --> Restore Icp to Original Setting
        ScalerSetBit_EXINT(P7C_31_ANA_CDR_07, ~(_BIT3 | _BIT2 | _BIT1 | _BIT0), ucIcp);
...
#endif
    {
        ...
        else // 2000~6000 ACDR
        {
            ...
            else
            {
                // Reload Tap0 = 21
                ScalerSetByte_EXINT(P7C_A4_L0_INIT_2, 0x15);
                ScalerSetByte_EXINT(P7C_B4_L1_INIT_2, 0x15);
                ScalerSetByte_EXINT(P7C_C4_L2_INIT_2, 0x15);
                ScalerSetByte_EXINT(P7C_AA_L0_INIT_8, 0x01);
                ScalerSetByte_EXINT(P7C_BA_L1_INIT_8, 0x01);
                ScalerSetByte_EXINT(P7C_CA_L2_INIT_8, 0x01);
                ScalerSetByte_EXINT(P7C_AA_L0_INIT_8, 0x00);
            ...
            if((ScalerGetByte_EXINT(P7C_27_KOFFSET_03) & 0x1F) > 24)
            {
                // Set Servo = 24
                ScalerSetBit_EXINT(P7C_A3_L0_INIT_1, ~(_BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0), (_BIT4 | _BIT3));
            }
            else if((ScalerGetByte_EXINT(P7C_27_KOFFSET_03) & 0x1F) < 8)
            {
                // Set Servo = 8
            ...
            if((ScalerGetByte_EXINT(P7C_28_KOFFSET_04) & 0x1F) > 24)
            {
                // Set Servo = 24
                ScalerSetBit_EXINT(P7C_B3_L1_INIT_1, ~(_BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0), (_BIT4 | _BIT3));
            }
            else if((ScalerGetByte_EXINT(P7C_28_KOFFSET_04) & 0x1F) < 8)
            {
                // Set Servo = 8
            ...
            if((ScalerGetByte_EXINT(P7C_29_KOFFSET_05) & 0x1F) > 24)
            {
                // Set Servo = 24
                ScalerSetBit_EXINT(P7C_C3_L2_INIT_1, ~(_BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0), (_BIT4 | _BIT3));
            }
            else if((ScalerGetByte_EXINT(P7C_29_KOFFSET_05) & 0x1F) < 8)
            {
                // Set Servo = 8
    ...
    ScalerSetBit_EXINT(P7C_12_ANA_CDR, ~_BIT3, _BIT3);

    // [0] en_data_manual = 1'b1 --> Enable Dataout
    ScalerSetBit_EXINT(P7C_11_DATA_TIMER, ~_BIT0, _BIT0);
...
void ScalerTMDSRx3IntHandler_EXINT0(void) using 1
{
...
#if(_SOURCE_BACKGROUND_DETECT_SUPPORT == _ON)
    if(g_ucSourceDetectingPort == _D3_INPUT_PORT)
    {
    ...
    if(CHECK_TMDS_DPHY_SWITCH(_D3_INPUT_PORT) == _TRUE)
    {
        ...
        if((ScalerGetByte_EXINT(P72_EF_HDMI_ERROR_TH) & 0xC0) == 0xC0)
        {
            ...
            if((GET_TMDS_RX3_PHY_SET() == _FALSE) || ((bit)ScalerTMDSRx3CheckCDRStable_EXINT0(g_usTMDSRx3ClkCount) == _FALSE))
            {
                ...
                if(ScalerGetBit_EXINT(P72_CA_HDMI_DP, _BIT6) == _BIT6)
                {
                    ...
                    SET_TMDS_OFFLINE_MEASURE_CLK(((DWORD)(_GDI_CLK_KHZ * 1024) * 2 / 100) / g_usTMDSRx3ClkCount);
                }
                else
                {
                    // TMDS_OFFLine Measure FREQ(MHz) = GDI_CLK(MHz) * 1024 / g_usTMDSRx3ClkCount
                    SET_TMDS_OFFLINE_MEASURE_CLK(((DWORD)(_GDI_CLK_KHZ * 1024) / 100) / g_usTMDSRx3ClkCount);
                }
...
#if((_TMDS_HDCP_2_2_SUPPORT == _ON) && ((_D3_INPUT_PORT_TYPE == _D3_HDMI_PORT) || (_D3_INPUT_PORT_TYPE == _D3_MHL_PORT)))
                if(GET_TMDS_RX3_HDCP2_AUTHENTICATION_DONE() == _TRUE)
                {
            ...
            ScalerSetBit_EXINT(P72_14_HDMI_CTRL_14, ~(_BIT3 | _BIT2 | _BIT1 | _BIT0), (GET_INPUT_PIXEL_CLK() / 10) >> 8);
            ScalerSetByte_EXINT(P72_15_HDMI_CTRL_15, (GET_INPUT_PIXEL_CLK() / 10));
        ...
        if((ScalerGetByte_EXINT(P72_E6_HDMI_FREQDET_CTRL) & 0x06) == 0x06)
        {
            ...
            {
#if(_SOURCE_BACKGROUND_DETECT_SUPPORT == _ON)
                if(GET_STORE_ACTIVED_PORT() == _D3_INPUT_PORT)
#endif
                {
    ...
    if((ScalerGetBit_EXINT(P72_17_HDMI_CTRL_17, _BIT2) == _BIT2) && (ScalerGetBit_EXINT(P72_18_HDMI_CTRL_18, _BIT2) == _BIT2))
    {
        ...
        ScalerSetBit_EXINT(P72_17_HDMI_CTRL_17, ~(_BIT2 | _BIT1 | _BIT0), _BIT2);
    ...
    {
        // Clear Lock Transition Flag
        ScalerSetBit_EXINT(P72_17_HDMI_CTRL_17, ~(_BIT2 | _BIT1 | _BIT0), _BIT1);
    ...
    {
        // Clear Lock Transition Flag
        ScalerSetBit_EXINT(P72_17_HDMI_CTRL_17, ~(_BIT2 | _BIT1 | _BIT0), _BIT0);
    ...
    if(ScalerGetBit_EXINT(P72_23_HDMI_CTRL_23, (_BIT6 | _BIT1)) == (_BIT6 | _BIT1))
    {
        ...
        if(ScalerGetBit_EXINT(P72_23_HDMI_CTRL_23, (_BIT2 | _BIT0)) == 0x00)
        {
            ...
            ScalerTMDSHdmiSetScdcInfo_EXINT0(_D3_INPUT_PORT);

#if(_HDMI_FREESYNC_SUPPORT == _ON)
            if(ScalerTMDSHdmiGetFreeSyncSupport_EXINT0(_D3_INPUT_PORT) == _FREESYNC_SUPPORT_ON)
            {
                ScalerTMDSHdmiSetAmdAupiScdcInfo_EXINT0(_D3_INPUT_PORT, _ENABLE);
            }
            else
            {
                ScalerTMDSHdmiSetAmdAupiScdcInfo_EXINT0(_D3_INPUT_PORT, _DISABLE);
            }
#endif
...
#if((_AUDIO_SUPPORT == _ON) && (_D3_HDMI_SUPPORT == _ON))
    ...
    if(ScalerGetBit_EXINT(P72_CA_HDMI_DP, (_BIT3 | _BIT1)) == (_BIT3 | _BIT1))
    {
        ...
        ScalerSetByte_EXINT(P72_C9_HDMI_AP, _P72_CA_PT_10_PORT_PAGE72_HDMI_CMCR);
        ScalerSetBit_EXINT(P72_CA_HDMI_DP, ~_BIT4, _BIT4);

        // Reload D code
        ScalerSetByte_EXINT(P72_C9_HDMI_AP, _P72_CA_PT_31_PORT_PAGE72_HDMI_WDCR0);
        ScalerSetBit_EXINT(P72_CA_HDMI_DP, ~_BIT6, _BIT6);
        ScalerSetByte_EXINT(P72_C9_HDMI_AP, _P72_CA_PT_31_PORT_PAGE72_HDMI_WDCR0);
        ScalerSetBit_EXINT(P72_CA_HDMI_DP, ~_BIT6, 0x00);
        ...
        {
            SET_AUDIO_D3_FW_TRACKING_RESET();
        }
#endif
        ...
        if(ScalerGetBit_EXINT(P72_CA_HDMI_DP, (_BIT3 | _BIT2 | _BIT1 | _BIT0)) == 0x00)
        {
            ...
            ScalerSetByte_EXINT(P72_C9_HDMI_AP, _P72_CA_PT_12_PORT_PAGE72_HDMI_SCAPR);
            ScalerSetByte_EXINT(P72_CA_HDMI_DP, ScalerAudioGetUserSamplingFreqSCode_EXINT0(_PORT_HDMI, GET_D3_SAMPLING_FREQ_USER_DEFINE()));

            // Set D code User Define
            ScalerSetByte_EXINT(P72_C9_HDMI_AP, _P72_CA_PT_13_PORT_PAGE72_HDMI_DCAPR0);
            ScalerSetByte_EXINT(P72_CA_HDMI_DP, HIBYTE(ScalerAudioGetUserSamplingFreqDCode_EXINT0(_PORT_HDMI, GET_D3_SAMPLING_FREQ_USER_DEFINE())));
            ScalerSetByte_EXINT(P72_C9_HDMI_AP, _P72_CA_PT_14_PORT_PAGE72_HDMI_DCAPR1);
            ScalerSetByte_EXINT(P72_CA_HDMI_DP, LOBYTE(ScalerAudioGetUserSamplingFreqDCode_EXINT0(_PORT_HDMI, GET_D3_SAMPLING_FREQ_USER_DEFINE())));
...
bit ScalerTMDSRx3CheckCDRStable(WORD usClkCount)
{
    ...
    ucStableIrq = ScalerGetByte(P72_EF_HDMI_ERROR_TH);
    ucUnstableIrq = ScalerGetByte(P72_E6_HDMI_FREQDET_CTRL);

    // Disable INT Clock Check
    ScalerSetBit(P72_EF_HDMI_ERROR_TH, ~(_BIT7 | _BIT6 | _BIT5), 0x00);
    ScalerSetBit(P72_E6_HDMI_FREQDET_CTRL, ~(_BIT6 | _BIT2 | _BIT1), 0x00);
        ...
        switch(GET_TMDS_RX3_CLK_MODE())
        {
            ...
            case _TMDS_MHL_PPMODE_PHY_SETTING:
                ...
                if(abs(usClkCount - (ScalerTMDSRx3MeasureClk(_CLK_SOURCE_RED) * 4)) > (usClkCount >> 6))
                {
                    ...
                    ScalerSetBit(P72_EF_HDMI_ERROR_TH, ~(_BIT7 | _BIT6 | _BIT5), (ucStableIrq & _BIT7));
                    ScalerSetBit(P72_E6_HDMI_FREQDET_CTRL, ~(_BIT6 | _BIT2 | _BIT1), (ucUnstableIrq & _BIT1));
...
WORD ScalerTMDSRx3MeasureClk(BYTE ucMeasureClk)
{
    ...
    if(GET_D3_TMDS_MODE() == _TMDS_MHL_MODE)
    {
        ...
        ScalerSetBit(P7C_1A_CMU_00, ~(_BIT7 | _BIT4), (_BIT7 | _BIT4));
    }
    else
#endif
    {
    ...
    ScalerSetByte(P72_C9_HDMI_AP, _P72_CA_PT_28_PORT_PAGE72_HDMI_NTX1024TR0);
    ScalerSetBit(P72_CA_HDMI_DP, ~(_BIT5 | _BIT4 | _BIT3), 0x00);
    ...
    // Start Measure
    ScalerSetByte(P72_C9_HDMI_AP, _P72_CA_PT_28_PORT_PAGE72_HDMI_NTX1024TR0);
    ScalerSetBit(P72_CA_HDMI_DP, ~(_BIT5 | _BIT4 | _BIT3), _BIT3);
    ...
    for(ucDelayCount = 0; ucDelayCount <= 8; ucDelayCount++)
    {
        ...
        ScalerSetByte(P72_C9_HDMI_AP, _P72_CA_PT_28_PORT_PAGE72_HDMI_NTX1024TR0);
        if(ScalerGetBit(P72_CA_HDMI_DP, _BIT3) == 0x00)
        {
            break;
        }
    }
    ...
    ScalerSetByte(P72_C9_HDMI_AP, _P72_CA_PT_28_PORT_PAGE72_HDMI_NTX1024TR0);
    pData[0] = ScalerGetByte(P72_CA_HDMI_DP);

    ScalerSetByte(P72_C9_HDMI_AP, _P72_CA_PT_29_PORT_PAGE72_HDMI_NTX1024TR1);
    pData[1] = ScalerGetByte(P72_CA_HDMI_DP);
...
WORD ScalerTMDSRx3MeasureClk_EXINT0(BYTE ucMeasureClk) using 1
{
    ...
    {
        // Enable Rx_EN (Lane0,3)
        ScalerSetBit_EXINT(P7C_1A_CMU_00, ~(_BIT7 | _BIT4), (_BIT7 | _BIT4));
    }
    else
#endif
    {
    ...
    // Disable Measure for Reset Measure Window
    ScalerSetByte_EXINT(P72_C9_HDMI_AP, _P72_CA_PT_28_PORT_PAGE72_HDMI_NTX1024TR0);
    ScalerSetBit_EXINT(P72_CA_HDMI_DP, ~(_BIT5 | _BIT4 | _BIT3), 0x00);
    ...
    // Start Measure
    ScalerSetByte_EXINT(P72_C9_HDMI_AP, _P72_CA_PT_28_PORT_PAGE72_HDMI_NTX1024TR0);
    ScalerSetBit_EXINT(P72_CA_HDMI_DP, ~(_BIT5 | _BIT4 | _BIT3), _BIT3);
...
bit ScalerTMDSRx3HdmiSetDeepColorPll(WORD usTMDSClkCount, BYTE ucColorDepth)
{
    ...
    if(ScalerGetDataPortBit(P72_C9_HDMI_AP, _P72_CA_PT_28_PORT_PAGE72_HDMI_NTX1024TR0, _BIT6) == _BIT6)
    {
    ...
    ScalerSetBit(P2E_A0_MN_SCLKG_CTRL, ~(_BIT6 | _BIT5 | _BIT4), (_BIT6 | _BIT4));
...
void ScalerTMDSRx3HdmiPacketDetect(bit bWait)
{
    if(bWait == _WAIT)
    {
        ...
        if(ScalerTimerPollingFlagProc(130, P72_CC_HDMI_GPVS, (_BIT6 | _BIT5), _TRUE) == _TRUE)
        {
            ...
            else if(ScalerGetBit(P72_CC_HDMI_GPVS, (_BIT6 | _BIT5)) == _BIT5)
            {
                ...
                if(ScalerTimerPollingFlagProc(180, P72_CC_HDMI_GPVS, _BIT6, _TRUE) == _TRUE)
                {
                    SET_HDMI_RX3_GCP_RECEIVED();
                }
                else
            ...
            else if(ScalerGetBit(P72_CC_HDMI_GPVS, (_BIT6 | _BIT5)) == _BIT6)
            {
                ...
                if(ScalerTimerPollingFlagProc(180, P72_CC_HDMI_GPVS, _BIT5, _TRUE) == _TRUE)
                {
                    SET_HDMI_RX3_AVI_INFO_FRAME_RECEIVED();
                }
                else
...
#if(_HDMI_IDENTIFY_GAME_SRC_SUPPORT == _ON)
        ScalerTMDSRx3SpdInfoPacketDetect(_WAIT);
#endif
        ...
        {
            SET_HDMI_RX3_HDR10_INFO_FRAME_RECEIVED();
        }
        else
        {
            CLR_HDMI_RX3_HDR10_INFO_FRAME_RECEIVED();
        }
#endif
    ...
    else
    {
        if(ScalerGetBit(P72_CC_HDMI_GPVS, _BIT6) == _BIT6)
        {
            ...
            ScalerSetBit(P72_CC_HDMI_GPVS, ~(_BIT6 | _BIT5), _BIT6);
        }
        else
        {
        ...
        if(ScalerGetBit(P72_CC_HDMI_GPVS, _BIT5) == _BIT5)
        {
            ...
            // Clear Detection Flag
            ScalerSetBit(P72_CC_HDMI_GPVS, ~(_BIT6 | _BIT5), _BIT5);
        }
        else
        {
...
#if(_HDMI_IDENTIFY_GAME_SRC_SUPPORT == _ON)
        ScalerTMDSRx3SpdInfoPacketDetect(_NOT_WAIT);
#endif
        ...
        if(ScalerGetDataPortBit(P72_C9_HDMI_AP, _P72_CA_PT_00_PORT_PAGE72_HDMI_SCR, _BIT6) == _BIT6)
        {
            ...
            ScalerSetDataPortBit(P72_C9_HDMI_AP, _P72_CA_PT_00_PORT_PAGE72_HDMI_SCR, ~(_BIT7 | _BIT6), _BIT6);
        }
        else
        {
            CLR_HDMI_RX3_HDR10_INFO_FRAME_RECEIVED();
        ...
        ScalerTimerActiveTimerEvent(130, _SCALER_TIMER_EVENT_RX3_TMDS_HDMI_PACKET_DETECT);
    }
}
...
void ScalerTMDSRx3HdmiAVMuteProc(void)
{
    ...
    if((ScalerGetBit(P72_CB_HDMI_SR, _BIT6) == 0x00) && (ScalerGetBit(P72_A6_TMDS_OUTCTL, (_BIT6 | _BIT5 | _BIT4)) == 0x00))
    {
        ...
        ScalerSetDataPortBit(P72_C9_HDMI_AP, _P72_CA_PT_30_PORT_PAGE72_HDMI_AVMCR, ~(_BIT6 | _BIT5 | _BIT3), (_BIT6 | _BIT5 | _BIT3));
...
void ScalerTMDSRx3FreqDetectDisable(void)
{
    ...
    ScalerSetBit(P72_A7_TMDS_PWDCTL, ~(_BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0), 0x00);

    // Power Down PLL
    ScalerSetBit(P2E_BD_MN_SCLKG_PLL_PWR, ~_BIT7, _BIT7);
    ...
    ScalerSetBit(P72_1A_HDMI_CTRL_1A, ~_BIT3, _BIT3);

    // Set time interval form bus free to Read Request(4.7us~500us)
    ScalerSetBit(P72_1B_HDMI_CTRL_1B, ~(_BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0), (_BIT4 | _BIT0));
}
...
BYTE ScalerTMDSRx3TMDSScanInputPort(BYTE ucInputType)
{
    ...
    if(GET_TMDS_RX3_PHY_SET() == _TRUE)
    {
        ...
        if((ScalerTMDSRx3CheckCDRStable(g_usTMDSRx3ClkCount) == _FALSE) || (ScalerTMDSRx3CheckDataDFE() == _FALSE) ||
           ...
           (ScalerTMDSRx3TMDSVideoDetect(_WAIT) == _TMDS_VIDEO_DATA_NONE))
        ...
        if((ScalerTMDSRx3CheckCDRStable(g_usTMDSRx3ClkCount) == _FALSE) || (ScalerTMDSRx3CheckDataDFE() == _FALSE) || (ScalerTMDSRx3TMDSVideoDetect(_WAIT) == _TMDS_VIDEO_DATA_NONE))
#else
        if((ScalerTMDSRx3CheckCDRStable(g_usTMDSRx3ClkCount) == _FALSE) || (ScalerTMDSRx3CheckDataDFE() == _FALSE))
#endif
...
#endif // End of #if(_D3_INPUT_PORT_TYPE == _D3_MHL_PORT)
        {
            ...
            if((GET_TMDS_RX3_HIGH_SPEED_MODE() == _FALSE) && (GET_D3_TMDS_MODE() != _TMDS_MHL_MODE)) // HDMI mode
            {
                DebugMessageRx3("TMDS Rx3 First 1:10 set phy not success", 1);
...
#if(_D3_HDMI_2_0_SUPPORT == _ON)
        if(ScalerGetBit(P72_38_HDMI_CTRL_38, _BIT4) == 0x00)
        {
            ScalerTimerActiveTimerEvent(SEC(1), _SCALER_TIMER_EVENT_RX3_HDMI2_SCRAMBLE_DETECT_TIMEOUT);
        }
#endif
        ...
        DebugMessageRx3("7. TMDS Lane0 LE Min", g_pucTMDSRx3LEMin[0]);
        DebugMessageRx3("7. TMDS Lane1 LE Min", g_pucTMDSRx3LEMin[1]);
        DebugMessageRx3("7. TMDS Lane2 LE Min", g_pucTMDSRx3LEMin[2]);
        DebugMessageRx3("7. TMDS Lane0 LE Max", g_pucTMDSRx3LEMax[0]);
        DebugMessageRx3("7. TMDS Lane1 LE Max", g_pucTMDSRx3LEMax[1]);
        DebugMessageRx3("7. TMDS Lane2 LE Max", g_pucTMDSRx3LEMax[2]);
        ...
        DebugMessageRx3("HDMI 2.0 Error Boundary set High Byte", ScalerGetByte(P72_14_HDMI_CTRL_14));
        DebugMessageRx3("HDMI 2.0 Error Boundary set Low Byte", ScalerGetByte(P72_15_HDMI_CTRL_15));
#endif
        ...
        if(((GET_MHL_RX3_ADOPTER_ID_HBYTE() == _ASUS_PADFONE2_ADOPTER_ID_HBYTE) && (GET_MHL_RX3_ADOPTER_ID_LBYTE() == _ASUS_PADFONE2_ADOPTER_ID_LBYTE)) &&
           !(((ScalerGetBit(P72_A9_TMDS_ABC0, _BIT7) == _BIT7) || (ScalerGetBit(P72_AE_TMDS_RPS, (_BIT1)) == _BIT1)) && (ScalerGetBit(P72_A9_TMDS_ABC0, _BIT6) == _BIT6)))
        {
            return _SOURCE_NONE;
        }
#endif
        ...
        if(GET_INPUT_PIXEL_CLK() > 3400)
        {
            ...
            ScalerSetBit(P7C_AF_L0_GDI_DE, ~(_BIT1 | _BIT0), _BIT0);
            ScalerSetBit(P7C_BF_L1_GDI_DE, ~(_BIT1 | _BIT0), _BIT0);
            ScalerSetBit(P7C_CF_L2_GDI_DE, ~(_BIT1 | _BIT0), _BIT0);
        }
        else
        {
...
#if(_HDMI_FREESYNC_SUPPORT == _ON)
            if(GET_HDMIRX3_AMD_SPD_INFO(_SPD_INFO_FREESYNC_SUPPORT) == _ENABLE)
            {
                SET_HDMI_FREESYNC_ENABLED();

                // Get SPD Free Sync Vfreq Max
                g_usFREESYNCVFreq_max = (WORD)GET_HDMIRX3_AMD_SPD_INFO(_SPD_INFO_FREESYNC_MAX_VFREQ) * 10;
                g_usFREESYNCVFreq_min = (WORD)GET_HDMIRX3_AMD_SPD_INFO(_SPD_INFO_FREESYNC_MIN_VFREQ) * 10;
...
bit ScalerTMDSRx3CheckDataDFE(void)
{
    ...
    if(ScalerGetBit(P7C_17_BANDGAP_03, _BIT1) != _BIT1)
    {
        for(pData[0] = 0; pData[0] <= 2; pData[0]++)
        {
            // Sel lane
            ScalerSetBit(P7C_F2_COEF_3, ~(_BIT7 | _BIT6), pData[0] << 6);

            // Read VTH
            ScalerSetBit(P7C_F0_COEF_1, ~(_BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0), 0x00);

            if((ScalerGetByte(P7C_F1_COEF_2) & 0x1F) == 0)
            {
                for(ucCnt = 0; ucCnt < 3; ucCnt++)
                {
                    ...
                    {
                        DebugMessageRx3("DFE Abnormal ", 0);
                        return _FALSE;
                    }
                }
            }
...
bit ScalerTMDSRx3TMDSStableDetect(BYTE ucSourceType)
{
    ...
    if((GET_D3_TMDS_MODE() == _TMDS_HDMI_MODE) && (ScalerGetBit(P72_00_HDMI_CTRL_00, _BIT0) == _BIT0))
    {
        if(ucSourceType == _SOURCE_HDMI)
        {
            if(GET_TMDS_RX3_HDMI_2_0_FORMAT_RESET_WAITING() == _FALSE)
            {
                if(ScalerGetDataPortBit(P72_C9_HDMI_AP, _P72_CA_PT_00_PORT_PAGE72_HDMI_SCR, _BIT3) != _BIT3)
                {
                    return _FALSE;
                }
                else
    ...
    if(GET_TMDS_RX3_Z0_ASSERTED() == _TRUE)
    {
        ...
        if(ScalerGetBit(P72_C0_HDCP_CR, _BIT0) == _BIT0)
        {
            if((ScalerGetBit(P72_A9_TMDS_ABC0, _BIT6) == _BIT6) && (!((ScalerGetBit(P72_AE_TMDS_RPS, _BIT1) == _BIT1) && (ScalerGetBit(P72_A9_TMDS_ABC0, _BIT5) == _BIT5))))
            {
            ...
            else if(((ScalerGetBit(P72_A9_TMDS_ABC0, _BIT7) == (_BIT7)) || (ScalerGetBit(P72_AE_TMDS_RPS, _BIT1) == (_BIT1))) && (ScalerGetBit(P72_A9_TMDS_ABC0, _BIT5) == (_BIT5)))
            {
                CLR_TMDS_RX3_Z0_ASSERTED();
...
#else
            if(ScalerTMDSRx3TMDSVideoDetect(_NOT_WAIT) == _TMDS_VIDEO_DATA_NONE)
#endif
            {
    ...
    if(ucSourceType == _SOURCE_HDMI)
    {
        if(GET_HDMI_RX3_PACKET_DETECTING() == _FALSE)
        {
            ...
            {
                if((GET_COLOR_SPACE() != _COLOR_SPACE_YCBCR422) && (ScalerTMDSRx3HdmiGetColorDepth() != GET_COLOR_DEPTH()))
                {
                    DebugMessageRx3("Color Depth Has Changed", GET_COLOR_DEPTH());
                    return _FALSE;
                }
            }
            else
            ...
            {
                if((bit)GET_HDMIRX3_AMD_SPD_INFO(_SPD_INFO_FREESYNC_SUPPORT) != GET_HDMI_FREESYNC_ENABLED())
                {
    ...
    {
        if(GET_D3_HDMI_PS_WAKEUP() == _TRUE)
        {
            CLR_D3_HDMI_PS_WAKEUP();

            SET_TMDS_RX3_HPD_TRIGGER_EVENT(_TMDS_HPD_EIZO_PS_WAKEUP);
...
void ScalerTMDSRx3HotPlugEvent(void)
{
    ...
    {
        ScalerTMDSRx3BeforeHotPlugEventProc(_TRUE);
    }
    else
    {
        ScalerTMDSRx3BeforeHotPlugEventProc(_FALSE);
    }

    // ACER mode and DVI port will only toggle Z0 130ms
#if((_D3_INPUT_PORT_TYPE == _D3_DVI_PORT) && (_TMDS_TOGGLE_HPD_Z0_FOR_ACER == _ON))
...
#endif
    {
        ...
        // wait
        if((GET_TMDS_RX3_HPD_TRIGGER_EVENT() == _TMDS_HPD_NORMAL) || (GET_TMDS_RX3_HPD_TRIGGER_EVENT() == _TMDS_HPD_EIZO_PS_WAKEUP))
        {
            ScalerTMDSRx3HotPlugHighProc(GET_HDMI_HOTPLUG_TIME());
        }
        else if(GET_TMDS_RX3_HPD_TRIGGER_EVENT() == _TMDS_HPD_HDCP_2_2)
        {
            ScalerTMDSRx3HotPlugHighProc(_TMDS_HDCP_2_2_HPD_TIME);
        }
        else if(GET_TMDS_RX3_HPD_TRIGGER_EVENT() == _TMDS_HPD_ACER)
        {
            ScalerTMDSRx3HotPlugHighProc(_TMDS_ACER_HPD_TIME);
        }
        else if(GET_TMDS_RX3_HPD_TRIGGER_EVENT() == _TMDS_HPD_EDID_SWITCH)
        {
            ScalerTMDSRx3HotPlugHighProc(_TMDS_EDID_SWITCH_HPD_TIME);
        }
        else
        {
            ScalerTMDSRx3HotPlugHighProc(GET_HDMI_HOTPLUG_TIME());
    ...
    if(GET_HDMI_Z0_HPD_TYPE() == _HDMI_HPD_Z0_TOGGLE_AT_SAME_TIME)
    {
        ...
        ScalerTMDSRx3AfterHotPlugEventProc(_TRUE);
    }
    else
    {
        // Restore Slave Address
        ScalerTMDSRx3AfterHotPlugEventProc(_FALSE);
    }
...
void ScalerTMDSRx3SwitchHdcpSlaveAddr(void)
{
    ScalerSetDataPortByte(P72_C3_HDCP_AP, _P72_C4_PT_C2_PORT_PAGE72_HDCP_SLAVE_ADD, 0x7F);
}
...
void ScalerTMDSRx3BeforeHotPlugEventProc(bit bDisableZ0)
{
...
#endif
    {
        ...
        ScalerGetDataPortByte(P72_C3_HDCP_AP, _P72_C4_PT_C2_PORT_PAGE72_HDCP_SLAVE_ADD, 1, &g_ucTMDSRx3SlaveAddr, _NON_AUTOINC);
...
#if(_DP_HDMI_COMPATIBLE_MODE == _ON)
        if(GET_TMDS_RX3_HPD_TRIGGER_EVENT() != _TMDS_HPD_EIZO_PS_WAKEUP)
#endif
        {
#if((_TMDS_HDCP_2_2_SUPPORT == _ON) && ((_D3_INPUT_PORT_TYPE == _D3_HDMI_PORT) || (_D3_INPUT_PORT_TYPE == _D3_MHL_PORT)))
            // Reset HDCP 2.2
...
#if((_D12_INPUT_PORT_TYPE == _D12_HDMI_PORT) && (_D12_INPUT_PORT_SWITCH_FROM == _SWITCH_FROM_D3))
    else if(((GET_D3_CABLE_STATUS() == _FALSE) && (PCB_GET_HDMI_SW_SELECT() == _SW_SEL_HDMI3)) ||
...
void ScalerTMDSRx3AfterHotPlugEventProc(bit bRestoreZ0)
{
    ...
    {
        // Restore Slave Address
        ScalerSetDataPortByte(P72_C3_HDCP_AP, _P72_C4_PT_C2_PORT_PAGE72_HDCP_SLAVE_ADD, g_ucTMDSRx3SlaveAddr);
        ...
        if(g_ucTmdsD3DdcramNackStatus == _FALSE)
        {
        ...
        ScalerMcuDdcciEnable(_ENABLE, _D3_INPUT_PORT);
#endif
...
void ScalerTMDSRx3Hdcp1p4AksvClear(void)
{
    ...
    ScalerSetBit(P72_AE_TMDS_RPS, ~_BIT1, 0x00);
}
