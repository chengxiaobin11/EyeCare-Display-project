...
//----------------------------------------------------------------------------------------------------
// ID Code      : RL6492_Series_Mcu.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------

#define __RL6492_SERIES_MCU__
...
#define _EMBEDDED_DDCRAM_DDC2_ADDRESS                       0xE280
...
BYTE xdata MCU_DDCRAM_D2[_D2_EMBEDDED_DDCRAM_MAX_SIZE]          _at_ _EMBEDDED_DDCRAM_DDC2_ADDRESS;
#endif // End of _DDC_CHANNEL_FOR_D2

#endif // End of #if(_D2_EMBEDDED_DDCRAM_MAX_SIZE != _EDID_SIZE_NONE)


//--------------------------------------------------
// EMCU MCU Arbiter (Page FD)
//--------------------------------------------------
volatile BYTE xdata MCU_FD21_FLASH_STATUS_CHECK                     _at_ 0xFD21;
volatile BYTE xdata MCU_FD22_FLASH_CHECK_VALUE_0                    _at_ 0xFD22;
volatile BYTE xdata MCU_FD23_FLASH_CHECK_VALUE_1                    _at_ 0xFD23;
volatile BYTE xdata MCU_FD24_UFP_BANK_START                         _at_ 0xFD24;
volatile BYTE xdata MCU_FD25_BOOTUP_BANK_START                      _at_ 0xFD25;
volatile BYTE xdata MCU_FD26_TYPEC_UFP_CONTROL                      _at_ 0xFD26;
volatile BYTE xdata MCU_FD27_SCALAR_UFP_DFP_COM_CTRL1               _at_ 0xFD27;
volatile BYTE xdata MCU_FD28_SCALAR_UFP_DFP_COM_CTRL2               _at_ 0xFD28;
volatile BYTE xdata MCU_FD29_UFP_DPF_IRQ_STATUS                     _at_ 0xFD29;
volatile BYTE xdata MCU_FD2A_UFP_IRQ_PRIORITY                       _at_ 0xFD2A;
volatile BYTE xdata MCU_FD2B_UFP_INT_PRIORITY                       _at_ 0xFD2B;
volatile BYTE xdata MCU_FD2C_FLASH_GATE_MCU_CTRL                    _at_ 0xFD2C;
...
volatile BYTE xdata MCU_FE49_DUMMY1                                 _at_ 0xFE49;
volatile BYTE xdata MCU_FE4A_DUMMY2                                 _at_ 0xFE4A;
volatile BYTE xdata MCU_FE4B_DUMMY3                                 _at_ 0xFE4B;
...
volatile BYTE xdata MCU_FEB4_XRAM_TYPEC_BIST_CTRL                   _at_ 0xFEB4;
volatile BYTE xdata MCU_FEB5_XRAM_TYPEC_DRF_BIST_CTRL               _at_ 0xFEB5;
volatile BYTE xdata MCU_FEB6_MCU_DUMMY2                             _at_ 0xFEB6;
volatile BYTE xdata MCU_FEB7_CACHE_TYPEC_SRAM_BIST_CTRL             _at_ 0xFEB7;
volatile BYTE xdata MCU_FEB8_CACHE_TYPEC_SRAM_DRF_BIST_CTRL         _at_ 0xFEB8;
...
#if((_D2_INPUT_PORT_TYPE == _D2_DVI_PORT) || (_D2_INPUT_PORT_TYPE == _D2_DUAL_DVI_PORT) || (_D2_INPUT_PORT_TYPE == _D2_HDMI_PORT) || (_D2_INPUT_PORT_TYPE == _D2_MHL_PORT))
    ...
    if(enumMode == _DDCCI_MODE)
    {
        ...
        switch(ucInputPort)
        {
            ...
            case _D2_INPUT_PORT:
                ...
                // DDCCI switch to DDC2 if D2 is assigned to DDC2
                MCU_FF22_IIC_CH_SEL &= ~(_BIT2 | _BIT1 | _BIT0);
    ...
    if(bEnable == _ENABLE)
    {
        switch(MCU_FF22_IIC_CH_SEL)
        {
            case _DDCCI_ADC_DDC:
                ...
                MCU_FE66_DDC2_DDC_CONTROL_2 |= _BIT4;
...
void ScalerMcuGpioInitial(void)
{
    ...
    // HUB Power control : Self-Power->Enable ; Vbus-Power->Disable
    PCB_HUB_SPWR_POWER(_SPWR_POWER_ON);
    PCB_HUB_VPWR_POWER(_VBUS_POWER_OFF);

    // PMIC Power control : Power Disable
    PCB_PMIC_POWER(_POWER_OFF);

    // PMIC Capacitance control : Capacitance Disconnect
    PCB_PMIC_VBUS_SOURCE_CONTROL(_VBUS_SOURCE_SWITCH_OFF);

    // 5V External for VBUS ; 5V External Enable
    PCB_5V_VBUSEXTERNAL_CONNECT(_5V_VBUSEXTERNAL_DISCONNECT);
...
void ScalerMcuDdcInitial(void)
{
    ...
    // A0 DDC force Nack when not embedded EDID
    ...
    // D2 DDC force Nack when not embedded EDID
...
void ScalerMcuDdcISPChannelCtrl(BYTE ucDdcChannel, bit bEnable)
{
    switch(ucDdcChannel)
    {
        ...
        case _DDC2:
            // DDC2 ISP
            MCU_FE66_DDC2_DDC_CONTROL_2 = (bEnable) ? (MCU_FE66_DDC2_DDC_CONTROL_2 | _BIT3) : (MCU_FE66_DDC2_DDC_CONTROL_2 & (~_BIT3));
            break;
