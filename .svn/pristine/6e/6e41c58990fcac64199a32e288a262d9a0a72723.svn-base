...
void ScalerIDomainVgipSetInputCapture(void)
{
    ...
    else
    {
        ...
        {
            SET_VGIP_INITIAL_VDELAY(GET_VGIP_INITIAL_VDELAY() + (g_stMDomainInputData.usVStart - _VGIP_IV_CAPTURE_MAX_VALUE));
            g_stMDomainInputData.usVStart = _VGIP_IV_CAPTURE_MAX_VALUE;
        }

        // Check if V capture delay exceeds maximum value
        if(GET_VGIP_INITIAL_VDELAY() > _HW_VGIP_V_DELAY_BIT_WIDTH)
        {
            DebugMessageScaler("[Warning] VGIP V Capture Delay exceeds the HW Limit!!! Please define (_VGIP_IV_CAPTURE_MAX_VALUE_ENLARGE == _ON).", GET_VGIP_INITIAL_VDELAY());
        }
...
void ScalerIDomainVgipAdjustIHVDelay(void)
{
...
#endif
    {
        ...
        if(ScalerFmtCnvFtoPGetStatus() == _TRUE)
        {
            if(g_stMDomainInputData.usVStart > usIV2DVdelay)
            {
                ...
                if(g_stMDomainInputData.usVStart <= (_VGIP_IV_CAPTURE_MIN_VALUE + _VGIP_IV_CAPTURE_MIN_VALUE % 2))
                {
                    ...
                    usIVSdelayExtend = (_VGIP_IV_CAPTURE_MIN_VALUE + _VGIP_IV_CAPTURE_MIN_VALUE % 2) - g_stMDomainInputData.usVStart;
                    g_stMDomainInputData.usVStart = _VGIP_IV_CAPTURE_MIN_VALUE + _VGIP_IV_CAPTURE_MIN_VALUE % 2;
                }
                else
                {
                    usIVSdelayExtend = g_stMDomainInputData.usVStart % 2;
...
#endif
        {
            if(g_stMDomainInputData.usVStart > usIV2DVdelay)
            {
                ...
                if(g_stMDomainInputData.usVStart <= _VGIP_IV_CAPTURE_MIN_VALUE)
                {
                    ...
                    // Get Difference of Remain IVSt and _VGIP_IV_CAPTURE_MIN_VALUE
                    usIVSdelayExtend = _VGIP_IV_CAPTURE_MIN_VALUE - g_stMDomainInputData.usVStart;
                    g_stMDomainInputData.usVStart = _VGIP_IV_CAPTURE_MIN_VALUE;
                }
                else
                {
        ...
        // Check if V capture delay exceeds maximum value
        if((usIV2DVdelay - usIVSdelayExtend + GET_VGIP_INITIAL_VDELAY()) > _HW_VGIP_V_DELAY_BIT_WIDTH)
        {
            // Modify IVSDelay = IVSDelay - IVSDelayExtend
            usIV2DVdelay -= usIVSdelayExtend;

            usIVSdelayExtend = (usIV2DVdelay + GET_VGIP_INITIAL_VDELAY()) - _HW_VGIP_V_DELAY_BIT_WIDTH;
...
#endif
            g_stMDomainInputData.usVStart = g_stMDomainInputData.usVStart + usIVSdelayExtend;
        }
        ...
        PDATA_DWORD(0) = ScalerMemoryGetIVS2DVSDelayPixel() - (DWORD)(usIV2DVdelay - usIVSdelayExtend) * g_stMDomainInputData.usHTotal;
