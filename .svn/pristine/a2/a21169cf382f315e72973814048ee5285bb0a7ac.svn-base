...
//----------------------------------------------------------------------------------------------------
// ID Code      : RL6851_Series_ScalerDPRx0.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------
...
void ScalerDpRx0SetDpcdWrite1ClearValue(BYTE ucDpcdAccessPortH, BYTE ucDpcdAccessPortM, BYTE ucDpcdAccessPortL, BYTE ucDpcdValue);
void ScalerDpRx0DpcdGroupReset(BYTE ucDpcdAccessPortH, BYTE ucDpcdAccessPortM, BYTE ucDpcdAccessPortL, WORD usLen);
void ScalerDpRx0DpcdLinkConfigReset(void);
void ScalerDpRx0DpcdSourceOUIReset(void);
...
void ScalerDpRx0SetDrrMsaForLut(StructTimingInfo *pstDpTimingInfo);
#endif
...
{
    BYTE ucAutoIncBackUp = ScalerGetByte_EXINT(PB7_C0_DPCD_CTRL);

    // Disable Wildcard IRQ for Error Counter
    ScalerSetBit_EXINT(PB7_FB_DP0_MANUAL_DPCD_IRQ, ~(_BIT7 | _BIT5 | _BIT3 | _BIT1 | _BIT0), 0x00);
...
void ScalerDpRx0ErrorCounterEnable_EXINT0(EnumDpLinkTrainingType enumDpLtType) using 1
{
    ...
    ScalerSetBit_EXINT(PB7_FB_DP0_MANUAL_DPCD_IRQ, ~(_BIT7 | _BIT5 | _BIT3 | _BIT1 | _BIT0), (_BIT1 | _BIT0));
}
...
void ScalerDpRx0AVMute(void)
{
    ...
    // Disable BS Detect Tracking IRQ
...
void ScalerDpRx0HSActiveTrackingMode(EnumDpHSTrackingType enumHSTrackingType)
{
    if(enumHSTrackingType == _DP_HS_TRACKING_FW_MODE)
    {
        ...
        ScalerSetBit(PB5_EA_HS_TRACKING_NEW_MODE1, ~_BIT5, _BIT5);
    ...
    {
        // HS Tracking Region By FW Setting
        ScalerSetBit(PB5_EA_HS_TRACKING_NEW_MODE1, ~_BIT5, 0x00);
    }
}
#endif
//--------------------------------------------------
...
void ScalerDpRx0DpcdGroupReset(BYTE ucDpcdAccessPortH, BYTE ucDpcdAccessPortM, BYTE ucDpcdAccessPortL, WORD usLen)
{
    ...
    ucAuxModeBackUp = ScalerGetByte(PB7_D0_AUX_MODE_SET);
    ucAutoIncBackUp = ScalerGetByte(PB7_C0_DPCD_CTRL);
    ...
    {
        // Enable Dpcd Auto Inc
        ScalerSetBit(PB7_C0_DPCD_CTRL, ~_BIT0, _BIT0);
    }

    while(usLen > 0)
    {
        // Reset Dpcd
        ScalerSetByte(PB7_C4_DPCD_DATA_PORT, 0x00);
    ...
    {
        // Disable Dpcd Auto Inc
        ScalerSetBit(PB7_C0_DPCD_CTRL, ~_BIT0, 0x00);
    }
...
void ScalerDpRx0DpcdLinkConfigReset(void)
{
    ...
    ScalerDpRx0DpcdGroupReset(0x00, 0x01, 0x00, 7);

    // Link Quality Reset 10B~10E
    ScalerDpRx0DpcdGroupReset(0x00, 0x01, 0x0B, 4);
}
...
{
    // Source OUI Reset
    ScalerDpRx0DpcdGroupReset(0x00, 0x03, 0x00, 12);
}
...
void ScalerDpRx0ChangeHdcpDpcdCapability(bit bEn)
{
    ...
    else
    {
        ...
        ScalerSetBit(PB7_A5_AUX_HDCP_IRQ2, ~_BIT6, 0x00);
...
void ScalerDpRx0SetDrrMsaForLut(StructTimingInfo *pstDpTimingInfo)
{
    WORD usTempMeasureHtotal = pstDpTimingInfo->usHTotal;
    ...
    if((usTempMsaHtotal > (usTempMeasureHtotal + usTempMsaHtotalMargin)) || (usTempMsaHtotal < (usTempMeasureHtotal - usTempMsaHtotalMargin)))
    {
        ...
        DebugMessageRx0("7. DP MAC RX0: Freesync Get MSA Htotal Fail  ", usTempMsaHtotal);
    }
    else
    {
        ...
        pstDpTimingInfo->usHTotal = usTempMsaHtotal;
    }
}
#endif
