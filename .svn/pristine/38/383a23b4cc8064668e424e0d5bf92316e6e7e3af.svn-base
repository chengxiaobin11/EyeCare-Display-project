...
//----------------------------------------------------------------------------------------------------
// ID Code      : RL6851_Series_ScalerCommonFreeSync.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------
...
EnumGetIVFFromTableSearch ScalerFreeSyncCalculateIVFmaxFromCVT(WORD *pusVfreqMax, const StructFreeSyncSourceInfo *pstSourceInfo, BYTE ucPortType);

#if(_FREESYNC_SUPPORT == _ON)
bit ScalerFreeSyncVfreqMaxInfoEnable(EnumSourceSearchPort enumInputPort, BYTE ucPortType);
DWORD ScalerFreeSyncGetFreeSyncInfo(EnumSourceSearchPort enumSearchPort, EnumAmdSpdInfoContent enumContent);
void ScalerFreeSyncSPDInfoReceive(EnumSourceSearchPort enumSearchPort);
#if(_TRANSLATOR_SUPPORT == _OFF)
void ScalerFreeSyncSupportSwitch(EnumSourceSearchPort enumSourcePort, EnumFreeSyncSupport enumSupport, EnumSourceSearchPort enumFreesyncTargetPort);
#endif
...
#if(_D0_DP_EXIST == _ON)
DWORD ScalerFreeSyncDpRx0GetAmdSpdInfo(EnumAmdSpdInfoContent enumContent);
...
#if(_D1_DP_EXIST == _ON)
DWORD ScalerFreeSyncDpRx1GetAmdSpdInfo(EnumAmdSpdInfoContent enumContent);
...
DWORD ScalerFreeSyncHDMIRx0GetAmdSpdInfo(EnumAmdSpdInfoContent enumContent);
#endif

#if(_TRANSLATOR_SUPPORT == _OFF)
void ScalerFreeSyncHDMISupportSwitch(EnumSourceSearchPort enumSourcePort, EnumFreeSyncSupport enumSupport, EnumSourceSearchPort enumFreesyncTargetPort);
#endif
...
bit ScalerFreeSyncEdidAnalysisCheckBoundry(const StructFreeSyncEdidInfo *pstEdidInfo, const StructFreeSyncEdidRefInfo *pstEdidRefInfo)
{
    ...
    if((pstEdidInfo->b1Interlace == pstEdidRefInfo->b1RefInterlace) &&
       ...
       (pstEdidInfo->usVheight == pstEdidRefInfo->usRefVheight) &&
...
void ScalerFreeSyncEdidAnalysisGetRefInfo(StructFreeSyncEdidRefInfo *pstEdidRefInfo, WORD usPixelClk, const StructFreeSyncSourceInfo *pstSourceInfo)
{
    ...
    pstEdidRefInfo->b1RefInterlace = GET_INPUT_TIMING_INTERLACE_FLG();
...
EnumGetIVFFromTableSearch ScalerFreeSyncCalculateIVFmaxFromCVT(WORD *pusVfreqMax, const StructFreeSyncSourceInfo *pstSourceInfo, BYTE ucPortType)
{
    ...
    if(ScalerDrrGetType() == _DRR_HDMI21_VRR_TYPE)
    {
        ...
        WORD usHtotalRB1 = (GET_INPUT_TIMING_HWIDTH() + 80);
        WORD usHtotalRB2 = (GET_INPUT_TIMING_HWIDTH() + 160);
        ...
        switch(ucPortType)
        {
            ...
            case _PORT_HDMI20:
                   ...
                   ((usHtotalRB2 <= usRefHtotalHBound) && (usHtotalRB2 >= usRefHtotalLBound)))
                {
                    ...
                    // Calculate IVF (0.1Hz)
                    DWORD ulVtotalCalByPixel = ((DWORD)pstSourceInfo->usHtotal * GET_INPUT_TIMING_VHEIGHT()) + ((DWORD)usPixelClk * _GET_CVT_MIN_VBLANK_PERIOD / 10);
                    WORD usVfreqMax = ((GET_DWORD_MUL_DIV(usPixelClk, 10000000, ulVtotalCalByPixel) + 5) / 10);

                    // Double Check IVF
                    WORD usIVfreqApproximate = ((DWORD)GET_MDOMAIN_INPUT_HFREQ() * 1000 / GET_INPUT_TIMING_VHEIGHT());
...
#elif((_D0_HDMI_SUPPORT == _ON) && (_HDMI_FREESYNC_SUPPORT == _ON))
            ScalerFreeSyncHDMISupportSwitch(enumSourcePort, enumSupport, enumFreesyncTargetPort);
#endif
...
DWORD ScalerFreeSyncDpRx0GetAmdSpdInfo(EnumAmdSpdInfoContent enumContent)
{
    ...
    if(ScalerSyncGetAmdVsdbVer(GET_DP_EMBEDDED_SWITCH_SELECT()) < _AMD_VER3)
    {
    ...
    if(ScalerGetByte(PB6_E0_DP_SDP_RSV0_MAT_HB1) == _SPD_INFO_TYPE)
    {
        bCheckFreeSync = _TRUE;
    }
#endif
    ...
    if(bCheckFreeSync == _TRUE)
    {
        ...
        {
            return (TO_DWORD(0, pucDPRSV0Data[3], pucDPRSV0Data[2], pucDPRSV0Data[1]));
        }
        ...
        if((pucDPRSV0Data[1] == 0x1A) && (pucDPRSV0Data[2] == 0x00) && (pucDPRSV0Data[3] == 0x00))
        {
            switch(enumContent)
            {
                ...
                case _SPD_INFO_PQ_EOTF_ACTIVE:
                    return (((pucDPRSV0Data[9] & _BIT5) != 0x00) ? _ON : _OFF);
                    break;
...
DWORD ScalerFreeSyncDpRx1GetAmdSpdInfo(EnumAmdSpdInfoContent enumContent)
{
    ...
    if(ScalerSyncGetAmdVsdbVer(_D1_INPUT_PORT) < _AMD_VER3)
    {
        usAmdVfreqEffectiveBit = _8_BIT;
    }
    else
...
DWORD ScalerFreeSyncHDMIRx0GetAmdSpdInfo(EnumAmdSpdInfoContent enumContent)
{
    ...
    if(((CHECK_TMDS_DPHY_SWITCH(_D0_INPUT_PORT) == _TRUE) && (ScalerSyncGetAmdVsdbVer(_D0_INPUT_PORT) < _AMD_VER3)) || ((CHECK_TMDS_DPHY_SWITCH(_D1_INPUT_PORT) == _TRUE) && (ScalerSyncGetAmdVsdbVer(_D1_INPUT_PORT) < _AMD_VER3)))
    {
    ...
    if(GET_TMDS_RX0_SPD_INFO_FRAME_RECEIVED() == _TRUE)
    {
        ...
        if(enumContent == _SPD_INFO_OUI)
        {
            return (TO_DWORD(0, pData[2], pData[1], pData[0]));
        }
