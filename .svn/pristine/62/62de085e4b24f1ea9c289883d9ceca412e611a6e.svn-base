...
//----------------------------------------------------------------------------------------------------
// ID Code      : RL6851_Series_ScalerCommonSync.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------
...
// CODE TABLES
//****************************************************************************
#if(_MCRC_STABILITY_CHECK_SUPPORT == _ON)
BYTE code tByPassTable[] =
{
    // Double Buffer //

    // I-dithering //
    4, _NON_AUTOINC, 0x9F, 0x00,
    4, _NON_AUTOINC, 0x89, 0x00, // disable i dither

    // D-dithering //
    4, _NON_AUTOINC, 0x9F, 0x00,
    4, _NON_AUTOINC, 0x6A, 0x00, // disable d dither

    //---------ICRC_M1-------------
    4, _NON_AUTOINC, 0x9F, 0x36,
    4, _NON_AUTOINC, 0x00, 0x08,

    //---------MCRC_Disp_M1-------------
    4, _NON_AUTOINC, 0x9F, 0x36,
    4, _NON_AUTOINC, 0x40, 0x08,

    //---------DCRC_M1-------------
    4, _NON_AUTOINC, 0x9F, 0x36,
    4, _NON_AUTOINC, 0x2C, 0x08,
};
#endif
...
#if(_DP_SUPPORT == _ON)
WORD g_usInputDpNativePixelClk;
#endif
...
StructDpCableStatus g_stDpCableStatus;
...
bit g_bDpIllegalIdlePatternCheck = _FALSE;
...
volatile bit g_bDpD0HpdIrqAssertT2Event = _FALSE;
#endif
#if(_D1_INPUT_PORT_TYPE == _D1_DP_PORT)
volatile bit g_bDpD1HpdIrqAssertT2Event = _FALSE;
#endif
#if(_D2_INPUT_PORT_TYPE == _D2_DP_PORT)
volatile bit g_bDpD2HpdIrqAssertT2Event = _FALSE;
#endif
...
#if(_D0_INPUT_PORT_TYPE == _D0_DP_PORT)
bit g_bDpD0LongHpdAllowed = _TRUE;
#endif
#if(_D1_INPUT_PORT_TYPE == _D1_DP_PORT)
bit g_bDpD1LongHpdAllowed = _TRUE;
#endif
#if(_D2_INPUT_PORT_TYPE == _D2_DP_PORT)
bit g_bDpD2LongHpdAllowed = _TRUE;
#endif
#endif
...
StructDPADAPTIVESYNCENABLE g_stDpADAPTIVESYNCInfo;
#endif
...
bit g_bFREESYNCGetIVFInfo;
#endif

bit g_bDigitalColorInfoChangeReset;

#if(_D0_TMDS_SUPPORT == _ON)
BYTE g_ucTmdsD0DdcramNackStatus;
#endif

#if(_D1_TMDS_SUPPORT == _ON)
BYTE g_ucTmdsD1DdcramNackStatus;
#endif

#if(_D2_TMDS_SUPPORT == _ON)
BYTE g_ucTmdsD2DdcramNackStatus;
#endif
...
StructHdmiRxPsWakeupHpd g_stHdmiPsWakeupHPD;
#endif
...
#if(_SOURCE_BACKGROUND_DETECT_SUPPORT == _ON)
BYTE g_ucSourceDetectingPort = _NO_INPUT_PORT;
#endif
...
void ScalerSyncSetAmdVsdbVer(BYTE ucInputPort, EnumAmdVsdbVer enumAmdVer);
EnumAmdVsdbVer ScalerSyncGetAmdVsdbVer(BYTE ucInputPort);
...
bit ScalerSyncDpGetHotPlugStatus(EnumSourceSearchPort enumInputPort);
void ScalerSyncDpRxSetHotPlug(EnumSourceSearchPort enumInputPort, bit bHighLow, WORD usLongHpdDuration);
bit ScalerSyncDpRxGetHotPlugPinDetect(EnumSourceSearchPort enumInputPort);

#if(_AUDIO_SUPPORT == _ON)
bit ScalerSyncDpRxAudioCapabilitySwitchProc(BYTE ucInputPort);
#endif
...
#if(_DP_ADAPTIVESYNC_SUPPORT == _ON)
bit ScalerSyncDpAdaptivesyncCapabilitySwitchProc(BYTE ucInputPort);
#endif

void ScalerSyncDpFakeLTProtect(void);
bit ScalerSyncDpCableStatus(EnumSourceSearchPort enumInputPort);
void ScalerSyncDpSetCableStatus(EnumSourceSearchPort enumInputPort, bit bStatus);
void ScalerSyncDpCheckHdcpEnable(void);
bit ScalerSyncDpGetLinkTrainingPass(EnumSourceSearchPort enumInputPort);
BYTE ScalerSyncDpRxGetConnectorType(EnumSourceSearchPort enumInputPort);

#if(_DP_IRQ_HPD_MODE == _DP_IRQ_HPD_T2_MODE)
void ScalerSyncDpHpdIrqAssertTimer2EventProc_EXINT(void);

#if(_SYSTEM_TIMER_QUARTER_RESOLUTION == _OFF)
void ScalerSyncDpAuxManualModeSetting_EXINT(EnumDpAuxManualModeSetting enumSetting);
#endif
#endif

#if(_TWO_CHIP_DATA_EXCHANGE_MODE != _DATA_EXCHANGE_MODE_NONE)
void ScalerSyncDpRepeaterPowerOffReset(void);
#endif

#if(_DP_TX_SUPPORT == _ON)
void ScalerSyncDpRxSinkCountUpdateProc(EnumSourceSearchPort enumInputPort);
#endif

#if(_DP_LONG_HPD_ALLOWED_BY_USER_SUPPORT == _ON)
void ScalerSyncDpLongHpdSetAllowed(EnumSourceSearchPort enumInputPort, bit bAllowed);
bit ScalerSyncDpLongHpdGetAllowed(EnumSourceSearchPort enumInputPort);
#endif

#if((_PORT_CONTROLLER_RTS5400_SERIES_SUPPORT == _ON) || (_PORT_CONTROLLER_USERDECITION_SUPPORT == _ON))
void ScalerSyncDpRxSetLaneMapping(EnumSourceSearchPort enumInputPort, EnumTypeCPinCfgType enumPinAssignment, EnumTypeCOrientation enumOrientation);
#endif
...
void ScalerSyncDpHdcpRepeaterCapableProc(void);
#endif
...
void ScalerSyncDpSourceWritePowerUpProc(void);
void ScalerSyncDpRxClrSourceWritePowerUp(EnumSourceSearchPort enumInputPort);
#endif
...
void ScalerSyncHdcp14KeySramGatedClk(bit bEnable);
...
bit ScalerSyncGetUrgentDpRxLinkLossInfo(void);
#endif
...
void ScalerSyncTMDSSetHdmiZ0HpdType(EnumHdmiZ0HpdType enumHdmiZ0HpdType, WORD usZ0LowPeriod, WORD usZ0HpdInterval);

#if((_HDMI_MULTI_EDID_SUPPORT == _ON) && (_TMDS_FS_ON_ONLY_TARGETPORT_HPD_SUPPORT == _ON) && (_HDMI_FREESYNC_SUPPORT == _ON))
void ScalerSyncTMDSClrHPDTriggerEvent(EnumSourceSearchPort enumInputPort);
#endif
#endif
...
#if(_MCRC_STABILITY_CHECK_SUPPORT == _ON)
void ScalerSyncColorPatternGenIDomainControl(bit bEnable);
void ScalerSyncCheckMCRC(void);
#endif
...
#if(_SOURCE_BACKGROUND_DETECT_SUPPORT == _ON)
void ScalerSyncStoreActivedPort(BYTE ucInputPort);
void ScalerSyncClearActivedPort(void);
#endif
...
#if(_DVI_SUPPORT == _ON)
bit ScalerSyncTmdsGetDigitalPort(BYTE ucInputPort);
#endif
...
void ScalerSyncDpCapabilitySwitchProc(void)
{
    ...
    if(GET_DP_RX_CAPABILITY_SWITCHED() == _TRUE)
    {
        ...
        {
#if(_AUDIO_SUPPORT == _ON)
            if(ScalerSyncDpRxAudioCapabilitySwitchProc(_D0_INPUT_PORT) == _TRUE)
            {
...
#if(_DP_ADAPTIVESYNC_SUPPORT == _ON)
#if(_HW_DIGITAL_PORT_COMBO_PHY_TYPE == _DIGITAL_PORT_COMBO_PHY_ALL)
            if(ScalerSyncDpAdaptivesyncCapabilitySwitchProc(GET_DP_EMBEDDED_SWITCH_SELECT()) == _TRUE)
            {
                bDPRx0DoLongHotPlug = _TRUE;
            }
#else
            if(ScalerSyncDpAdaptivesyncCapabilitySwitchProc(_D0_INPUT_PORT) == _TRUE)
            {
                bDPRx0DoLongHotPlug = _TRUE;
            }
#endif
...
#if(_DP_ADAPTIVESYNC_SUPPORT == _ON)
            if(ScalerSyncDpAdaptivesyncCapabilitySwitchProc(_D6_INPUT_PORT) == _TRUE)
            {
                bDPRx0DoLongHotPlug = _TRUE;
            }
#endif
        ...
        if(ScalerDpTxBlockStatusCheck(_D0_INPUT_PORT) == _TRUE)
        {
            bDPRx0DoLongHotPlug = _TRUE;
        }
#endif

        if((bDPRx0DoLongHotPlug == _TRUE) && (ScalerSyncDpGetHotPlugStatus(GET_DP_EMBEDDED_SWITCH_SELECT()) == _HIGH))
        {
#if(_DP_MST_SUPPORT == _ON)
...
#if(_AUDIO_SUPPORT == _ON)
        if(ScalerSyncDpRxAudioCapabilitySwitchProc(_D1_INPUT_PORT) == _TRUE)
        {
...
#else
        if(ScalerSyncDpAdaptivesyncCapabilitySwitchProc(_D1_INPUT_PORT) == _TRUE)
        {
            bDPRx1DoLongHotPlug = _TRUE;
        }
#endif
...
#if((_DP_TX_SUPPORT == _ON) && (_DP_TX_BLOCK_CONTROL_BEFORE_STREAM_OUTPUT == _ON))
        if(ScalerDpTxBlockStatusCheck(_D1_INPUT_PORT) == _TRUE)
        {
            bDPRx1DoLongHotPlug = _TRUE;
        }
#endif

        if((bDPRx1DoLongHotPlug == _TRUE) && (ScalerSyncDpGetHotPlugStatus(GET_DP_EMBEDDED_SWITCH_SELECT()) == _HIGH))
        {
#if(_DP_MST_SUPPORT == _ON)
...
#endif
        {
            ...
            {
                pData[0] = (GET_DP_TX_DOWNSTREAM_UP_REQUEST_CAP_STATUS() << 1);

                SET_DP_TX_DPCD_111(pData);

#if((_DP_FREESYNC_SUPPORT == _ON) || (_DP_ADAPTIVESYNC_SUPPORT == _ON))
                // Clear Freesync & AdaptiveSync enable bit
...
bit ScalerSyncDpVersionChange(EnumSourceSearchPort enumSourceSearchPort, EnumDpVersionType enumDpVersion)
{
    switch(enumSourceSearchPort)
    {
        ...
        case _D6_INPUT_PORT:
            ...
            {
#if((_EIZO_CUSTOMIZED_VALUE_OF_DPCD_SUPPORT == _ON) && (_D0_DP_CONNECTOR_TYPE == _DP_CONNECTOR_MINI))
                // For Eizo Type-C case, DP version is always set to DP1.2, so check the 00080h value instead of DP version
                if(ScalerDpRx0GetDpcdInfo(0x00, 0x00, 0x80) == 0x00)
                {
                    if(enumDpVersion > _DP_VERSION_1_1)
                    {
                        return _TRUE;
                    }
                }
        ...
        case _D1_INPUT_PORT:
            ...
            {
#if((_EIZO_CUSTOMIZED_VALUE_OF_DPCD_SUPPORT == _ON) && (_D1_DP_CONNECTOR_TYPE == _DP_CONNECTOR_MINI))
                // For Eizo Type-C case, DP version is always set to DP1.2, so check the 00080h value instead of DP version
                if(ScalerDpRx1GetDpcdInfo(0x00, 0x00, 0x80) == 0x00)
                {
                    if(enumDpVersion > _DP_VERSION_1_1)
                    {
                        return _TRUE;
...
bit ScalerSyncDpGetHotPlugStatus(EnumSourceSearchPort enumInputPort)
{
    switch(enumInputPort)
    {
        ...
        case _D0_INPUT_PORT:
            ...
            return GET_DP_RX_D0_HPD_STATUS();
#endif

#if(_D1_INPUT_PORT_TYPE == _D1_DP_PORT)
        case _D1_INPUT_PORT:

            return GET_DP_RX_D1_HPD_STATUS();
#endif
...
void ScalerSyncDpRxSetHotPlug(EnumSourceSearchPort enumInputPort, bit bHighLow, WORD usLongHpdDuration)
{
    ...
    switch(enumInputPort)
    {
        ...
        case _D0_INPUT_PORT:
            ...
            SET_DP_RX_D0_HPD_STATUS(bHighLow);
        ...
        case _D1_INPUT_PORT:
            ...
            SET_DP_RX_D1_HPD_STATUS(bHighLow);
...
#if(_HW_DP_DPCD_EXT_CAP_PHYSICAL_ADDRESS_SUPPORT == _ON)
                    ScalerDpRx0SetDpcdValue(0x00, 0x22, 0x04, 0x01);

                    ScalerDpRx0SetDpcdValue(0x00, 0x22, 0x0A, 0x06);
#endif
                }
                else
                {
...
#if(_HW_DP_DPCD_EXT_CAP_PHYSICAL_ADDRESS_SUPPORT == _ON)
                    ScalerDpRx0SetDpcdValue(0x00, 0x22, 0x04, 0x00);

                    ScalerDpRx0SetDpcdValue(0x00, 0x22, 0x0A, 0x00);
#endif
                }
...
#if(_HW_DP_DPCD_EXT_CAP_PHYSICAL_ADDRESS_SUPPORT == _ON)
                    ScalerDpRx1SetDpcdValue(0x00, 0x22, 0x04, 0x01);

                    ScalerDpRx1SetDpcdValue(0x00, 0x22, 0x0A, 0x06);
#endif
#endif
                }
                else
...
#if(_HW_DP_DPCD_EXT_CAP_PHYSICAL_ADDRESS_SUPPORT == _ON)
                    ScalerDpRx1SetDpcdValue(0x00, 0x22, 0x04, 0x00);

                    ScalerDpRx1SetDpcdValue(0x00, 0x22, 0x0A, 0x00);
#endif
#endif
                }
...
bit ScalerSyncDpAdaptivesyncCapabilitySwitchProc(BYTE ucInputPort)
{
    switch(ucInputPort)
    {
        ...
        case _D0_INPUT_PORT:
            ...
            if(GET_DP_RX0_ADAPTIVESYNC_CAPABILITY_SWITCH() == _TRUE)
            {
                CLR_DP_RX0_ADAPTIVESYNC_CAPABILITY_SWITCH();

                if(GET_DP_RX0_ADAPTIVESYNC_SUPPORT() == _DP_ADAPTIVESYNC_SUPPORT_ON)
                {
                    ScalerDpRx0SetDpcdBitValue(0x00, 0x00, 0x07, ~_BIT6, _BIT6);
                    ...
                    ScalerDpRx0SetDpcdBitValue(0x00, 0x22, 0x07, ~_BIT6, _BIT6);
#endif
                    g_ucDpRx0DPCD02214hValue = _BIT0;
                }
                else
                {
                    ScalerDpRx0SetDpcdBitValue(0x00, 0x00, 0x07, ~_BIT6, 0x00);
        ...
        case _D1_INPUT_PORT:
            ...
            if(GET_DP_RX1_ADAPTIVESYNC_CAPABILITY_SWITCH() == _TRUE)
            {
                CLR_DP_RX1_ADAPTIVESYNC_CAPABILITY_SWITCH();

                if(GET_DP_RX1_ADAPTIVESYNC_SUPPORT() == _DP_ADAPTIVESYNC_SUPPORT_ON)
                {
#if(_HW_DIGITAL_PORT_COMBO_PHY_TYPE == _DIGITAL_PORT_COMBO_PHY_ALL)
                    ScalerDpRx0SetDpcdBitValue(0x00, 0x00, 0x07, ~_BIT6, _BIT6);
                    ...
                    ScalerDpRx1SetDpcdBitValue(0x00, 0x22, 0x07, ~_BIT6, _BIT6);
#endif
                    g_ucDpRx1DPCD02214hValue = _BIT0;
#endif
                }
                else
                {
        ...
        case _D2_INPUT_PORT:
            ...
            if(GET_DP_RX2_ADAPTIVESYNC_CAPABILITY_SWITCH() == _TRUE)
            {
                CLR_DP_RX2_ADAPTIVESYNC_CAPABILITY_SWITCH();

                if(GET_DP_RX2_ADAPTIVESYNC_SUPPORT() == _DP_ADAPTIVESYNC_SUPPORT_ON)
                {
#if(_HW_DIGITAL_PORT_COMBO_PHY_TYPE == _DIGITAL_PORT_COMBO_PHY_ALL)
                    ScalerDpRx0SetDpcdBitValue(0x00, 0x00, 0x07, ~_BIT6, _BIT6);
        ...
        case _D6_INPUT_PORT:
            ...
            if(GET_DP_RX6_ADAPTIVESYNC_CAPABILITY_SWITCH() == _TRUE)
            {
                CLR_DP_RX6_ADAPTIVESYNC_CAPABILITY_SWITCH();

                if(GET_DP_RX6_ADAPTIVESYNC_SUPPORT() == _DP_ADAPTIVESYNC_SUPPORT_ON)
                {
                    ScalerDpRx0SetDpcdBitValue(0x00, 0x00, 0x07, ~_BIT6, _BIT6);
...
bit ScalerSyncDpCableStatus(EnumSourceSearchPort enumInputPort)
{
    switch(enumInputPort)
    {
        ...
        case _D0_INPUT_PORT:
            ...
            return g_stDpCableStatus.b1D0Status;
#endif

#if(_D1_INPUT_PORT_TYPE == _D1_DP_PORT)
        case _D1_INPUT_PORT:

            return g_stDpCableStatus.b1D1Status;
#endif

#if(_D2_INPUT_PORT_TYPE == _D2_DP_PORT)
        case _D2_INPUT_PORT:

            return g_stDpCableStatus.b1D2Status;
#endif

#if(_D6_INPUT_PORT_TYPE == _D6_DP_PORT)
        case _D6_INPUT_PORT:

            return g_stDpCableStatus.b1D6Status;
#endif
...
void ScalerSyncDpSetCableStatus(EnumSourceSearchPort enumInputPort, bit bStatus)
{
    ...
    switch(enumInputPort)
    {
        ...
        case _D0_INPUT_PORT:
            ...
            g_stDpCableStatus.b1D0Status = bStatus;
        ...
        case _D1_INPUT_PORT:
            ...
            g_stDpCableStatus.b1D1Status = bStatus;
        ...
        case _D2_INPUT_PORT:
            ...
            g_stDpCableStatus.b1D2Status = bStatus;
        ...
        case _D6_INPUT_PORT:
            ...
            g_stDpCableStatus.b1D6Status = bStatus;
...
bit ScalerSyncDpGetLinkTrainingPass(EnumSourceSearchPort enumInputPort)
{
    BYTE ucLTStatus = _DP_LINK_TRAINING_NONE;
    ...
    switch(enumInputPort)
    {
        ...
        case _D6_INPUT_PORT:
            ...
            ucLTStatus = GET_DP_RX0_LINK_TRAINING_STATUS();
        ...
        case _D1_INPUT_PORT:
            ...
            ucLTStatus = GET_DP_RX0_LINK_TRAINING_STATUS();
#else
            ucLTStatus = GET_DP_RX1_LINK_TRAINING_STATUS();
#endif
    ...
    if(ucLTStatus == _DP_NORMAL_LINK_TRAINING_PASS)
    {
        return _TRUE;
    }

    if(ucLTStatus == _DP_FAKE_LINK_TRAINING_PASS)
    {
        return _TRUE;
    }
...
void ScalerSyncDpHpdIrqAssertTimer2EventProc_EXINT(void)
{
    ...
    {
#if(_D0_INPUT_PORT_TYPE == _D0_DP_PORT)
        if(GET_DP_D0_HPD_IRQ_ASSERT_T2_EVENT() == _TRUE)
        {
            if(PCB_D0_HOTPLUG_DETECT() == _D0_HOT_PLUG_LOW)
            {
                PCB_D0_HOTPLUG(_D0_HOT_PLUG_HIGH);

                CLR_DP_D0_HPD_IRQ_ASSERT_T2_EVENT();
            }
        }
#endif

#if(_D1_INPUT_PORT_TYPE == _D1_DP_PORT)
        if(GET_DP_D1_HPD_IRQ_ASSERT_T2_EVENT() == _TRUE)
        {
            if(PCB_D1_HOTPLUG_DETECT() == _D1_HOT_PLUG_LOW)
            {
                PCB_D1_HOTPLUG(_D1_HOT_PLUG_HIGH);

                CLR_DP_D1_HPD_IRQ_ASSERT_T2_EVENT();
            }
        }
#endif

#if(_D2_INPUT_PORT_TYPE == _D2_DP_PORT)
        if(GET_DP_D2_HPD_IRQ_ASSERT_T2_EVENT() == _TRUE)
        {
            if(PCB_D2_HOTPLUG_DETECT() == _D2_HOT_PLUG_LOW)
            {
                PCB_D2_HOTPLUG(_D2_HOT_PLUG_HIGH);

                CLR_DP_D2_HPD_IRQ_ASSERT_T2_EVENT();
            }
        }
#endif
    }
...
void ScalerSyncDpAuxManualModeSetting_EXINT(EnumDpAuxManualModeSetting enumSetting)
{
    ...
    {
#if(_D0_DP_EXIST == _ON)
        g_ucDpRx0AuxModeBackup_EXINT = ScalerGetBit_EXINT(PB7_D0_AUX_MODE_SET, _BIT1);

        SET_DP_RX0_AUX_MANUAL_MODE_EXINT();
#endif

#if(_D1_DP_EXIST == _ON)
        g_ucDpRx1AuxModeBackup_EXINT = ScalerGetBit_EXINT(PBA_D0_AUX_MODE_SET, _BIT1);
    ...
    {
#if(_D0_DP_EXIST == _ON)
        if(g_ucDpRx0AuxModeBackup_EXINT == _BIT1)
        {
...
#if(_D1_DP_EXIST == _ON)
        if(g_ucDpRx1AuxModeBackup_EXINT == _BIT1)
        {
...
#pragma restore
#endif
#endif // End of #if(_DP_IRQ_HPD_MODE == _DP_IRQ_HPD_T2_MODE)

#if(_TWO_CHIP_DATA_EXCHANGE_MODE != _DATA_EXCHANGE_MODE_NONE)
//--------------------------------------------------
// Description  : Dp Repeater Power Off Reset
// Input Value  : None
// Output Value : None
//--------------------------------------------------
void ScalerSyncDpRepeaterPowerOffReset(void)
...
#if((_D1_INPUT_PORT_TYPE != _D1_NO_PORT) && (_D1_DP_REPEATER_TYPE == _REPEATER_RTS_TRANSLATOR))
...
void ScalerSyncDpRxSinkCountUpdateProc(EnumSourceSearchPort enumInputPort)
{
    if(GET_DP_TX_DOWNSTREAM_SINK_COUNT_CHANGE() == _TRUE)
    {
        CLR_DP_TX_DOWNSTREAM_SINK_COUNT_CHANGE();
...
#endif
        {
            if(ScalerDpRx0GetDpcdBitInfo(0x00, 0x01, 0x11, _BIT1) == 0x00)
            {
                if(enumInputPort == _D0_INPUT_PORT)
                {
                    ScalerDpRx0UpdateSinkCount(GET_DP_TX_DOWNSTREAM_SINK_COUNT() + 1);
                }
                else
                {
                    ScalerDpRx0UpdateSinkCount(1);
                }
            }
        }
#endif
...
#endif
        {
            if(ScalerDpRx1GetDpcdBitInfo(0x00, 0x01, 0x11, _BIT1) == 0x00)
            {
                if(enumInputPort == _D1_INPUT_PORT)
                {
...
void ScalerSyncDpLongHpdSetAllowed(EnumSourceSearchPort enumInputPort, bit bAllowed)
{
    switch(enumInputPort)
    {
        ...
        case _D0_INPUT_PORT:
            ...
            g_bDpD0LongHpdAllowed = bAllowed;
        ...
        case _D1_INPUT_PORT:
            ...
            g_bDpD1LongHpdAllowed = bAllowed;
        ...
        case _D2_INPUT_PORT:
            ...
            g_bDpD2LongHpdAllowed = bAllowed;
...
bit ScalerSyncDpLongHpdGetAllowed(EnumSourceSearchPort enumInputPort)
{
    switch(enumInputPort)
    {
        ...
        case _D0_INPUT_PORT:
            ...
            return g_bDpD0LongHpdAllowed;
#endif

#if(_D1_INPUT_PORT_TYPE == _D1_DP_PORT)
        case _D1_INPUT_PORT:

            return g_bDpD1LongHpdAllowed;
#endif

#if(_D2_INPUT_PORT_TYPE == _D2_DP_PORT)
        case _D2_INPUT_PORT:

            return g_bDpD2LongHpdAllowed;
#endif
...
bit ScalerSyncDpHdcpRepeaterCheckUrgentEvent(void)
{
    ...
    if(GET_DP_RX_HDCP2_UPSTREAM_PORT() == _HDCP_D0_PORT)
    {
        if(GET_DP_RX0_HDCP2_REPEATER_CAPABLE() == _TRUE)
        {
            ...
            if(GET_DP_RX0_HDCP2_AUTH_STATE() == _DP_RX_HDCP2_REPEATER_STATE_AUTH_DONE)
            {
                return _FALSE;
            }
            ...
            if(GET_DP_RX0_HDCP2_AUTH_STATE() != _DP_RX_HDCP2_REPEATER_STATE_IDLE)
            {
                ...
                {
                    if((GET_DP_RX0_HDCP2_AUTH_STATE() == _DP_RX_HDCP2_REPEATER_STATE_AUTH_1) ||
                       (GET_DP_RX0_HDCP2_AUTH_STATE() == _DP_RX_HDCP2_REPEATER_STATE_AUTH_2))
                    {
                        return _TRUE;
                    }
                }

                if(GET_DP_RX0_HDCP2_UPSTREAM_EVENT() == _DP_RX_HDCP2_UPSTREAM_AUTH_EVENT_START_HANDSHAKE)
                {
                    return _TRUE;
                }
            }
    ...
    if(GET_DP_RX_HDCP_UPSTREAM_PORT() == _HDCP_D0_PORT)
    {
        ...
        {
            if(GET_DP_RX0_HDCP_AUTH_STATE() == _DP_RX_HDCP_STATE_AUTH_DONE)
            {
                return _FALSE;
            }
            ...
            if(GET_DP_RX0_HDCP_AUTH_STATE() != _DP_RX_HDCP_STATE_IDLE)
            {
                ...
                {
                    if((GET_DP_RX0_HDCP_AUTH_STATE() == _DP_RX_HDCP_STATE_AUTH_1) ||
                ...
                if(GET_DP_RX0_HDCP_UPSTREAM_EVENT() == _DP_RX_HDCP_UPSTREAM_AUTH_EVENT_R0_PASS)
                {
                    return _TRUE;
                }
            }
    ...
    if(GET_DP_RX_HDCP2_UPSTREAM_PORT() == _HDCP_D1_PORT)
    {
        if(GET_DP_RX1_HDCP2_REPEATER_CAPABLE() == _TRUE)
        {
            ...
            if(GET_DP_RX1_HDCP2_AUTH_STATE() == _DP_RX_HDCP2_REPEATER_STATE_AUTH_DONE)
            {
                return _FALSE;
            }
            ...
            if(GET_DP_RX1_HDCP2_AUTH_STATE() != _DP_RX_HDCP2_REPEATER_STATE_IDLE)
            {
                ...
                {
                    if((GET_DP_RX1_HDCP2_AUTH_STATE() == _DP_RX_HDCP2_REPEATER_STATE_AUTH_1) ||
                       (GET_DP_RX1_HDCP2_AUTH_STATE() == _DP_RX_HDCP2_REPEATER_STATE_AUTH_2))
                    {
                        return _TRUE;
                    }
                }

                if(GET_DP_RX1_HDCP2_UPSTREAM_EVENT() == _DP_RX_HDCP2_UPSTREAM_AUTH_EVENT_START_HANDSHAKE)
                {
                    return _TRUE;
                }
            }
    ...
    if(GET_DP_RX_HDCP_UPSTREAM_PORT() == _HDCP_D1_PORT)
    {
        ...
        {
            if(GET_DP_RX1_HDCP_AUTH_STATE() == _DP_RX_HDCP_STATE_AUTH_DONE)
            {
                return _FALSE;
            }
            ...
            if(GET_DP_RX1_HDCP_AUTH_STATE() != _DP_RX_HDCP_STATE_IDLE)
            {
                ...
                {
                    if((GET_DP_RX1_HDCP_AUTH_STATE() == _DP_RX_HDCP_STATE_AUTH_1) ||
                ...
                if(GET_DP_RX1_HDCP_UPSTREAM_EVENT() == _DP_RX_HDCP_UPSTREAM_AUTH_EVENT_R0_PASS)
                {
                    return _TRUE;
                }
            }
    ...
    if((GET_DP_RX_HDCP2_UPSTREAM_PORT() != _HDCP_NO_PORT) &&
       (GET_DP_TX_HDCP2_AUTH_STATE() != _HDCP_2_2_TX_STATE_IDLE))
    {
        if(GET_DP_TX_HDCP2_AUTH_HOLD() == _TRUE)
        {
            return _TRUE;
        }
...
bit ScalerSyncDpHdcpDownstreamReAuthCheck(void)
{
    ...
    {
        // In SST mode
        if(ScalerDpRx0GetDpcdBitInfo(0x00, 0x01, 0x11, _BIT0) == 0x00)
        {
            return ((ScalerDpRx0HdcpCheckValid() == _TRUE) && (ScalerDpRx0HDCPCheckLvp() == _TRUE));
        }
    }
    ...
    {
        // In SST mode
        if(ScalerDpRx1GetDpcdBitInfo(0x00, 0x01, 0x11, _BIT0) == 0x00)
        {
            return ((ScalerDpRx1HdcpCheckValid() == _TRUE) && (ScalerDpRx1HDCPCheckLvp() == _TRUE));
        }
    }
...
{
#if(_D0_DP_EXIST == _ON)
    ScalerDpRx0Hdcp2RepeaterCapableProc();
#endif

#if(_D1_DP_EXIST == _ON)
    ScalerDpRx1Hdcp2RepeaterCapableProc();
#endif
}
#endif
#endif // End of #if(_DP_TX_SUPPORT == _ON)
...
bit ScalerSyncDpFakePowerOffCheck(EnumSourceSearchPort enumInputPort)
{
    ...
    if(ScalerDpRxMstGetMstCapability(enumInputPort) == _TRUE)
    {
        // Let MSG keep proccessing
        if(ScalerDpRxMstGetSrcMsgEnable(enumInputPort) == _TRUE)
        {
            // Keep HPD high if upstream is source
            if(ScalerDpRxMstGetUpstreamIsSrc(enumInputPort) == _TRUE)
            {
        ...
        if((GET_DP_GEN_DOWN_REQUEST_MSG() != _DP_MSG_NONE) || (GET_DP_RX_SELF_GEN_DOWN_REQUEST_MSG() == _TRUE) || (GET_DP_TX_WAIT_DOWN_REPLY_CLEAR_PAYLOAD_ID_TABLE() == _TRUE))
        {
            return _TRUE;
        }
    }
...
void ScalerSyncDpSourceWritePowerUpProc(void)
{
    ...
    if(GET_DP_RX0_SOURCE_WRITE_POWER_UP() == _DP_SOURCE_WRITE_POWER_UP_TRIGGERED)
    {
        ...
        SET_DP_RX0_SOURCE_WRITE_POWER_UP(_DP_SOURCE_WRITE_POWER_UP_TIMER_ACTIVE);
    }
#endif

#if(_D1_DP_EXIST == _TRUE)
    if(GET_DP_RX1_SOURCE_WRITE_POWER_UP() == _DP_SOURCE_WRITE_POWER_UP_TRIGGERED)
    {
        ...
        SET_DP_RX1_SOURCE_WRITE_POWER_UP(_DP_SOURCE_WRITE_POWER_UP_TIMER_ACTIVE);
    }
#endif
}
...
void ScalerSyncDpRxClrSourceWritePowerUp(EnumSourceSearchPort enumInputPort)
{
    switch(enumInputPort)
    {
        ...
        case _D0_INPUT_PORT:
            ...
            CLR_DP_RX0_SOURCE_WRITE_POWER_UP();
...
//--------------------------------------------------
// Description  : Hdcp14 Key Sram Gated Clk
// Input Value  : None
// Output Value : None
//--------------------------------------------------
void ScalerSyncHdcp14KeySramGatedClk(bit bEnable)
{
    if(bEnable == _ENABLE)
    {
        // Enable SRAM Module Clock Gating
        ScalerSetBit(P2_C2_HDCP_PCR, ~_BIT7, 0x00);
    }
    else
    {
        // Disable SRAM Module Clock Gating
        ScalerSetBit(P2_C2_HDCP_PCR, ~_BIT7, _BIT7);
    }
}
...
bit ScalerSyncCheckUrgentEvent(void)
{
#if((_DP_SUPPORT == _ON) && (_DP_LINK_INTERVAL_CHECK_SUPPORT == _ON))
    // Link Loss Check For HPD_IRQ
    if(ScalerDpRxLinkStatusLoss() == _TRUE)
    {
...
#if(_DP_LINK_INTERVAL_CHECK_SUPPORT == _ON)
//--------------------------------------------------
// Description  : Get Dp Rx Urgent Flag for Link Loss Case
// Input Value  : None
// Output Value : _TRUE or _FALSE
//--------------------------------------------------
bit ScalerSyncGetUrgentDpRxLinkLossInfo(void)
{
    bit bUrgentLinkLoss = _FALSE;

#if(_D0_DP_EXIST == _ON)
    if(GET_DP_RX0_LINK_TRAINING_STATUS() == _DP_NORMAL_LINK_TRAINING_PASS)
    {
        bUrgentLinkLoss |= GET_DP_RX0_URGENT_LINK_LOSS();
    }
#endif

    return bUrgentLinkLoss;
}
#endif
...
void ScalerSyncTMDSHPDToggleProc(void)
{
    ...
    if(GET_TMDS_RX3_HPD_TRIGGER_EVENT() != _TMDS_HPD_NONE)
    {
        ...
        if(GET_HDMI_Z0_HPD_TYPE() == _HDMI_HPD_TOGGLE_BEFORE_Z0_TOGGLE)
        {
            ScalerTMDSBeforeHotPlugEventProc(_D3_INPUT_PORT, _FALSE);
        }
        else
        {
            ScalerTMDSBeforeHotPlugEventProc(_D3_INPUT_PORT, _TRUE);
        }
    ...
    if(GET_TMDS_RX4_HPD_TRIGGER_EVENT() != _TMDS_HPD_NONE)
    {
        ...
        if(GET_HDMI_Z0_HPD_TYPE() == _HDMI_HPD_TOGGLE_BEFORE_Z0_TOGGLE)
        {
            ScalerTMDSBeforeHotPlugEventProc(_D4_INPUT_PORT, _FALSE);
        }
        else
        {
            ScalerTMDSBeforeHotPlugEventProc(_D4_INPUT_PORT, _TRUE);
        }
    ...
    if(GET_TMDS_RX5_HPD_TRIGGER_EVENT() != _TMDS_HPD_NONE)
    {
        ...
        if(GET_HDMI_Z0_HPD_TYPE() == _HDMI_HPD_TOGGLE_BEFORE_Z0_TOGGLE)
        {
            ScalerTMDSBeforeHotPlugEventProc(_D5_INPUT_PORT, _FALSE);
        }
        else
        {
            ScalerTMDSBeforeHotPlugEventProc(_D5_INPUT_PORT, _TRUE);
        }
        ...
        for(ucI = 1; ucI <= ucPortNum; ucI ++)
        {
            ...
            switch(pstTmdsHpdSeq[ucI].enumPort)
            {
                ...
                case _D1_INPUT_PORT:
                    ...
                    {
                        // Restore Z0 Status
#if(_HW_DIGITAL_PORT_COMBO_PHY_TYPE == _DIGITAL_PORT_COMBO_PHY_ALL) // RL6369 and RL6463
                        ScalerSetBit(PB_B4_IMPEDANCE_01, ~(_BIT3 | _BIT2 | _BIT1 | _BIT0), g_ucTMDSRx1Z0Status);
#elif(_HW_DIGITAL_PORT_COMBO_PHY_TYPE == _DIGITAL_PORT_COMBO_PHY_D1_D2) // RL6492
                ...
                case _D2_INPUT_PORT:
                    ...
                    {
                        // Restore Z0 Status
#if(_HW_DIGITAL_PORT_COMBO_PHY_TYPE == _DIGITAL_PORT_COMBO_PHY_ALL) // RL6369
    ...
    for(ucI = 1; ucI <= ucPortNum; ucI ++)
    {
        ...
        switch(pstTmdsHpdSeq[ucI].enumPort)
        {
            ...
            case _D3_INPUT_PORT:
                ...
                ScalerTMDSHotPlugLowProc(_D3_INPUT_PORT);
                break;
#endif
            ...
            case _D4_INPUT_PORT:
                ...
                ScalerTMDSHotPlugLowProc(_D4_INPUT_PORT);
                break;
#endif
            ...
            case _D5_INPUT_PORT:
                ...
                ScalerTMDSHotPlugLowProc(_D5_INPUT_PORT);
                break;
#endif
    ...
    for(ucI = 1; ucI <= ucPortNum; ucI ++)
    {
        ...
        switch(pstTmdsHpdSeq[ucI].enumPort)
        {
            ...
            case _D0_INPUT_PORT:
                ...
                ScalerTMDSHotPlugHighProc(_D0_INPUT_PORT, pstTmdsHpdSeq[ucI].usHpdTime - pstTmdsHpdSeq[ucI - 1].usHpdTime);
            ...
            case _D1_INPUT_PORT:
                ...
                ScalerTMDSHotPlugHighProc(_D1_INPUT_PORT, pstTmdsHpdSeq[ucI].usHpdTime - pstTmdsHpdSeq[ucI - 1].usHpdTime);
            ...
            case _D2_INPUT_PORT:
                ...
                ScalerTMDSHotPlugHighProc(_D2_INPUT_PORT, pstTmdsHpdSeq[ucI].usHpdTime - pstTmdsHpdSeq[ucI - 1].usHpdTime);
            ...
            case _D3_INPUT_PORT:
                ...
                ScalerTMDSHotPlugHighProc(_D3_INPUT_PORT, pstTmdsHpdSeq[ucI].usHpdTime - pstTmdsHpdSeq[ucI - 1].usHpdTime);

                if(GET_HDMI_Z0_HPD_TYPE() == _HDMI_HPD_Z0_TOGGLE_AT_SAME_TIME)
                {
                    ScalerTMDSAfterHotPlugEventProc(_D3_INPUT_PORT, _TRUE);
                }
                else
                {
                    ScalerTMDSAfterHotPlugEventProc(_D3_INPUT_PORT, _FALSE);
                }
                break;
#endif
            ...
            case _D4_INPUT_PORT:
                ...
                ScalerTMDSHotPlugHighProc(_D4_INPUT_PORT, pstTmdsHpdSeq[ucI].usHpdTime - pstTmdsHpdSeq[ucI - 1].usHpdTime);

                if(GET_HDMI_Z0_HPD_TYPE() == _HDMI_HPD_Z0_TOGGLE_AT_SAME_TIME)
                {
                    ScalerTMDSAfterHotPlugEventProc(_D4_INPUT_PORT, _TRUE);
                }
                else
                {
                    ScalerTMDSAfterHotPlugEventProc(_D4_INPUT_PORT, _FALSE);
                }
                break;
#endif
            ...
            case _D5_INPUT_PORT:
                ...
                ScalerTMDSHotPlugHighProc(_D5_INPUT_PORT, pstTmdsHpdSeq[ucI].usHpdTime - pstTmdsHpdSeq[ucI - 1].usHpdTime);

                if(GET_HDMI_Z0_HPD_TYPE() == _HDMI_HPD_Z0_TOGGLE_AT_SAME_TIME)
                {
                    ScalerTMDSAfterHotPlugEventProc(_D5_INPUT_PORT, _TRUE);
                }
                else
                {
                    ScalerTMDSAfterHotPlugEventProc(_D5_INPUT_PORT, _FALSE);
                }
                break;
#endif
...
#if((_HDMI_MULTI_EDID_SUPPORT == _ON) && (_TMDS_FS_ON_ONLY_TARGETPORT_HPD_SUPPORT == _ON) && (_HDMI_FREESYNC_SUPPORT == _ON))
//--------------------------------------------------
// Description  : CLR HPD Trigger Event
// Input Value  : Input Port
// Output Value : None
//--------------------------------------------------
void ScalerSyncTMDSClrHPDTriggerEvent(EnumSourceSearchPort enumInputPort)
...
void ScalerSyncColorPatternGenIDomainControl(bit bEnable)
{
    ...
    {
        ScalerSetBit(P0_10_M1_VGIP_CTRL, ~_BIT7, 0);
    }
}

//--------------------------------------------------
// Description  : Process Required to Check MCRC stable or not
// Input Value  : None
// Output Value : None
//--------------------------------------------------
void ScalerSyncCheckMCRC(void)
{
    ...
    BYTE pucMDomainMCRCValue[3] = {0};

    BYTE pucMDomainMCRCValue_Backup_A[3] = {0};
    BYTE pucMDomainMCRCValue_Backup_B[3] = {0};
    DWORD ulcnt = 0;

    WORD usMCRCAddress = P36_41_M_M1_CRC_1;

    while(ScalerGetByte(0x0006) == 0x88)
    {
        ScalerTimerWaitForEvent(_EVENT_DVS);

        while((pucMDomainMCRCValue_Backup_A[0] == 0) && (bFinish == _TRUE))
        {
            for(ucCRCRegOffset = 0; ucCRCRegOffset < sizeof(pucMDomainMCRCValue_Backup_A); ucCRCRegOffset++)
            {
                pucMDomainMCRCValue_Backup_A[ucCRCRegOffset] = ScalerGetByte(usMCRCAddress + ucCRCRegOffset);
            }
            DebugMessageFRC("ADDRESS", usMCRCAddress);

            for(ucCRCRegOffset = 0; ucCRCRegOffset < sizeof(pucMDomainMCRCValue_Backup_A); ucCRCRegOffset++)
            {
                DebugMessageFRC("MCRC", pucMDomainMCRCValue_Backup_A[ucCRCRegOffset]);
            }
            if(memcmp(pucMDomainMCRCValue_Backup_A, pucMDomainMCRCValue_Backup_B, sizeof(pucMDomainMCRCValue_Backup_A)) != 0)
            {
                *pucMDomainMCRCValue_Backup_B = *pucMDomainMCRCValue_Backup_A;
                bFinish |= _FALSE;
            }
        }
        ...
        while(ScalerGetByte(0x0006) == 0x88)
        {
            ...
            for(ucCRCRegOffset = 0; ucCRCRegOffset < sizeof(pucMDomainMCRCValue_Backup_A); ucCRCRegOffset++)
            {
                pucMDomainMCRCValue[ucCRCRegOffset] = ScalerGetByte(usMCRCAddress + ucCRCRegOffset);
            }

            if(memcmp(pucMDomainMCRCValue, pucMDomainMCRCValue_Backup_A, sizeof(pucMDomainMCRCValue)) != 0)
            {
                DebugMessageFRC("!!!ADDRESS", usMCRCAddress);
                for(ucCRCRegOffset = 0; ucCRCRegOffset < sizeof(pucMDomainMCRCValue); ucCRCRegOffset++)
                {
                    DebugMessageFRC("!!!MCRC ERROR", pucMDomainMCRCValue[ucCRCRegOffset]);
                }
                ScalerSetByte(0x0006, 0xFF);
            }

            ScalerSetBit(0x0002, ~(_BIT1 | _BIT0), 0x00);

            if(ScalerGetBit(0x0002, (_BIT1 | _BIT0)) != 0x00)
            {
                ScalerSetByte(0x0006, 0x77);
                DebugMessageFRC("UDFL/OVFL M1", ScalerGetByte(0x0002));
                MCU_FE0E_PORT56_PIN_REG = ~MCU_FE0E_PORT56_PIN_REG;
            }

            ulcnt += 1;
            if(ulcnt > 2000)
            {
                DebugMessageFRC("ALIVE", 0);
                ulcnt = 0;
            }
            ScalerDebug();
        }
...
{
#if(_MCRC_STABILITY_CHECK_SUPPORT == _ON)
    while(ScalerGetByte(0x0006) == 0x66)
    {
        ScalerSyncColorPatternGenIDomainControl(_ENABLE);
        ScalerBurstWrite(tByPassTable, sizeof(tByPassTable), GET_CURRENT_BANK_NUMBER(), 0x00, _BURSTWRITE_COMMAND, _BURSTWRITE_FROM_FLASH);
        ScalerSetByte(0x0006, 0x00);
    }
    while(ScalerGetByte(0x0006) == 0x77)
    {
        ScalerSyncColorPatternGenIDomainControl(_DISABLE);
        ScalerSetByte(0x0006, 0x00);
    }
    while(ScalerGetByte(0x0006) == 0x88)
    {
...
void ScalerSyncResetProc(BYTE ucInputPort)
{
    ...
    ScalerTMDSClrHdmi21VtemReceived();
#endif
#endif
...
#if(_DIGITAL_PORT_SUPPORT == _ON)
            ScalerSyncHdcp14KeySramGatedClk(_DISABLE);
#endif
...
#if(_TMDS_Z0_POWER_SAVING_TOGGLE_SUPPORT == _ON)
            ScalerTMDSClkLaneZ0StartToggle();
#endif

#if(_DIGITAL_PORT_SUPPORT == _ON)
            ScalerSyncHdcp14KeySramGatedClk(_ENABLE);
#endif
...
#if(_SOURCE_BACKGROUND_DETECT_SUPPORT == _ON)
//--------------------------------------------------
// Description  : Store Current Actived Port
// Input Value  : Current Actived Port
// Output Value : None
//--------------------------------------------------
void ScalerSyncStoreActivedPort(BYTE ucInputPort)
{
    SET_STORE_ACTIVED_PORT(ucInputPort);
}

//--------------------------------------------------
// Description  : Clear Current Actived Port
// Input Value  : None
// Output Value : None
//--------------------------------------------------
void ScalerSyncClearActivedPort(void)
{
    CLR_STORE_ACTIVED_PORT();
}
#endif
...
#if(_SOURCE_BACKGROUND_DETECT_SUPPORT == _ON)
    // Save sys port, use in TMDS INT handler to prevent from overwritting the colorinfo
    g_ucSourceDetectingPort = ucInputPort;
#endif
...
#if((_DVI_SUPPORT == _ON) || (_HDMI_SUPPORT == _ON))
    ScalerTMDSMacDdcSwitchCtrl(ucInputPort);
    ...
    switch(ucInputPort)
    {
        ...
        case _D6_INPUT_PORT:
...
#if((_D0_INPUT_PORT_TYPE == _D0_HDMI_PORT) || (_D0_INPUT_PORT_TYPE == _D0_MHL_PORT) || (_D0_INPUT_PORT_TYPE == _D0_DVI_PORT))
...
#if((_D0_INPUT_PORT_TYPE == _D0_HDMI_PORT) || (_D0_INPUT_PORT_TYPE == _D0_MHL_PORT) || (_D0_INPUT_PORT_TYPE == _D0_DVI_PORT))
            if((GET_HDMI_HOTPLUG_TOGGLE() == _TRUE) && ((GET_DIGITAL_PORT_SWITCH_TO_D0() == _FALSE) || (GET_TMDS_RX0_PS_WAKEUP_HPD_TOGGLE() == _TRUE)) && (GET_HDMI_Z0_HPD_TYPE() == _HDMI_HPD_Z0_TOGGLE_AT_SAME_TIME))
            {
#if(_HDMI_HPD_TOGGLE_BY_Z0_OFF_DDC_TOGGLE == _ON)
                if(((MCU_FFE4_DDC0_EDID_IRQ & _BIT2) == _BIT2) || (GET_TMDS_RX0_AC_ON_HPD_TOGGLE_EVENT() == _TRUE))
#endif
                {
...
#endif
                    {
                        ...
                        ScalerTMDSHotPlugEvent(_D0_INPUT_PORT);
                        CLR_TMDS_RX0_PS_WAKEUP_HPD_TOGGLE();

#if(_HDMI_HPD_TOGGLE_BY_Z0_OFF_DDC_TOGGLE == _ON)
                        CLR_TMDS_RX0_AC_ON_HPD_TOGGLE_EVENT();
#endif
                    }
                }
            }
            ...
            if(GET_DIGITAL_PORT_SWITCH_TO_D0() == _FALSE)
            {
...
#if((_D0_INPUT_PORT_TYPE == _D0_HDMI_PORT) || (_D0_INPUT_PORT_TYPE == _D0_MHL_PORT) || (_D0_INPUT_PORT_TYPE == _D0_DVI_PORT))
                if((GET_HDMI_HOTPLUG_TOGGLE() == _TRUE) && ((GET_DIGITAL_PORT_SWITCH_TO_D0() == _FALSE) || (GET_TMDS_RX0_PS_WAKEUP_HPD_TOGGLE() == _TRUE)) && (GET_HDMI_Z0_HPD_TYPE() == _HDMI_Z0_TOGGLE_BEFORE_HPD_TOGGLE))
                {
#if(_HDMI_HPD_TOGGLE_BY_Z0_OFF_DDC_TOGGLE == _ON)
                    if(((MCU_FFE4_DDC0_EDID_IRQ & _BIT2) != _BIT2) && (GET_TMDS_RX0_AC_ON_HPD_TOGGLE_EVENT() == _FALSE))
                    {
                        ScalerGDIPhyPowerProc(_D0_INPUT_PORT, _POWER_ACTION_OFF_TO_NORMAL);
                    }
                    else
                ...
                else if((GET_HDMI_HOTPLUG_TOGGLE() == _TRUE) && ((GET_DIGITAL_PORT_SWITCH_TO_D0() == _FALSE) || (GET_TMDS_RX0_PS_WAKEUP_HPD_TOGGLE() == _TRUE)) && (GET_HDMI_Z0_HPD_TYPE() == _HDMI_HPD_TOGGLE_BEFORE_Z0_TOGGLE))
                {
#if(_HDMI_HPD_TOGGLE_BY_Z0_OFF_DDC_TOGGLE == _ON)
                    if(((MCU_FFE4_DDC0_EDID_IRQ & _BIT2) == _BIT2) || (GET_TMDS_RX0_AC_ON_HPD_TOGGLE_EVENT() == _TRUE))
#endif
        ...
        case _D1_INPUT_PORT:
...
#if((_D1_INPUT_PORT_TYPE == _D1_HDMI_PORT) || (_D1_INPUT_PORT_TYPE == _D1_MHL_PORT) || (_D1_INPUT_PORT_TYPE == _D1_DVI_PORT))
...
#if((_D1_INPUT_PORT_TYPE == _D1_HDMI_PORT) || (_D1_INPUT_PORT_TYPE == _D1_MHL_PORT) || (_D1_INPUT_PORT_TYPE == _D1_DVI_PORT))
            if((GET_HDMI_HOTPLUG_TOGGLE() == _TRUE) && ((GET_DIGITAL_PORT_SWITCH_TO_D1() == _FALSE) || (GET_TMDS_RX1_PS_WAKEUP_HPD_TOGGLE() == _TRUE)) && (GET_HDMI_Z0_HPD_TYPE() == _HDMI_HPD_Z0_TOGGLE_AT_SAME_TIME))
            {
#if(_HDMI_HPD_TOGGLE_BY_Z0_OFF_DDC_TOGGLE == _ON)
                if(((MCU_FFE5_DDC1_EDID_IRQ & _BIT2) == _BIT2) || (GET_TMDS_RX1_AC_ON_HPD_TOGGLE_EVENT() == _TRUE))
#endif
                {
#if(_TMDS_DDC_CHECK_BUSY_BEFORE_HPD == _ON)
...
#endif
                    {
                        ...
                        ScalerTMDSHotPlugEvent(_D1_INPUT_PORT);
                        CLR_TMDS_RX1_PS_WAKEUP_HPD_TOGGLE();

#if(_HDMI_HPD_TOGGLE_BY_Z0_OFF_DDC_TOGGLE == _ON)
                        CLR_TMDS_RX1_AC_ON_HPD_TOGGLE_EVENT();
#endif
                    }
                }
            }
            ...
            if(GET_DIGITAL_PORT_SWITCH_TO_D1() == _FALSE)
            {
...
#if((_D1_INPUT_PORT_TYPE == _D1_HDMI_PORT) || (_D1_INPUT_PORT_TYPE == _D1_MHL_PORT) || (_D1_INPUT_PORT_TYPE == _D1_DVI_PORT))
                if((GET_HDMI_HOTPLUG_TOGGLE() == _TRUE) && ((GET_DIGITAL_PORT_SWITCH_TO_D1() == _FALSE) || (GET_TMDS_RX1_PS_WAKEUP_HPD_TOGGLE() == _TRUE)) && (GET_HDMI_Z0_HPD_TYPE() == _HDMI_Z0_TOGGLE_BEFORE_HPD_TOGGLE))
                {
#if(_HDMI_HPD_TOGGLE_BY_Z0_OFF_DDC_TOGGLE == _ON)
                    if(((MCU_FFE5_DDC1_EDID_IRQ & _BIT2) != _BIT2) && (GET_TMDS_RX1_AC_ON_HPD_TOGGLE_EVENT() == _FALSE))
                    {
                        ScalerGDIPhyPowerProc(_D1_INPUT_PORT, _POWER_ACTION_OFF_TO_NORMAL);
                    }
                    else
                ...
                else if((GET_HDMI_HOTPLUG_TOGGLE() == _TRUE) && ((GET_DIGITAL_PORT_SWITCH_TO_D1() == _FALSE) || (GET_TMDS_RX1_PS_WAKEUP_HPD_TOGGLE() == _TRUE)) && (GET_HDMI_Z0_HPD_TYPE() == _HDMI_HPD_TOGGLE_BEFORE_Z0_TOGGLE))
                {
#if(_HDMI_HPD_TOGGLE_BY_Z0_OFF_DDC_TOGGLE == _ON)
                    if(((MCU_FFE5_DDC1_EDID_IRQ & _BIT2) == _BIT2) || (GET_TMDS_RX1_AC_ON_HPD_TOGGLE_EVENT() == _TRUE))
#endif
        ...
        case _D2_INPUT_PORT:
...
#if((_D2_INPUT_PORT_TYPE == _D2_HDMI_PORT) || (_D2_INPUT_PORT_TYPE == _D2_MHL_PORT) || (_D2_INPUT_PORT_TYPE == _D2_DVI_PORT))
...
#if((_D2_INPUT_PORT_TYPE == _D2_HDMI_PORT) || (_D2_INPUT_PORT_TYPE == _D2_MHL_PORT) || (_D2_INPUT_PORT_TYPE == _D2_DVI_PORT))
            if((GET_HDMI_HOTPLUG_TOGGLE() == _TRUE) && ((GET_DIGITAL_PORT_SWITCH_TO_D2() == _FALSE) || (GET_TMDS_RX2_PS_WAKEUP_HPD_TOGGLE() == _TRUE)) && (GET_HDMI_Z0_HPD_TYPE() == _HDMI_HPD_Z0_TOGGLE_AT_SAME_TIME))
            {
#if(_HDMI_HPD_TOGGLE_BY_Z0_OFF_DDC_TOGGLE == _ON)
                if(((MCU_FE69_DDC2_EDID_IRQ & _BIT2) == _BIT2) || (GET_TMDS_RX2_AC_ON_HPD_TOGGLE_EVENT() == _TRUE))
#endif
                {
#if(_TMDS_DDC_CHECK_BUSY_BEFORE_HPD == _ON)
            ...
            if(GET_DIGITAL_PORT_SWITCH_TO_D2() == _FALSE)
            {
...
#if((_D2_INPUT_PORT_TYPE == _D2_HDMI_PORT) || (_D2_INPUT_PORT_TYPE == _D2_MHL_PORT) || (_D2_INPUT_PORT_TYPE == _D2_DVI_PORT))
                if((GET_HDMI_HOTPLUG_TOGGLE() == _TRUE) && ((GET_DIGITAL_PORT_SWITCH_TO_D2() == _FALSE) || (GET_TMDS_RX2_PS_WAKEUP_HPD_TOGGLE() == _TRUE)) && (GET_HDMI_Z0_HPD_TYPE() == _HDMI_Z0_TOGGLE_BEFORE_HPD_TOGGLE))
                {
#if(_HDMI_HPD_TOGGLE_BY_Z0_OFF_DDC_TOGGLE == _ON)
                    if(((MCU_FE69_DDC2_EDID_IRQ & _BIT2) != _BIT2) && (GET_TMDS_RX2_AC_ON_HPD_TOGGLE_EVENT() == _FALSE))
                    {
                        ScalerGDIPhyPowerProc(_D2_INPUT_PORT, _POWER_ACTION_OFF_TO_NORMAL);
                    }
                    else
                ...
                else if((GET_HDMI_HOTPLUG_TOGGLE() == _TRUE) && ((GET_DIGITAL_PORT_SWITCH_TO_D2() == _FALSE) || (GET_TMDS_RX2_PS_WAKEUP_HPD_TOGGLE() == _TRUE)) && (GET_HDMI_Z0_HPD_TYPE() == _HDMI_HPD_TOGGLE_BEFORE_Z0_TOGGLE))
                {
#if(_HDMI_HPD_TOGGLE_BY_Z0_OFF_DDC_TOGGLE == _ON)
                    if(((MCU_FE69_DDC2_EDID_IRQ & _BIT2) == _BIT2) || (GET_TMDS_RX2_AC_ON_HPD_TOGGLE_EVENT() == _TRUE))
#endif
        ...
        case _D3_INPUT_PORT:
...
#if((_D3_INPUT_PORT_TYPE == _D3_HDMI_PORT) || (_D3_INPUT_PORT_TYPE == _D3_MHL_PORT) || (_D3_INPUT_PORT_TYPE == _D3_DVI_PORT))
            if(GET_DIGITAL_PORT_SWITCH_TO_D3() == _FALSE)
            {
...
#if((_D3_INPUT_PORT_TYPE == _D3_HDMI_PORT) || (_D3_INPUT_PORT_TYPE == _D3_MHL_PORT) || (_D3_INPUT_PORT_TYPE == _D3_DVI_PORT))
            if((GET_HDMI_HOTPLUG_TOGGLE() == _TRUE) && ((GET_DIGITAL_PORT_SWITCH_TO_D3() == _FALSE) || (GET_TMDS_RX3_PS_WAKEUP_HPD_TOGGLE() == _TRUE)) && (GET_HDMI_Z0_HPD_TYPE() == _HDMI_HPD_Z0_TOGGLE_AT_SAME_TIME))
            {
#if(_HDMI_HPD_TOGGLE_BY_Z0_OFF_DDC_TOGGLE == _ON)
                if(((MCU_FE72_DDC3_EDID_IRQ & _BIT2) == _BIT2) || (GET_TMDS_RX3_AC_ON_HPD_TOGGLE_EVENT() == _TRUE))
#endif
                {
#if(_TMDS_DDC_CHECK_BUSY_BEFORE_HPD == _ON)
            ...
            if(GET_DIGITAL_PORT_SWITCH_TO_D3() == _FALSE)
            {
...
#if((_D3_INPUT_PORT_TYPE == _D3_HDMI_PORT) || (_D3_INPUT_PORT_TYPE == _D3_MHL_PORT) || (_D3_INPUT_PORT_TYPE == _D3_DVI_PORT))
                if((GET_HDMI_HOTPLUG_TOGGLE() == _TRUE) && ((GET_DIGITAL_PORT_SWITCH_TO_D3() == _FALSE) || (GET_TMDS_RX3_PS_WAKEUP_HPD_TOGGLE() == _TRUE)) && (GET_HDMI_Z0_HPD_TYPE() == _HDMI_Z0_TOGGLE_BEFORE_HPD_TOGGLE))
                {
#if(_HDMI_HPD_TOGGLE_BY_Z0_OFF_DDC_TOGGLE == _ON)
                    if(((MCU_FE72_DDC3_EDID_IRQ & _BIT2) != _BIT2) && (GET_TMDS_RX3_AC_ON_HPD_TOGGLE_EVENT() == _FALSE))
                    {
                        ScalerGDIPhyPowerProc(_D3_INPUT_PORT, _POWER_ACTION_OFF_TO_NORMAL);
                    }
                    else
                ...
                else if((GET_HDMI_HOTPLUG_TOGGLE() == _TRUE) && ((GET_DIGITAL_PORT_SWITCH_TO_D3() == _FALSE) || (GET_TMDS_RX3_PS_WAKEUP_HPD_TOGGLE() == _TRUE)) && (GET_HDMI_Z0_HPD_TYPE() == _HDMI_HPD_TOGGLE_BEFORE_Z0_TOGGLE))
                {
#if(_HDMI_HPD_TOGGLE_BY_Z0_OFF_DDC_TOGGLE == _ON)
                    if(((MCU_FE72_DDC3_EDID_IRQ & _BIT2) == _BIT2) || (GET_TMDS_RX3_AC_ON_HPD_TOGGLE_EVENT() == _TRUE))
#endif
        ...
        case _D4_INPUT_PORT:
...
#if((_D4_INPUT_PORT_TYPE == _D4_HDMI_PORT) || (_D4_INPUT_PORT_TYPE == _D4_MHL_PORT) || (_D4_INPUT_PORT_TYPE == _D4_DVI_PORT))
            if(GET_DIGITAL_PORT_SWITCH_TO_D4() == _FALSE)
            {
...
#if((_D4_INPUT_PORT_TYPE == _D4_HDMI_PORT) || (_D4_INPUT_PORT_TYPE == _D4_MHL_PORT) || (_D4_INPUT_PORT_TYPE == _D4_DVI_PORT))
            if((GET_HDMI_HOTPLUG_TOGGLE() == _TRUE) && ((GET_DIGITAL_PORT_SWITCH_TO_D4() == _FALSE) || (GET_TMDS_RX4_PS_WAKEUP_HPD_TOGGLE() == _TRUE)) && (GET_HDMI_Z0_HPD_TYPE() == _HDMI_HPD_Z0_TOGGLE_AT_SAME_TIME))
            {
#if(_HDMI_HPD_TOGGLE_BY_Z0_OFF_DDC_TOGGLE == _ON)
                if(((MCU_FE7B_DDC4_EDID_IRQ & _BIT2) == _BIT2) || (GET_TMDS_RX4_AC_ON_HPD_TOGGLE_EVENT() == _TRUE))
#endif
                {
#if(_TMDS_DDC_CHECK_BUSY_BEFORE_HPD == _ON)
...
#endif
                    {
                        ...
                        ScalerTMDSHotPlugEvent(_D4_INPUT_PORT);
                        CLR_TMDS_RX4_PS_WAKEUP_HPD_TOGGLE();

#if(_HDMI_HPD_TOGGLE_BY_Z0_OFF_DDC_TOGGLE == _ON)
                        CLR_TMDS_RX4_AC_ON_HPD_TOGGLE_EVENT();
#endif
                    }
                }
            }
            ...
            {
#if((_D4_INPUT_PORT_TYPE == _D4_HDMI_PORT) || (_D4_INPUT_PORT_TYPE == _D4_MHL_PORT) || (_D4_INPUT_PORT_TYPE == _D4_DVI_PORT))
                if((GET_HDMI_HOTPLUG_TOGGLE() == _TRUE) && ((GET_DIGITAL_PORT_SWITCH_TO_D4() == _FALSE) || (GET_TMDS_RX4_PS_WAKEUP_HPD_TOGGLE() == _TRUE)) && (GET_HDMI_Z0_HPD_TYPE() == _HDMI_Z0_TOGGLE_BEFORE_HPD_TOGGLE))
                {
#if(_HDMI_HPD_TOGGLE_BY_Z0_OFF_DDC_TOGGLE == _ON)
                    if(((MCU_FE7B_DDC4_EDID_IRQ & _BIT2) != _BIT2) && (GET_TMDS_RX4_AC_ON_HPD_TOGGLE_EVENT() == _FALSE))
                    {
                        ScalerGDIPhyPowerProc(_D4_INPUT_PORT, _POWER_ACTION_OFF_TO_NORMAL);
                    }
                    else
                ...
                else if((GET_HDMI_HOTPLUG_TOGGLE() == _TRUE) && ((GET_DIGITAL_PORT_SWITCH_TO_D4() == _FALSE) || (GET_TMDS_RX4_PS_WAKEUP_HPD_TOGGLE() == _TRUE)) && (GET_HDMI_Z0_HPD_TYPE() == _HDMI_HPD_TOGGLE_BEFORE_Z0_TOGGLE))
                {
#if(_HDMI_HPD_TOGGLE_BY_Z0_OFF_DDC_TOGGLE == _ON)
                    if(((MCU_FE7B_DDC4_EDID_IRQ & _BIT2) == _BIT2) || (GET_TMDS_RX4_AC_ON_HPD_TOGGLE_EVENT() == _TRUE))
#endif
        ...
        case _D5_INPUT_PORT:
...
#if((_D5_INPUT_PORT_TYPE == _D5_HDMI_PORT) || (_D5_INPUT_PORT_TYPE == _D5_MHL_PORT) || (_D5_INPUT_PORT_TYPE == _D5_DVI_PORT))
            if(GET_DIGITAL_PORT_SWITCH_TO_D5() == _FALSE)
            {
...
#if((_D5_INPUT_PORT_TYPE == _D5_HDMI_PORT) || (_D5_INPUT_PORT_TYPE == _D5_MHL_PORT) || (_D5_INPUT_PORT_TYPE == _D5_DVI_PORT))
            if((GET_HDMI_HOTPLUG_TOGGLE() == _TRUE) && ((GET_DIGITAL_PORT_SWITCH_TO_D5() == _FALSE) || (GET_TMDS_RX5_PS_WAKEUP_HPD_TOGGLE() == _TRUE)) && (GET_HDMI_Z0_HPD_TYPE() == _HDMI_HPD_Z0_TOGGLE_AT_SAME_TIME))
            {
#if(_HDMI_HPD_TOGGLE_BY_Z0_OFF_DDC_TOGGLE == _ON)
                if(((MCU_FE84_DDC5_EDID_IRQ & _BIT2) == _BIT2) || (GET_TMDS_RX5_AC_ON_HPD_TOGGLE_EVENT() == _TRUE))
#endif
                {
#if(_TMDS_DDC_CHECK_BUSY_BEFORE_HPD == _ON)
...
#endif
                    {
                        ...
                        ScalerTMDSHotPlugEvent(_D5_INPUT_PORT);
                        CLR_TMDS_RX5_PS_WAKEUP_HPD_TOGGLE();

#if(_HDMI_HPD_TOGGLE_BY_Z0_OFF_DDC_TOGGLE == _ON)
                        CLR_TMDS_RX5_AC_ON_HPD_TOGGLE_EVENT();
#endif
                    }
                }
            }
            ...
            {
#if((_D5_INPUT_PORT_TYPE == _D5_HDMI_PORT) || (_D5_INPUT_PORT_TYPE == _D5_MHL_PORT) || (_D5_INPUT_PORT_TYPE == _D5_DVI_PORT))
                if((GET_HDMI_HOTPLUG_TOGGLE() == _TRUE) && ((GET_DIGITAL_PORT_SWITCH_TO_D5() == _FALSE) || (GET_TMDS_RX5_PS_WAKEUP_HPD_TOGGLE() == _TRUE)) && (GET_HDMI_Z0_HPD_TYPE() == _HDMI_Z0_TOGGLE_BEFORE_HPD_TOGGLE))
                {
#if(_HDMI_HPD_TOGGLE_BY_Z0_OFF_DDC_TOGGLE == _ON)
                    if(((MCU_FE84_DDC5_EDID_IRQ & _BIT2) != _BIT2) && (GET_TMDS_RX5_AC_ON_HPD_TOGGLE_EVENT() == _FALSE))
                    {
                        ScalerGDIPhyPowerProc(_D5_INPUT_PORT, _POWER_ACTION_OFF_TO_NORMAL);
                    }
                    else
                ...
                else if((GET_HDMI_HOTPLUG_TOGGLE() == _TRUE) && ((GET_DIGITAL_PORT_SWITCH_TO_D5() == _FALSE) || (GET_TMDS_RX5_PS_WAKEUP_HPD_TOGGLE() == _TRUE)) && (GET_HDMI_Z0_HPD_TYPE() == _HDMI_HPD_TOGGLE_BEFORE_Z0_TOGGLE))
                {
#if(_HDMI_HPD_TOGGLE_BY_Z0_OFF_DDC_TOGGLE == _ON)
                    if(((MCU_FE84_DDC5_EDID_IRQ & _BIT2) == _BIT2) || (GET_TMDS_RX5_AC_ON_HPD_TOGGLE_EVENT() == _TRUE))
#endif
...
#if(_D0_HDMI_SUPPORT == _ON)
        case _D0_INPUT_PORT:
            return GET_DIGITAL_PORT_SWITCH_TO_D0();
            break;
#endif
