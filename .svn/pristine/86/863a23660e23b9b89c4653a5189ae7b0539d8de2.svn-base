...
void SysPowerInitial(void)
{
...
#if((_HW_DP_AUX_CLK_DEFAULT_FROM_M2PLL_SUPPORT == _ON) && (_HW_M2PLL_POWER_CTRL_BY_MCU_RESET_SUPPORT == _OFF))
    // Power on Xtal Clock
    ...
    UserCommonInterfaceHdmiAllPortExternalEdidParser();
#endif
...
#if((_DELL_HDMI_HPD_DDC_STYLE_SUPPORT == _ON) && (_HDMI_2_0_SUPPORT == _ON))
    ScalerTMDSRxAllPortScdcEnableSetting(_DISABLE);
#endif
    ...
    //////////////////////////
#if(_DVI_SUPPORT == _ON)
    ScalerTMDSRxAcOnInitial();
#endif

#if(_HDMI_2_0_SUPPORT == _ON)
    UserCommonInterfaceTmdsGetVersionScdcInfo();
#endif
    ...
    ScalerCecInitial();
    ScalerCecFunctionEnable(UserInterfaceCecGetCecActive());
#endif
    ...
    {
        ScalerDpInitial(_D0_INPUT_PORT, UserInterfaceGetDPVersion(_D0_INPUT_PORT), UserInterfaceGetDpLaneCount(_D0_INPUT_PORT), UserCommonInterfaceGetDpMstCapablePort(), UserInterfaceGetFreeSyncEnable(_D0_INPUT_PORT), UserCommonAudioDpGetCapabilityInitialState(_D0_INPUT_PORT));
    }
    else if(GET_DP_EMBEDDED_SWITCH_SELECT() == _D1_INPUT_PORT)
    {
        ScalerDpInitial(_D1_INPUT_PORT, UserInterfaceGetDPVersion(_D1_INPUT_PORT), UserInterfaceGetDpLaneCount(_D1_INPUT_PORT), UserCommonInterfaceGetDpMstCapablePort(), UserInterfaceGetFreeSyncEnable(_D1_INPUT_PORT), UserCommonAudioDpGetCapabilityInitialState(_D1_INPUT_PORT));
    }
    else if(GET_DP_EMBEDDED_SWITCH_SELECT() == _D2_INPUT_PORT)
    {
        ScalerDpInitial(_D2_INPUT_PORT, UserInterfaceGetDPVersion(_D2_INPUT_PORT), UserInterfaceGetDpLaneCount(_D2_INPUT_PORT), UserCommonInterfaceGetDpMstCapablePort(), UserInterfaceGetFreeSyncEnable(_D2_INPUT_PORT), UserCommonAudioDpGetCapabilityInitialState(_D2_INPUT_PORT));
    }
    ...
    ScalerDpRxHdcp14RepeaterCapabilityCheck(_D0_INPUT_PORT, UserCommonInterfaceGetDpMstCapablePort(), UserInterfaceGetDpCloneOutputStatus());
#endif

#if((_D6_INPUT_PORT_TYPE == _D6_DP_PORT) && (_D6_INPUT_PORT_SWITCH_FROM == _SWITCH_FROM_D0))
    if(GET_D6_SWITCH_SELECT() == _D0_INPUT_PORT)
    {
        ScalerDpInitial(_D6_INPUT_PORT, UserInterfaceGetDPVersion(_D6_INPUT_PORT), UserInterfaceGetDpLaneCount(_D6_INPUT_PORT), UserCommonInterfaceGetDpMstCapablePort(), UserInterfaceGetFreeSyncEnable(_D6_INPUT_PORT), UserCommonAudioDpGetCapabilityInitialState(_D6_INPUT_PORT));
        ScalerDpInitial(_D0_INPUT_PORT, UserInterfaceGetDPVersion(_D0_INPUT_PORT), UserInterfaceGetDpLaneCount(_D0_INPUT_PORT), UserCommonInterfaceGetDpMstCapablePort(), UserInterfaceGetFreeSyncEnable(_D0_INPUT_PORT), UserCommonAudioDpGetCapabilityInitialState(_D0_INPUT_PORT));
    }
...
#if(_DP_HDCP14_REPEATER_BIT_DISABLE_FOR_TX_CLONE_OFF == _ON)
    ScalerDpRxHdcp14RepeaterCapabilityCheck(_D1_INPUT_PORT, UserCommonInterfaceGetDpMstCapablePort(), UserInterfaceGetDpCloneOutputStatus());
#endif
    ScalerDpInitial(_D1_INPUT_PORT, UserInterfaceGetDPVersion(_D1_INPUT_PORT), UserInterfaceGetDpLaneCount(_D1_INPUT_PORT), UserCommonInterfaceGetDpMstCapablePort(), UserInterfaceGetFreeSyncEnable(_D1_INPUT_PORT), UserCommonAudioDpGetCapabilityInitialState(_D1_INPUT_PORT));
#endif
...
void SysPowerSwitch(EnumPowerAction enumSwitch)
{
    switch(enumSwitch)
    {
        case _POWER_ACTION_NORMAL_TO_PS:
...
#if(_HDMI_SUPPORT == _ON)
            if((UserInterfaceGetForcePSStatus() == _TRUE) || (UserInterfaceForcePSWakeupTmdsHpdToggle() == _USER_FORCE_PS_TMDS_HPD_TOGGLE))
            {
            ...
            if((GET_D0_CABLE_STATUS() == _FALSE) || (UserInterfaceGetRepeaterIntoPowerOff() == _DATA_EXCHANGE_REPEATER_POWER_OFF))
#elif((_D1_INPUT_PORT_TYPE != _D1_NO_PORT) && (_D1_DP_REPEATER_TYPE == _REPEATER_RTS_TRANSLATOR))
            if((GET_D1_CABLE_STATUS() == _FALSE) || (UserInterfaceGetRepeaterIntoPowerOff() == _DATA_EXCHANGE_REPEATER_POWER_OFF))
#endif
            {
                ...
                // Clear status that might cause PS predetect passed unexpetedly
                ScalerSyncDpRepeaterPowerOffReset();
            }
#endif
...
void SysPowerSystemClockSelect(BYTE ucClockSel)
{
    ...
    else
    {
        ...
        // Internal CLK Select to External Crystal.
...
void SysPowerLoadHDCPKey(void)
{
    ...
    ScalerTMDSRxAllPortChangeHdcpCap(_TMDS_HDCP_NONE);
#else
    ScalerTMDSRxAllPortChangeHdcpCap(UserInterfaceTmdsAcOnGetHdcpCap());
#endif
#endif
...
bit SysPowerCheckFakeOff(void)
{
    ...
    else
    {
        ...
        else if(ScalerSyncDpFakePowerOffCheck(SysSourceGetInputPort()) == _TRUE)
        {
