...
StructSystemDataType code g_stSystemDefaultData =
{
    ...
    _DP_AUX_ADJR_DEFAULT_VALUE,        // DPAuxAdjrSetting
    ...
    _FALSE,                            // Eizo DP Illegal Idle Pattern Check Support
#if(_SOURCE_BACKGROUND_DETECT_SUPPORT == _ON)
    _SOURCE_BG_SWITCH_AUTO_WITHOUT_GROUP, // Source Bg Scan Type
#endif
...
BYTE UserCommonNVRamGetSystemData(EnumSystemDataItemDataType enumDataType)
{
    ...
    switch(enumDataType)
    {
        ...
        case _SOURCE_BG_SCAN_TYPE:
            ucValue = g_stSystemData.b1SourceBgScanType;
            break;
#endif
    ...
    ((_HDMI_FREESYNC_SUPPORT == _ON) || (_TMDS_HDCP_2_2_SUPPORT == _ON)))
        ...
        case _HDMI_HOT_PLUG_SUPPORT_TIME:
            ucValue = g_stSystemData.b4HDMIHotPlugSupportTime;
            break;
        ...
        case _DP_ILLEGAL_IDLE_PATTERN_CHECK:
            ucValue = g_stSystemData.b1DPIllegalIdlePatternCheck;
            break;
...
void UserCommonNVRamSetSystemData(EnumSystemDataItemDataType enumDataType, BYTE ucValue)
{
    switch(enumDataType)
    {
...
#if(_SOURCE_BACKGROUND_DETECT_SUPPORT == _ON)
        case _SOURCE_BG_SCAN_TYPE:
            g_stSystemData.b1SourceBgScanType = ucValue;
            break;
#endif
        ...
        case _HDMI_HOT_PLUG_SUPPORT_TIME:
            ...
            g_stSystemData.b4HDMIHotPlugSupportTime = ucValue;

#if(_DIGITAL_PORT_SUPPORT == _ON)
            SET_HDMI_HOTPLUG_TIME(g_stSystemData.b4HDMIHotPlugSupportTime);
#endif
            break;
        ...
        case _DP_ILLEGAL_IDLE_PATTERN_CHECK:
            g_stSystemData.b1DPIllegalIdlePatternCheck = ucValue;

#if(_DP_SUPPORT == _ON)

#if(_DP_ILLEGAL_IDLE_PATTERN_CHECK_SUPPORT == _ON)
            if(g_stSystemData.b1DPIllegalIdlePatternCheck == _ON)
            {
...
bit UserCommonEepromRead(DWORD ulSubAddr, WORD usLength, BYTE *pucReadArray)
{
    ...
    if(UserInterfaceSystemEepromReadCustomProc(ulSubAddr, usLength, pucReadArray) == _FAIL)
    {
        return _FAIL;
    }
    return _SUCCESS;
...
#if(_USER_SYSTEM_EEPROM_CUSTOMIZATION == _ON)
    if(UserInterfaceSystemEepromWriteCustomProc(ulSubAddr, usLength, pucWriteArray) == _FAIL)
    {
        return _FAIL;
    }
