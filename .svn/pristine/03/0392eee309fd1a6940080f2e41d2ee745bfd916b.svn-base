...
void ScalerTMDSRx1SwitchHdcpSlaveAddr(void);
void ScalerTMDSRx1BeforeHotPlugEventProc(bit bDisableZ0);
void ScalerTMDSRx1AfterHotPlugEventProc(bit bRestoreZ0);
...
void ScalerTMDSRx1Initial(void)
{
    ...
    memset(&g_stTmdsMacRx1SpdInfo, 0x00, sizeof(g_stTmdsMacRx1SpdInfo));
#endif
...
void ScalerTMDSRx1SetPhy_EXINT0(void) using 1
{
    ...
    ScalerSetBit_EXINT(P7A_1A_CMU_00, ~_BIT3, 0x00);
    DELAY_5US_EXINT();
    ScalerSetBit_EXINT(P7A_1A_CMU_00, ~_BIT2, 0x00);
    DELAY_5US_EXINT();
    ScalerSetBit_EXINT(P7A_1A_CMU_00, ~_BIT1, 0x00);
    DELAY_5US_EXINT();
    ScalerSetBit_EXINT(P7A_1A_CMU_00, ~_BIT0, 0x00);
    ...
    if(ScalerGetBit_EXINT(P7A_17_BANDGAP_03, _BIT1) == _BIT1)  // MHL Mode
    {
        if(ScalerGetBit_EXINT(P7A_1B_CMU_01, _BIT0) == _BIT0) // DCDR
        {
            ...
            DELAY_5US_EXINT();
            ScalerSetBit_EXINT(P7A_1A_CMU_00, ~_BIT5, 0x00);
            DELAY_5US_EXINT();
        ...
        else
        {
            ...
            ScalerSetBit_EXINT(P7A_1A_CMU_00, ~_BIT7, _BIT7);
            DELAY_5US_EXINT();
            ScalerSetBit_EXINT(P7A_1A_CMU_00, ~_BIT6, 0x00);
            DELAY_5US_EXINT();
            ScalerSetBit_EXINT(P7A_1A_CMU_00, ~_BIT5, 0x00);
            DELAY_5US_EXINT();
            ScalerSetBit_EXINT(P7A_1A_CMU_00, ~_BIT4, _BIT4);
    ...
    {
#if(_D1_HDMI_2_0_SUPPORT == _ON)
        if(GET_TMDS_RX1_HIGH_SPEED_MODE() == _FALSE)  // 1:10
#endif
        {
            if(GET_INPUT_PIXEL_CLK() <= 375)
            {
...
void ScalerTMDSRx1IntHandler_EXINT0(void) using 1
{
...
#if(_SOURCE_BACKGROUND_DETECT_SUPPORT == _ON)
    if(g_ucSourceDetectingPort == _D1_INPUT_PORT)
    {
    ...
    if(CHECK_TMDS_DPHY_SWITCH(_D1_INPUT_PORT) == _TRUE)
    {
        ...
        if((ScalerGetByte_EXINT(P71_EF_HDMI_ERROR_TH) & 0xC0) == 0xC0)
        {
            ...
            if((GET_TMDS_RX1_PHY_SET() == _FALSE) || ((bit)ScalerTMDSRx1CheckCDRStable_EXINT0(g_usTMDSRx1ClkCount) == _FALSE))
            {
                ...
                {
                    ScalerTimerWDActivateTimerEvent_EXINT0(500, _SCALER_WD_TIMER_EVENT_RX1_HDCP2_SET_REAUTH);
                }
#endif
            }
        ...
        if((ScalerGetByte_EXINT(P71_E6_HDMI_FREQDET_CTRL) & 0x06) == 0x06)
        {
            ...
            {
#if(_SOURCE_BACKGROUND_DETECT_SUPPORT == _ON)
                if(GET_STORE_ACTIVED_PORT() == _D1_INPUT_PORT)
#endif
                {
...
#if((_TMDS_HDCP_2_2_SUPPORT == _ON) && ((_D1_INPUT_PORT_TYPE == _D1_HDMI_PORT) || (_D1_INPUT_PORT_TYPE == _D1_MHL_PORT)))
                ScalerTimerWDCancelTimerEvent_EXINT0(_SCALER_WD_TIMER_EVENT_RX1_HDCP2_SET_REAUTH);
#endif
...
#if((_AUDIO_SUPPORT == _ON) && (_D1_HDMI_SUPPORT == _ON))
    ...
    if(ScalerGetBit_EXINT(P71_CA_HDMI_DP, (_BIT3 | _BIT1)) == (_BIT3 | _BIT1))
    {
        ...
        if(GET_AUDIO_D1_SAMPLING_FREQ() <= _AUDIO_FREQ_48K)
        {
            SET_AUDIO_D1_FW_TRACKING_RESET();
        }
#endif
        ...
        if(ScalerGetBit_EXINT(P71_CA_HDMI_DP, (_BIT3 | _BIT2 | _BIT1 | _BIT0)) == 0x00)
        {
            ...
            ScalerSetByte_EXINT(P71_CA_HDMI_DP, ScalerAudioGetUserSamplingFreqSCode_EXINT0(_PORT_HDMI, GET_D1_SAMPLING_FREQ_USER_DEFINE()));

            // Set D code User Define
            ScalerSetByte_EXINT(P71_C9_HDMI_AP, _P71_CA_PT_13_PORT_PAGE71_HDMI_DCAPR0);
            ScalerSetByte_EXINT(P71_CA_HDMI_DP, HIBYTE(ScalerAudioGetUserSamplingFreqDCode_EXINT0(_PORT_HDMI, GET_D1_SAMPLING_FREQ_USER_DEFINE())));
            ScalerSetByte_EXINT(P71_C9_HDMI_AP, _P71_CA_PT_14_PORT_PAGE71_HDMI_DCAPR1);
            ScalerSetByte_EXINT(P71_CA_HDMI_DP, LOBYTE(ScalerAudioGetUserSamplingFreqDCode_EXINT0(_PORT_HDMI, GET_D1_SAMPLING_FREQ_USER_DEFINE())));
...
bit ScalerTMDSRx1CheckCDRStable(WORD usClkCount)
{
...
#endif // End of #if(_D1_INPUT_PORT_TYPE == _D1_MHL_PORT)
    {
        ...
        if(GET_TMDS_RX1_HIGH_SPEED_MODE() == _TRUE)
        {
            ...
            if(abs(usClkCount - (ScalerTMDSRx1MeasureClk(_CLK_SOURCE_BLUE) * 4)) > (usClkCount >> 6))
            {
                // Restore INT Clock Check
                ScalerSetBit(P71_EF_HDMI_ERROR_TH, ~(_BIT7 | _BIT6 | _BIT5), (ucStableIrq & _BIT7));
                ScalerSetBit(P71_E6_HDMI_FREQDET_CTRL, ~(_BIT6 | _BIT2 | _BIT1), (ucUnstableIrq & _BIT1));
            ...
            if(abs(usClkCount - (ScalerTMDSRx1MeasureClk(_CLK_SOURCE_GREEN) * 4)) > (usClkCount >> 6))
            {
                // Restore INT Clock Check
                ScalerSetBit(P71_EF_HDMI_ERROR_TH, ~(_BIT7 | _BIT6 | _BIT5), (ucStableIrq & _BIT7));
                ScalerSetBit(P71_E6_HDMI_FREQDET_CTRL, ~(_BIT6 | _BIT2 | _BIT1), (ucUnstableIrq & _BIT1));
...
BYTE ScalerTMDSRx1CheckCDRStable_EXINT0(WORD usClkCount) using 1
{
...
#endif // End of #if(_D1_INPUT_PORT_TYPE == _D1_MHL_PORT)
    {
        ...
        if(GET_TMDS_RX1_HIGH_SPEED_MODE() == _TRUE)
        {
            ...
            if(abs(usClkCount - (ScalerTMDSRx1MeasureClk_EXINT0(_CLK_SOURCE_BLUE) * 4)) > (usClkCount >> 6))
            {
                return _FALSE;
            }

            if(abs(usClkCount - (ScalerTMDSRx1MeasureClk_EXINT0(_CLK_SOURCE_GREEN) * 4)) > (usClkCount >> 6))
            {
                return _FALSE;
            }
        }
...
#if(_HDMI_IDENTIFY_GAME_SRC_SUPPORT == _ON)
        ScalerTMDSRx1SpdInfoPacketDetect(_WAIT);
#endif
...
#if(_HDMI_IDENTIFY_GAME_SRC_SUPPORT == _ON)
        ScalerTMDSRx1SpdInfoPacketDetect(_NOT_WAIT);
#endif
...
BYTE ScalerTMDSRx1TMDSScanInputPort(BYTE ucInputType)
{
    ...
    if(GET_TMDS_RX1_PHY_SET() == _TRUE)
    {
...
#endif // End of #if(_D1_INPUT_PORT_TYPE == _D1_MHL_PORT)
        {
            ...
            {
                DebugMessageRx1("TMDS Rx2 First 1:10 set phy not success", 1);
        ...
        {
            ScalerTimerActiveTimerEvent(SEC(1), _SCALER_TIMER_EVENT_RX1_HDMI2_SCRAMBLE_DETECT_TIMEOUT);
        }
#endif
...
#else
            if(ScalerTMDSRx1TMDSVideoDetect(_NOT_WAIT) == _TMDS_VIDEO_DATA_NONE)
#endif
            {
...
void ScalerTMDSRx1HotPlugEvent(void)
{
    ...
    {
        ScalerTMDSRx1BeforeHotPlugEventProc(_TRUE);
    }
    else
    {
        ScalerTMDSRx1BeforeHotPlugEventProc(_FALSE);
    }
...
#endif
    {
        ...
        {
            ScalerTMDSRx1HotPlugHighProc(GET_HDMI_HOTPLUG_TIME());
        }
        else if(GET_TMDS_RX1_HPD_TRIGGER_EVENT() == _TMDS_HPD_HDCP_2_2)
        {
            ScalerTMDSRx1HotPlugHighProc(_TMDS_HDCP_2_2_HPD_TIME);
        }
        else if(GET_TMDS_RX1_HPD_TRIGGER_EVENT() == _TMDS_HPD_ACER)
        {
            ScalerTMDSRx1HotPlugHighProc(_TMDS_ACER_HPD_TIME);
        }
        else if(GET_TMDS_RX1_HPD_TRIGGER_EVENT() == _TMDS_HPD_EDID_SWITCH)
        {
            ScalerTMDSRx1HotPlugHighProc(_TMDS_EDID_SWITCH_HPD_TIME);
        }
        else
        {
            ScalerTMDSRx1HotPlugHighProc(GET_HDMI_HOTPLUG_TIME());
    ...
    if(GET_HDMI_Z0_HPD_TYPE() == _HDMI_HPD_Z0_TOGGLE_AT_SAME_TIME)
    {
        ...
        ScalerTMDSRx1AfterHotPlugEventProc(_TRUE);
    }
    else
    {
        // Restore Slave Address
        ScalerTMDSRx1AfterHotPlugEventProc(_FALSE);
    }
