/********************************************************************************/
/*   Copyright (c) 2021 Realtek Semiconductor Corp. All rights reserved.        */
/*                                                                              */
/*   SPDX-License-Identifier: LicenseRef-Realtek-Proprietary                    */
/*                                                                              */
/*   This software component is confidential and proprietary to Realtek         */
/*   Semiconductor Corp. Disclosure, reproduction, redistribution, in whole     */
/*   or in part, of this work and its derivatives without express permission    */
/*   is prohibited.                                                             */
/********************************************************************************/

//----------------------------------------------------------------------------------------------------
// ID Code      : User_Panel_include.h No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------

//--------------------------------------------------
// Panel Include
//--------------------------------------------------
#include "./EXAMPLE/PANEL_NONE.h"
#include "./EXAMPLE/PANEL_LVDS_EXAMPLE.h"
#include "./EXAMPLE/PANEL_DPTX_EXAMPLE.h"
#include "./EXAMPLE/PANEL_VBO_EXAMPLE.h"
#include "./EXAMPLE/PANEL_TTL_EXAMPLE.h"

#if(_PANEL_TYPE == _BOE_MV238FHB_NY1_LVDS)
#include "68_BOE_MV238FHB_NY1_LVDS.h"
#elif(_PANEL_TYPE == _BOE_MV238FHB_NF1_LVDS_FHD180)
#include "76_BOE_MV238FHB_NF1_LVDS_FHD180.h"
#elif(_PANEL_TYPE == _BOE_MV270FHB_NY0_LVDS)
#include "67_BOE_MV270FHB_NY0_LVDS.h"
#elif(_PANEL_TYPE == _INNOLUX_MV270KCA_D8B_EDP_QHD180HZ)
#include "69_INNOLUX_MV270KCA_D8B_EDP_QHD180HZ.h"
#elif(_PANEL_TYPE==_PANDA_LC215DTBA_LVDS_FHD75HZ)
#include "70_PANDA_LVDS_LC215DTBA.h"
#elif(_PANEL_TYPE==_BOE_DV245QHB_N20_EDP_QHD180HZ)
#include "71_BOE_DV245QHB_N20_EDP_QHD180HZ.h"
#elif(_PANEL_TYPE==_BOE_MV238FHB_N63_LVDS_FHD100HZ)
#include "72_BOE_MV238FHB_N63_LVDS_FHD100HZ.h"
#elif(_PANEL_TYPE==_BOE_ME270QHB_NF2_EDP_QHD180HZ)
#include "73_BOE_ME270QHB_NF2_EDP_QHD180HZ.h"
#elif(_PANEL_TYPE == _INNOLUX_MV270KCA_E8B_EDP_QHD180HZ)
#include "74_INNOLUX_MV270KCA_E8B_EDP_QHD180HZ.h"
#elif(_PANEL_TYPE==_BOE_HV320FHB_F4A_LVDS_FHD75HZ)
#include "75_BOE_HV320FHB_F4A.h"
#elif(_PANEL_TYPE==_CSOT_SG2701B084_LVDS_FHD200HZ)
#include "133_CSOT_SG2701B084_LVDS_FHD200HZ.h"
#elif(_PANEL_TYPE==_CSOT_SG2381B084_LVDS_FHD240HZ)
#include "134_CSOT_SG2381B084_LVDS_FHD240HZ.h"
#elif(_PANEL_TYPE==_BOE_MV270QH7B00_LVDS_QHD144HZ)
#include "135_BOE_MV270QH7B00_LVDS_QHD144HZ.h"



#else

#include "00_CMO_LVDS_17.h"
#include "01_CMO_LVDS_19_WXGA.h"
#include "02_AU_LVDS_21_5_WUXGA.h"
#include "03_AU_LVDS_27_WUXGA_FREESYNC.h"
#include "04_MULTIPANEL_LVDS_21_5_WUXGA.h"
#include "05_AU_LVDS_22_WSXGA.h"
#include "06_AU_LVDS_22_WSXGA_QC.h"
#include "07_AU_LVDS_23_WUXGA.h"
#include "08_AU_LVDS_24_WUXGA.h"
#include "09_AU_DP_21_5_WUXGA.h"
#include "10_LG_LED_DP_27_UHD.h"
#include "14_LG_LED_LVDS_27_WUXGA.h"
#include "15_SAMSUNG_LED_DP_15_6_UHD.h"
#include "16_AUO_LED_DP_32_UHD.h"
#include "17_SAMSUNG_LED_DP_23_6_UHD.h"
#include "18_INNOLUX_LED_VBO_28_UHD.h"
#include "19_LG_LED_LVDS_23_UHD.h"
#include "20_INNOLUX_LED_VBO_40_UHD.h"
#include "21_AU_LVDS_21_5_WUXGA_FREESYNC.h"
#include "22_LG_LED_DP_27_QQHD.h"
#include "23_LG_LED_DP_27_QHD.h"
#include "24_LG_LED_DP_27_QHD_QC.h"
#include "25_AU_DP_21_5_WUXGA_FREESYNC.h"
#include "26_INNOLUX_TTL_7.h"
#include "27_MULTIPANEL_DP_21_5_WUXGA.h"
#include "28_LG_LED_DP_17_QHD.h"
#include "29_LG_LED_DP_34_WFHD.h"
#include "30_AUO_eDP_27_UHD.h"
#include "31_AUO_VBO_27_WUXGA_FREESYNC.h"
#include "32_CSOT_LVDS_23_8_WUXGA_FREESYNC.h"
#include "33_BOE_LED_LVDS_27_QHD_FREESYNC.h"
#include "66_AUO_VBO_M250HTN01_0.h"

#endif


