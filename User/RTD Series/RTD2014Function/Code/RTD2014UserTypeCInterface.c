...
void UserInterfaceTypeCGetSnkInfo(BYTE ucInputPort, StructTypeCSnkInfo *pstTypeCSnkInfo)
{
    switch(ucInputPort)
    {
        case _D0_INPUT_PORT:
            ...
            // Sink Minimum PDP (100 * 0.1 = 10W) : Minimum Power Required by the Sink.
            // Note : Sink Minimum PDP Shall be less than or equal to the Sink Operational PDP.
            pstTypeCSnkInfo->usSnkMinPDP = 100;

            // Sink Operational PDP (100 * 0.1 = 10W) : Manufacturer Recommended PDP of the Sink.
            pstTypeCSnkInfo->usSnkOpePDP = 100;

            // Sink Maximum PDP (180 * 0.1 = 18W) : Highest Amount of Power the Sink Consumes Under Any Operating Condition.
            // Note : Sink Maximum PDP Shall Not be less than the Sink Operational PDP.
            pstTypeCSnkInfo->usSnkMaxPDP = 180;
...
EnumTypeCPdoNumber UserInterfaceTypeCGetEPRPdoCount(BYTE ucInputPort, EnumTypeCPowerRole enumPowerRole)
{
    ...
    switch(ucInputPort)
    {
        case _D0_INPUT_PORT:
            ...
            if(enumPowerRole == _TYPE_C_POWER_SRC)
            {
                // Default 2 EPR PDO : 28V@(PDP/28)A and 1 AVS PDO 15V~28V@(PDP/AVS Voltage)A
                return _TYPE_C_PDO_NUM_2;
            }
            else if(enumPowerRole == _TYPE_C_POWER_SNK)
            {
        ...
        case _D0_INPUT_PORT:
            ...
            switch(ucPdoIndex)
            {
                ...
                case _TYPE_C_PDO_NUM_3:
                    ...
                    pstTypeCUserSrcPdo->usMinVoltage = 30;
                    pstTypeCUserSrcPdo->usMaxVoltage = 90;
                    pstTypeCUserSrcPdo->usMaxCurrent = 300;
#endif
                    break;
...
EnumTypeCUnconstrainPowerExist UserInterfaceTypeCGetUnconstrainedPow(BYTE ucInputPort)
{
    ...
    // =======================================================================
    // User can Use "SysTypeCGetLocPowExist" to Get Local Power (Adaptor or Power Board) Status
    // User can Use "SysTypeCGetPowerRole" to Get Port's Power Role
    // User can Use "SysTypeCGetPdoStatus" to Get Port's PDO Status
    // User can Use "SysTypeCGetPartnerPdo" to Get Port Partner Source/Sink PDO Info
    // =======================================================================
    ...
    switch(ucInputPort)
    {
        case _D0_INPUT_PORT:
            ...
            // Use "SysTypeCGetLocPowExist" to Determine the Unconstrained Power Status.
            if(SysTypeCGetLocPowExist() == _TYPE_C_LOCAL_POWER_EXIST)
            {
                enumTypeCUPExist = _TYPE_C_UNCONSTRAIN_POWER_EXIST;
            }
            else if(SysTypeCGetLocPowExist() == _TYPE_C_LOCAL_POWER_NOT_EXIST)
            {
        ...
        case _D0_INPUT_PORT:
            ...
            switch(ucPdoIndex)
            {
                case _TYPE_C_PDO_NUM_1:
                    ...
                    pstTypeCUserProtectInfo->ucOvpDebounce = 0;
                    pstTypeCUserProtectInfo->ucUvpDebounce = 0;
                    pstTypeCUserProtectInfo->ucOcpDebounce = 0;

                    // Vbus OCP Calculation Include Peak Current or not
                    pstTypeCUserProtectInfo->enumPeakCurrentProtect = _TYPE_C_PEAK_CURRENT_PROTECT_ENABLE;
